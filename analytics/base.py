...
class BaseAnalyzer(ABC):
    ...
    def _filter_edges_by_weight(self, min_weight: float = None) -> List[tuple]:
        ...
        return [
            (u, v, data) for u, v, data in self._graph.edges(data=True)
...
class GraphMetrics:
    ...
    @staticmethod
    def calculate_centrality(graph: nx.Graph, measure: CentralityMeasure, top_k: int = 10) -> Dict[str, float]:
        ...
        # Return top K nodes
        return dict(sorted(centrality.items(), key=lambda x: x[1], reverse=True)[:top_k])
    ...
    @staticmethod
    def calculate_shortest_paths(graph: nx.Graph, source: str, targets: List[str] = None) -> Dict[str, int]:
