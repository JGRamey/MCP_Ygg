...
class EnhancedYggdrasilIntegrator:
    ...
    def _load_domain_ids(self, domain_key: str, csv_path_suffix: str = None):
        ...
        if csv_path_suffix:
            concepts_file = self.csv_path / csv_path_suffix / f"{domain_key}_concepts.csv"
        else:
        ...
        if concepts_file.exists():
            ...
            if not df.empty:
                ...
                self.existing_ids[domain_key] = max(existing_nums) if existing_nums else 0
            else:
    ...
    def estimate_art_date(self, art_form: str) -> int:
        ...
        art_dates = {
            'Paintings': -40000, 'Cave': -40000, 'Sculpture': -35000,
            'Drawing': -40000, 'Music': -40000, 'Dance': -30000,
            'Theatre': -2500, 'Photography': 1826, 'Digital': 1960,
            'Video': 1960, 'Film': 1890, 'Architecture': -10000,
            'Printmaking': 200, 'Installation': 1950, 'Performance': 1950
        }
    ...
    def integrate_domain(self, domain: str, subdomain: str = None) -> bool:
        ...
        success &= self.append_to_csv(csv_path, f'{file_prefix}_relationships', new_relationships)
    ...
    def run_integration(self) -> bool:
        ...
        logger.info(f"Integration complete: {success_count}/{total_operations} operations successful")
    ...
    def estimate_language_date(self, language_area: str) -> int:
        ...
        lang_dates = {
            'Grammar': -50000, 'Writing': -3100, 'Phonetics': -50000,
            'Etymology': -3000, 'Translation': -2000, 'Computation': 1950,
            'Applied': 1940, 'Pragmatics': -400, 'Semantics': -400
        }
    ...
    def estimate_math_date(self, math_area: str) -> int:
        ...
        math_dates = {
            'Arithmetic': -20000, 'Geometry': -3000, 'Algebra': -1800,
            'Calculus': 1665, 'Statistics': 1663, 'Logic': -350,
            'Analysis': 1821, 'Topology': 1736, 'Number Theory': -500,
            'Combinatorics': -300, 'Game Theory': 1944, 'Cryptography': -400,
            'Pure': -3000, 'Applied': -1000, 'Interdisciplinary': -300
        }
        ...
        for _, row in df.iterrows():
            ...
            if pd.notna(main_category) and main_category.strip():
                ...
                concepts.append({
                    ...
                    'earliest_evidence_date': self.estimate_philosophy_date(current_category),
            ...
            if pd.notna(subcategory) and subcategory.strip() and category_id:
                ...
                concepts.append({
                    ...
                    'type': 'branch',
                    'level': 3,
                    'description': description if pd.notna(description) else f"{subcategory_clean} within {current_category}",
                    'earliest_evidence_date': self.estimate_philosophy_date(subcategory_clean),
                    'earliest_evidence_type': 'Philosophical',
                    'location': 'Global',
                    'language': 'Various',
                    'properties': f"Subcategory of {current_category}",
    ...
    def estimate_philosophy_date(self, phil_area: str) -> int:
        ...
        phil_dates = {
            'Ethics': -2400, 'Wisdom': -2600, 'Classical': -600,
            'Metaphysics': -350, 'Logic': -350, 'Epistemology': -400,
            'Existentialism': 1841, 'Political': -400, 'Religion': -2500,
            'Eastern': -1500, 'Modern': 1600, 'Contemporary': 1900
        }
        ...
        for _, row in df.iterrows():
            ...
            if pd.notna(main_category) and main_category.strip():
                ...
                concepts.append({
                    ...
                    'earliest_evidence_date': self.estimate_religion_date(current_category),
            ...
            if pd.notna(subcategories) and subcategories.strip() and category_id:
                ...
                for subcat in subcat_items:
                    ...
                    concepts.append({
                        ...
                        'level': 3,
                        'description': subcat['description'],
                        'earliest_evidence_date': self.estimate_religion_date(subcat['name']),
                        'earliest_evidence_type': 'Religious',
                        'location': 'Global',
                        'language': 'Various',
                        'properties': f"Subcategory of {current_category}",
    ...
    def estimate_religion_date(self, religion_area: str) -> int:
        ...
        religion_dates = {
            'Christianity': 30, 'Islam': 610, 'Judaism': -1300,
            'Hinduism': -1500, 'Buddhism': -500, 'Zoroastrianism': -600,
            'Greek': -800, 'Roman': -700, 'Norse': -200,
            'Egyptian': -3100, 'Mesopotamian': -3500, 'Celtic': -500,
            'Monotheistic': -1300, 'Polytheistic': -3000
        }
    ...
    def map_science_schema(self, df: pd.DataFrame) -> Tuple[List[Dict], List[Dict]]:
        ...
        for _, row in df.iterrows():
            ...
            if pd.notna(discipline) and discipline.strip() and discipline not in ['NaN', '']:
                ...
                concepts.append({
                    ...
                    'earliest_evidence_date': self.estimate_science_date(current_discipline),
            ...
            # Subcategory (level 3)
            if pd.notna(subcategory) and subcategory.strip() and discipline_id and subcategory not in ['NaN', '']:
                subcategory_clean = subcategory.strip()
                ...
                concepts.append({
                    ...
                    'type': 'branch',
                    'level': 3,
                    'description': description if pd.notna(description) else f"{subcategory_clean} within {current_discipline}",
                    'earliest_evidence_date': self.estimate_science_date(subcategory_clean),
                    'earliest_evidence_type': 'Scientific',
                    'location': 'Global',
                    'language': 'Scientific',
    ...
    def estimate_science_date(self, science_area: str) -> int:
        ...
        science_dates = {
            'Astronomy': -1800, 'Physics': -600, 'Chemistry': -1500,
            'Biology': -400, 'Medicine': -2600, 'Mathematics': -20000,
            'Mechanics': -300, 'Quantum': 1900, 'Relativity': 1905,
            'Nuclear': 1896, 'Organic': 1828, 'Genetics': 1866,
            'Evolution': 1859, 'Thermodynamics': 1824
        }
    ...
    def map_astrology_schema(self, df: pd.DataFrame) -> Tuple[List[Dict], List[Dict]]:
        ...
        for _, row in df.iterrows():
            ...
            subcategory = row.get('Column 2', '') or row.get('Subcategory (if applicable)', '')
            description = row.get('Column 3', '') or row.get('Description', '')
            ...
            if pd.notna(category) and category.strip() and category not in ['NaN', '']:
                ...
                concepts.append({
                    ...
                    'earliest_evidence_date': self.estimate_astrology_date(current_category),
                    'earliest_evidence_type': 'Historical',
                    'location': 'Global',
                    'language': 'Various',
            ...
            # Subcategory (level 3)
            if pd.notna(subcategory) and subcategory.strip() and category_id and subcategory not in ['NaN', '', '-']:
                subcategory_clean = subcategory.strip()
                ...
                concepts.append({
                    ...
                    'level': 3,
                    'description': description if pd.notna(description) else f"{subcategory_clean} within {current_category}",
                    'earliest_evidence_date': self.estimate_astrology_date(subcategory_clean),
                    'earliest_evidence_type': 'Historical',
                    'location': 'Global',
                    'language': 'Various',
                    'properties': f"Subcategory of {current_category}",
    ...
    def estimate_astrology_date(self, astro_area: str) -> int:
        ...
        astro_dates = {
            'Astrology': -2000, 'Natal': -600, 'Horary': -400,
            'Electional': -400, 'Mundane': -600, 'Medical': -400,
            'Homeopathy': 1796, 'Phrenology': 1796, 'Numerology': -500,
            'Graphology': 1622
        }
    ...
    def map_technology_schema(self, df: pd.DataFrame) -> Tuple[List[Dict], List[Dict]]:
        ...
        for _, row in df.iterrows():
            ...
            if pd.notna(example) and example.strip():
                ...
                full_description = description if pd.notna(description) else f"Ancient technology: {example_clean}"
                ...
                concepts.append({
                    ...
                    'earliest_evidence_date': self.estimate_technology_date(example_clean),
                    'earliest_evidence_type': 'Archaeological',
                    'location': self.extract_location(example_clean),
                    'language': 'Various',
                    'properties': f"Ancient technology example",
                    'research_status': 'speculative' if 'hypothesis' in example_clean.lower() else 'confirmed'
                })
    ...
    def estimate_technology_date(self, tech_item: str) -> int:
        ...
        tech_dates = {
            ...
            'Abusir': -2500, 'Puma Punku': 600, 'Göbekli Tepe': -9600,
            'Antikythera': -100, 'Roman': -753, 'Egyptian': -3100,
            'Mesopotamian': -3500, 'Stone tools': -2600000
        }
    ...
    def extract_location(self, tech_item: str) -> str:
        ...
        locations = {
            'Egypt': 'Egypt', 'Bolivia': 'Bolivia', 'Turkey': 'Turkey',
            'Greek': 'Greece', 'Roman': 'Rome', 'Mesopotamian': 'Mesopotamia'
        }
