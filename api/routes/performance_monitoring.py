...
async def test_scraping_performance(request: PerformanceTestRequest):
    ...
    try:
        if not HighPerformanceScraper:
            raise HTTPException(status_code=500, detail="High-performance scraper not available")
        ...
        async with HighPerformanceScraper(
            ...
            logger.info(f"✅ Performance test completed: {result.success_rate:.1%} success, {avg_time:.2f}s avg")
...
async def run_benchmark(request: BenchmarkRequest):
    ...
    try:
        ...
        logger.info(f"🏁 Running benchmark: {request.iterations} iterations on {len(test_urls)} URLs")
        ...
        for iteration in range(request.iterations):
            async with HighPerformanceScraper(
                ...
                logger.info(f"Iteration {iteration + 1}: {iteration_result['success_rate']:.1%} success, {iteration_result['average_time']:.2f}s avg")
        
        # Calculate overall statistics
        all_success_rates = [r['success_rate'] for r in benchmark_results]
        all_avg_times = [r['average_time'] for r in benchmark_results if r['average_time'] > 0]
        ...
        overall_stats = {
            ...
            'iterations_meeting_target': sum(1 for r in benchmark_results if r['target_met'])
        }
        ...
        logger.info(f"🏆 Benchmark completed: {overall_stats['overall_success_rate']:.1%} success rate, {overall_stats['overall_avg_time']:.2f}s avg time")
...
async def get_current_metrics():
    ...
    try:
        ...
        system_info = {
            ...
            'last_test_timestamp': performance_history[-1]['timestamp'] if performance_history else None
        }
...
async def get_performance_targets():
    ...
    try:
        ...
        if performance_history:
            recent_tests = [t for t in performance_history if isinstance(t.get('timestamp'), datetime) and t['timestamp'] > datetime.now() - timedelta(hours=24)]
            
            if recent_tests:
                # Analyze recent performance
                recent_performance_tests = [t for t in recent_tests if t.get('test_type') != 'benchmark']
                
                if recent_performance_tests:
                    avg_max_time = sum(t.get('max_time', 0) for t in recent_performance_tests) / len(recent_performance_tests)
                    avg_success_rate = sum(t.get('success_rate', 0) for t in recent_performance_tests) / len(recent_performance_tests)
...
async def quick_performance_test():
    ...
    try:
        ...
        async with HighPerformanceScraper(max_concurrent=5, timeout=8) as scraper:
            ...
            logger.info(f"⚡ Quick test: {quick_result['status']} (max: {max_time:.2f}s)")
...
async def performance_health_check():
    ...
    try:
        health_status = {
            ...
            'last_test': performance_history[-1]['timestamp'].isoformat() if performance_history else None,
