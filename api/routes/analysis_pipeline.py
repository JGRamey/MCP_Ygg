...
try:
    from data.staging_manager import Priority, ProcessingStatus, StagingManager
...
class AnalysisRequest(BaseModel):
    ...
    processing_mode: AnalysisMode = Field(
        AnalysisMode.SEQUENTIAL, description="Processing mode"
    )
    agent_parameters: Optional[Dict[str, Any]] = Field(
...
async def run_analysis(request: AnalysisRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if content.processing_status in [
        ...
        estimated_duration = sum(
            agent_times.get(agent, 30) for agent in request.selected_agents
        )
        ...
        logger.info(
            f"Started analysis job {job_id} for submission {request.submission_id}"
        )
...
async def run_batch_analysis(
    ...
    try:
        ...
        if invalid_submissions:
            raise HTTPException(
                status_code=400, detail=f"Invalid submissions: {invalid_submissions}"
            )
        ...
        logger.info(
            f"Started batch analysis {batch_id} for {len(request.submission_ids)} submissions"
        )
...
async def list_batch_jobs(
    limit: int = Query(20, description="Maximum number of batches")
):
    """
...
async def aggregate_results_by_domain(status: str = "analyzed"):
    ...
    try:
        ...
        aggregates = await staging_manager.aggregate_results_by_domain(
            processing_status
        )
...
async def batch_process_by_priority(
    priority: str, background_tasks: BackgroundTasks, batch_size: int = 10
):
    """
    ...
    try:
        ...
        # Start background processing
        background_tasks.add_task(
            process_batch_background, priority_enum, mock_batch_processor, batch_size
        )
...
async def list_staged_content(
    ...
    try:
        ...
        if status:
            content_list = [
                c for c in content_list if c.processing_status.value == status
            ]
            ...
            results.append(
                {
                    ...
                    "content_preview": (
                        ...
                        if len(content.raw_content) > 200
...
async def approve_content(submission_id: str, reviewer_notes: Optional[str] = None):
    """
...
async def reject_content(
    submission_id: str, rejection_reason: str, reviewer_notes: Optional[str] = None
):
    """
    ...
    try:
        ...
        # Reject content
        await staging_manager.reject_content(
            submission_id, rejection_reason, reviewer_notes
        )
...
async def get_pipeline_metrics():
    ...
    try:
        ...
        completed_jobs = len(
            [j for j in active_jobs.values() if j.status == WorkflowStatus.COMPLETED]
        )
        failed_jobs = len(
            [j for j in active_jobs.values() if j.status == WorkflowStatus.FAILED]
        )
        running_jobs = len(
            [j for j in active_jobs.values() if j.status == WorkflowStatus.RUNNING]
        )
            ...
            content={
                ...
                "workflows": {
                    ...
                    "active_workflows": len(
                        [w for w in workflows.values() if w["status"] == "active"]
                    ),
                },
                "batch_processing": {
                    ...
                    "active_batches": len(
                        [
                            ...
                            if b.overall_status == WorkflowStatus.RUNNING
                        ]
                    ),
                },
            }
...
async def execute_analysis_pipeline(
    ...
    try:
        ...
        if processing_mode == AnalysisMode.PARALLEL:
            await execute_agents_parallel(
                job, content, selected_agents, agent_parameters
            )
        else:
...
async def execute_agents_parallel(
    ...
    for agent_type in selected_agents:
        task = asyncio.create_task(
            execute_single_agent(agent_type, content, agent_parameters)
        )
...
async def execute_single_agent(
    agent_type: AgentType, content, agent_parameters: Dict[str, Any]
):
...
async def execute_batch_analysis(
    ...
    try:
        ...
        logger.info(
            f"Batch analysis {batch_id} completed: {batch.completed_jobs}/{batch.total_jobs} successful"
        )
...
async def process_batch_background(priority: Priority, processor_func, batch_size: int):
    ...
    try:
        ...
        logger.info(
            f"Batch processing completed for {priority.value}: {results['processed']} processed, {results['failed']} failed"
        )
