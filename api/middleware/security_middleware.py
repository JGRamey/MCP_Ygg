...
class SecurityConfig:
    ...
    def __init__(self, config_path: str = "config/security.yaml"):
        ...
        self.access_token_expire_minutes = self.config["authentication"][
            "access_token_expire_minutes"
        ]
        self.refresh_token_expire_days = self.config["authentication"][
...
class TokenManager:
            ...
            try:
                self.redis_client.setex(
                    ...
                    int(
                        ...
                        if expires_delta
                        else self.config.access_token_expire_minutes * 60
                    ),
                    json.dumps(data),
                )
            except Exception as e:
    ...
    def verify_token(self, token: str, token_type: str = "access") -> TokenData:
        ...
        try:
            ...
            return TokenData(
                username=username, user_id=user_id, role=role, permissions=permissions
            )
...
class UserManager:
    ...
    def create_default_admin(self):
        ...
        hashed_password, salt = self.password_manager.get_password_hash(
            default_password
        )
...
class EncryptionManager:
    ...
    def decrypt_file(
        self, encrypted_file_path: Path, output_path: Optional[Path] = None
    ) -> Path:
