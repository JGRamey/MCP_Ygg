...
def create_concept_form(
    concept_data: Optional[Dict[str, Any]] = None, mode: str = "create"
) -> Dict[str, Any]:
    """
    ...
    st.markdown(
        f'<div class="form-header">{"Edit" if mode == "edit" else "Create"} Concept</div>',
    ...
    with st.form(f"concept_form_{mode}"):
            ...
            form_data["domain"] = st.selectbox(
                ...
                index=(
                    ...
                    if not concept_data
                        ...
                        "🔬 Science",
                        "💻 Technology",
                    ].index(concept_data.get("domain", "🎨 Art"))
                ),
        ...
        with col2:
            form_data["type"] = st.selectbox(
                ...
                index=(
                    ...
                    if not concept_data
                        ...
                        "Movement",
                        "Institution",
                    ].index(concept_data.get("type", "Person"))
                ),
            ...
            form_data["priority"] = st.selectbox(
                ...
                index=(
                    1
                    if not concept_data
                    else ["Low", "Medium", "High", "Critical"].index(
        ...
        with col1:
            form_data["submitted"] = st.form_submit_button(
                f"{'Update' if mode == 'edit' else 'Create'} Concept", type="primary"
            )
    ...
    if form_data.get("submitted") or form_data.get("save_draft"):
        form_data["is_valid"] = bool(
            form_data.get("title")
            and form_data.get("domain")
            and form_data.get("description")
        )
...
def create_upload_form(
    accepted_types: List[str] = None, max_file_size: int = 200
) -> Dict[str, Any]:
    """
    ...
    with st.form("file_upload_form"):
        ...
        upload_data["tags"] = st.text_input(
            "Tags (optional)", help="Comma-separated tags to apply to uploaded content"
        )
        ...
        for file in upload_data["files"]:
            ...
            if file.size > max_file_size * 1024 * 1024:
                upload_data["errors"].append(
                    f"File {file.name} exceeds {max_file_size}MB limit"
                )
                continue
            ...
            if file_ext not in accepted_types:
                upload_data["errors"].append(
                    f"File {file.name} has unsupported type: {file_ext}"
                )
                continue
...
def create_search_form(search_types: List[str] = None) -> Dict[str, Any]:
    ...
    st.markdown('<div class="form-section">', unsafe_allow_html=True)
    st.markdown(
        '<div class="form-header">🔍 Advanced Search</div>', unsafe_allow_html=True
    )

    with st.form("search_form"):
        # Search query
        search_data["query"] = st.text_input(
            "Search Query *", help="Enter your search terms or query"
        )
        ...
        with col1:
            search_data["search_type"] = st.selectbox(
                "Search Type", search_types, help="Choose the type of search to perform"
            )
        ...
        with st.expander("Advanced Options", expanded=False):
            if search_data.get("search_type") == "Text Search":
                ...
                search_data["max_depth"] = st.number_input(
                    "Max Relationship Depth", min_value=1, max_value=10, value=3
                )
        ...
        # Submit button
        search_data["submitted"] = st.form_submit_button("🔍 Search", type="primary")
...
def create_settings_form(current_settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    st.markdown('<div class="form-section">', unsafe_allow_html=True)
    st.markdown(
        '<div class="form-header">⚙️ Application Settings</div>', unsafe_allow_html=True
    )
