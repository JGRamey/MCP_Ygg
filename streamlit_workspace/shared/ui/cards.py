...
def create_metric_card(label: str, value: Union[str, int, float], 
    ...
    delta_html = f'<div class="metric-change {delta_class}">{delta}</div>' if delta else ''
    help_html = f'<div class="metric-help" title="{help_text}">ⓘ</div>' if help_text else ''
...
def create_data_card(title: str, content: Dict[str, Any], 
                    actions: Optional[List[Dict[str, str]]] = None,
                    card_type: str = "default") -> Dict[str, bool]:
    """
    ...
    for key, value in content.items():
        if isinstance(value, dict):
            ...
            if card_type == 'concept' and key in ['domain', 'type', 'created']:
                st.markdown(f'<div class="concept-meta">{key}: {value}</div>', 
...
def create_concept_card(concept: Dict[str, Any], 
                       show_relationships: bool = False,
                       editable: bool = True) -> Dict[str, bool]:
    """
...
def create_stats_grid(stats: Dict[str, Union[str, int, float]], 
                     columns: int = 4) -> None:
    """
