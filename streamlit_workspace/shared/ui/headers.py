...
def create_page_header(title: str, description: str, icon: str = "🌳", 
                      show_status: bool = True, status_info: Optional[Dict[str, Any]] = None) -> None:
    """
    ...
    if show_status and status_info:
        ...
        with col1:
            ...
            status_class = 'status-online' if neo4j_status == 'Connected' else 'status-offline'
            st.markdown(f'<span class="{status_class}">Neo4j: {neo4j_status}</span>', 
                       unsafe_allow_html=True)
        
        with col2:
            qdrant_status = status_info.get('qdrant', 'Unknown')
            status_class = 'status-online' if qdrant_status == 'Connected' else 'status-offline'
            st.markdown(f'<span class="{status_class}">Qdrant: {qdrant_status}</span>', 
                       unsafe_allow_html=True)
        
        with col3:
            redis_status = status_info.get('redis', 'Unknown')
            status_class = 'status-online' if redis_status == 'Connected' else 'status-offline'
            st.markdown(f'<span class="{status_class}">Redis: {redis_status}</span>', 
                       unsafe_allow_html=True)
...
def create_section_header(title: str, description: Optional[str] = None, 
                         expandable: bool = False, expanded: bool = True) -> Any:
    """
...
def create_page_navigation(current_page: str, available_pages: List[Dict[str, str]]) -> str:
    """
    ...
    selected = st.selectbox("Navigate to:", page_options, 
...
def create_status_header(title: str, status: str, last_update: Optional[datetime] = None,
    ...
    with col2:
        ...
        st.markdown(f'<span class="{status_class}">Status: {status.title()}</span>', 
                   unsafe_allow_html=True)
