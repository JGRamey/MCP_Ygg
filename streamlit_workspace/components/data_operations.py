...
class DataOperations:
        ...
        try:
            ...
            for i, file in enumerate(files):
                try:
                    ...
                    file_result = await self._process_single_file(file, processing_options)
                    ...
                    self.logger.error(f"Error processing file {file.name}: {file_error}")
                    failed_files.append({
                        'name': file.name,
            ...
            return {'processed': 0, 'failed': len(files) if files else 0, 'results': [], 'errors': []}
    ...
    def _validate_file(self, file: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(file, 'size') and file.size > max_size:
                return {'valid': False, 'error': f'File too large: {file.size / 1024 / 1024:.1f}MB (max 50MB)'}
    ...
    async def _process_single_file(self, file: Any, options: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def start_web_scraping(
        ...
        try:
            ...
            return {'scraped': 0, 'failed': len(urls) if urls else 0, 'results': [], 'errors': []}
        ...
        try:
            ...
            with st.spinner("Saving document..."):
                ...
                with col2:
                    ...
                    st.info(f"**Language:** {doc_data.get('language', 'Not specified')}")
    ...
    def import_batch_data(self, df: pd.DataFrame, mappings: Dict[str, str]) -> bool:
        ...
        try:
            ...
            missing_mappings = [field for field in required_mappings if mappings.get(field) == 'None']
