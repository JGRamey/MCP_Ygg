...
class UIComponents:
    ...
    def render_header(self):
        ...
        try:
            # Main header
            st.markdown(
                '<h1 class="main-header">🌳 MCP Server Dashboard</h1>',
            ...
            # Last refresh info
            if (
                hasattr(self.dashboard_state, "last_refresh")
                and self.dashboard_state.last_refresh
            ):
    ...
    def render_sidebar(self) -> str:
        ...
        try:
            ...
            selected_page = st.sidebar.radio(
                "Select Page", pages, key="main_navigation"
            )
    ...
    def create_metric_card(
        ...
        try:
            ...
            if delta:
                with col2:
                    delta_class = (
                        "status-good" if delta_color == "normal" else "status-error"
                    )
                    st.markdown(
    ...
    def create_data_card(
        self, title: str, content: str, actions: Optional[List[str]] = None
    ) -> None:
        """
                ...
                for i, action in enumerate(actions):
                    with cols[i]:
                        if st.button(action, key=f"action_{title}_{i}"):
                            st.session_state[
                                f"action_{action.lower().replace(' ', '_')}"
                            ] = True
...
def create_data_card(
    title: str, content: str, actions: Optional[List[str]] = None
) -> None:
    """Create a data card."""
    ui_components = UIComponents()
