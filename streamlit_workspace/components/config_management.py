...
class DashboardConfig:
    ...
    def _apply_config_data(self, config_data: Dict[str, Any]):
        ...
        if "dashboard" in config_data:
            ...
            self.refresh_interval = dash_config.get("refresh_interval", self.refresh_interval)
            self.max_display_items = dash_config.get("max_display_items", self.max_display_items)
            self.chart_height = dash_config.get("chart_height", self.chart_height)
            self.enable_real_time = dash_config.get("enable_real_time", self.enable_real_time)
        
        if "performance" in config_data:
            perf_config = config_data["performance"]
            self.cache_ttl = perf_config.get("cache_ttl", self.cache_ttl)
            self.max_concurrent_operations = perf_config.get("max_concurrent_operations", self.max_concurrent_operations)
    ...
    def _load_from_environment(self):
        ...
        self.query_timeout = int(os.getenv("DASHBOARD_QUERY_TIMEOUT", str(self.query_timeout)))
        ...
        try:
            ...
            for agent_name, (agent_class, description) in agent_configs.items():
                try:
                    ...
                    self.logger.error(f"Failed to initialize {agent_name}: {agent_error}")
                    self.agent_status[agent_name] = {
                        'status': 'failed',
            ...
            self.agents = agents
            self.logger.info(f"Agent initialization complete: {len(agents)}/{len(agent_configs)} agents active")
        ...
        try:
            if metric_name in self.performance_metrics:
                if metric_name == "avg_response_time":
                    ...
                    self.performance_metrics[metric_name] = (current_avg * (query_count - 1) + value) / query_count
                else:
