...
def show_growth_trends():
    ...
    with col4:
        growth_rate = (
            ((concepts_added[-1] - concepts_added[0]) / concepts_added[0] * 100)
            if concepts_added[0] > 0
            else 0
        )
...
def show_network_analysis():
    ...
    st.info(
        "🔄 Interactive network visualization would be displayed here using NetworkX and Plotly"
    )
...
def show_domain_analysis():
    ...
    if domains:
        ...
        df_domains["Percentage"] = (
            df_domains["concept_count"] / total_concepts * 100
        ).round(2)
        df_domains["Coverage Rating"] = df_domains["concept_count"].apply(
            lambda x: (
                "High"
                if x > total_concepts * 0.15
                else "Medium" if x > total_concepts * 0.08 else "Low"
            )
        )
        ...
        with col2:
            balanced_domains = len(
                df_domains[df_domains["Coverage Rating"] == "Medium"]
            )
        ...
        underrepresented = df_domains[df_domains["Coverage Rating"] == "Low"][
            "domain"
        ].tolist()
        if underrepresented:
            st.warning(
                f"⚠️ Consider expanding these domains: {', '.join(underrepresented)}"
            )

        overrepresented = df_domains[df_domains["Coverage Rating"] == "High"][
            "domain"
        ].tolist()
        if overrepresented:
