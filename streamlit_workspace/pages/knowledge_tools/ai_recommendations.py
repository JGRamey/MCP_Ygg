...
def show_relationship_suggestions():
    ...
    if st.button("🤖 Generate Relationship Suggestions", type="primary"):
        with st.spinner("Analyzing content and generating relationship suggestions..."):
            ...
            st.success(
                f"✅ Generated {len(filtered_suggestions)} relationship suggestions"
            )
...
def show_missing_concept_suggestions():
    ...
    if st.button("🔍 Analyze Missing Concepts", type="primary"):
        with st.spinner("Analyzing knowledge gaps and identifying missing concepts..."):
            ...
            if focus_domains:
                missing_concepts = [
                    c for c in missing_concepts if c["domain"] in focus_domains
                ]
            ...
            for concept in missing_concepts:
                ...
                with col1:
                    if st.button(
                        f"🏗️ Create {concept['name']}", key=f"create_{concept['name']}"
                    ):
                        st.session_state.wizard_step = 1
                        st.session_state.suggested_concept = concept
                        st.info(
                            f"Concept creation wizard would pre-populate with {concept['name']}"
                        )
                with col2:
        ...
        with st.spinner("Analyzing concept descriptions and generating tags..."):
            ...
            sample_tags = [
                ...
                },
                {
                    ...
                    "tags": ["15th-16th Century", "European Art", "Cultural Movement"],
                    "confidence": 0.93,
                },
            ]
...
def show_improvement_suggestions():
    ...
    priority_filter = st.selectbox(
        "Priority Filter", ["All", "High Only", "Medium+", "Low Only"]
    )
    ...
    if st.button("🔍 Analyze Improvement Opportunities", type="primary"):
        with st.spinner("Analyzing concepts for improvement opportunities..."):
            ...
            if improvement_types:
                improvements = [
                    i for i in improvements if i["type"] in improvement_types
                ]
            ...
            if priority_filter != "All":
                if priority_filter == "High Only":
                    ...
                    improvements = [
                        i for i in improvements if i["impact"] in ["High", "Medium"]
                    ]
            ...
            if improvements:
                ...
                for imp in improvements:
                    impact_counts[imp["impact"]] = (
                        impact_counts.get(imp["impact"], 0) + 1
                    )
                    effort_counts[imp["effort"]] = (
                        effort_counts.get(imp["effort"], 0) + 1
                    )
