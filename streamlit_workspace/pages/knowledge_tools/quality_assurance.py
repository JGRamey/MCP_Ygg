...
def display_quality_results(quality_results):
    ...
    for issue_type, issues in quality_results.items():
        if issues:
            severity = (
                "high" if len(issues) > 10 else "medium" if len(issues) > 5 else "low"
            )

            with st.expander(
                f"🚨 {issue_type.replace('_', ' ').title()} ({len(issues)} issues)",
                expanded=len(issues) > 0,
            ):
                st.markdown(
                    f'<div class="issue-card issue-{severity}">', unsafe_allow_html=True
                )
    ...
    for i, concept1 in enumerate(concepts):
        ...
        for j, concept2 in enumerate(concepts[i + 1 :], i + 1):
            ...
            similarity = SequenceMatcher(
                None, concept1["name"].lower(), concept2["name"].lower()
            ).ratio()
...
def run_relationship_validation():
    ...
    if st.button("🔍 Validate Relationships"):
        ...
        with col1:
            ...
            st.metric(
                "Circular References",
                f"{status} {validation_results['circular_references']}",
            )
        ...
        with col2:
            ...
            st.metric(
                ...
                f"{status} {validation_results['broken_relationships']}",
            )

        with col3:
            status = "✅" if validation_results["missing_parents"] == 0 else "❌"
            st.metric(
                "Missing Parents", f"{status} {validation_results['missing_parents']}"
            )
        ...
        with col4:
            ...
            st.metric(
                "Invalid Strengths",
                f"{status} {validation_results['invalid_strengths']}",
            )
