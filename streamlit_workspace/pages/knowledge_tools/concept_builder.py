...
from utils.database_operations import create_concept, get_concept_by_id, search_concepts
...
def show_wizard_step_1():
    ...
    with col2:
        if st.button(
            "Next →",
            type="primary",
            disabled=not all([concept_name, domain, concept_id]),
        ):
...
def show_wizard_step_2():
    ...
    st.info(
        f"💡 For {domain} domain: Level 1 = {domain}, Level 2 = Major categories, Level 3+ = Specific concepts"
    )
    ...
    with col2:
        if st.button("Next →", type="primary"):
            st.session_state.wizard_data.update({"type": concept_type, "level": level})
...
def show_wizard_step_3():
    ...
    with col1:
        ...
        cultural_context = st.text_input(
            "Cultural Context", placeholder="e.g., Ancient Greek, Medieval European"
        )

    with col2:
        earliest_date = st.number_input(
            "Earliest Evidence Date", value=None, placeholder="Year"
        )
        latest_date = st.number_input(
            "Latest Evidence Date", value=None, placeholder="Year"
        )
...
def show_wizard_step_4():
    ...
    parent_search = st.text_input(
        "Search for parent concept", placeholder="Start typing to search..."
    )
    ...
    related_search = st.text_input(
        "Search for related concepts", placeholder="Start typing to search..."
    )
    ...
    if related_search:
        ...
        if related_candidates:
            ...
            selected_related = st.multiselect(
                "Select Related Concepts", related_options
            )
...
def show_wizard_step_5():
    ...
    for check, result in validation_results.items():
        if result["status"] == "pass":
            st.markdown(
                f'<div class="validation-result validation-pass">✅ {check}: {result["message"]}</div>',
                unsafe_allow_html=True,
            )
        elif result["status"] == "warn":
            st.markdown(
                f'<div class="validation-result validation-warn">⚠️ {check}: {result["message"]}</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<div class="validation-result validation-fail">❌ {check}: {result["message"]}</div>',
                unsafe_allow_html=True,
            )
    ...
    with col2:
        all_passed = all(
            r["status"] in ["pass", "warn"] for r in validation_results.values()
        )
...
def show_bulk_import():
    ...
    import_method = st.radio(
        "Import Method", ["📋 CSV Upload", "📝 Text List", "🔗 API Import"]
    )
    ...
    if import_method == "📋 CSV Upload":
            ...
            try:
                ...
                for req_col in required_columns:
                    mapping[req_col] = st.selectbox(
                        f"Map to '{req_col}'", df.columns.tolist()
                    )
        ...
        concept_type = st.selectbox(
            "Default Type", ["root", "sub_root", "branch", "leaf"]
        )
...
def show_concept_cloner():
    ...
    source_search = st.text_input(
        "Search for concept to clone", placeholder="Enter concept name or ID..."
    )
    ...
    if source_search:
        ...
        if candidates:
            ...
            selected_candidate = st.selectbox(
                "Select Concept to Clone", candidate_options
            )
                ...
                if source_concept:
                    ...
                    with st.form("clone_form"):
                        new_name = st.text_input(
                            "New Concept Name", value=f"{source_concept['name']}_Copy"
                        )
                        new_id = st.text_input(
                            "New Concept ID",
                            value=generate_concept_id(
                                new_name, source_concept["domain"]
                            ),
                        )

                        # What to copy
                        copy_relationships = st.checkbox(
                            "Copy Relationships", value=False
                        )
