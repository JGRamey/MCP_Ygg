...
class FileManagerApp:
    ...
    def __init__(self):
        ...
        if "file_manager_initialized" not in st.session_state:
            ...
            st.session_state.database_filters = DatabaseFilters(
                domains=[], content_types=[]
            )
    ...
    def _render_sidebar(self):
        ...
        with st.sidebar:
            ...
            st.session_state.database_filters = {
                "domains": [
                    d.split(" ", 1)[1].lower() for d in selected_domains
                ],  # Remove emoji and lowercase
            ...
            neo4j_status = (
                "🟢 Connected" if self.neo4j_manager.is_connected() else "🔴 Offline"
            )
            st.write(f"**Neo4j**: {neo4j_status}")

            qdrant_status = (
                "🟢 Connected" if self.qdrant_manager.is_connected() else "🔴 Offline"
            )
            st.write(f"**Qdrant**: {qdrant_status}")

            # CSV files status
            csv_files = self.csv_manager.get_csv_files(
                DatabaseFilters(domains=[], content_types=[])
            )
    ...
    def _show_quick_stats(self):
        ...
        with col2:
            ...
            if self.neo4j_manager.is_connected():
                try:
                    result = self.neo4j_manager.graph.run(
                        "MATCH (n) RETURN count(n) as count"
                    ).data()
                    node_count = result[0]["count"] if result else 0
                    st.metric("Neo4j Nodes", f"{node_count:,}")

                    rel_result = self.neo4j_manager.graph.run(
    ...
    def _render_scraped_content_interface(self, filters: DatabaseFilters):
        ...
        st.info(
            "📋 Scraped content management would integrate with the staging system from Phase 4."
        )
    ...
    def _render_csv_to_neo4j_import(self):
        ...
        if selected_csv:
            ...
            selected_file = next(
                f for f in csv_files if f.domain == domain and f.name == filename
            )
            ...
            with col1:
                ...
                batch_size = st.number_input(
                    "Batch Size", value=100, min_value=10, max_value=1000
                )
            ...
            if st.button("🚀 Start Import", type="primary"):
                self._execute_csv_import(
                    selected_file, node_label, batch_size, skip_duplicates
                )
    ...
    def _execute_csv_import(
        self, csv_file, node_label: str, batch_size: int, skip_duplicates: bool
    ):
        ...
        try:
            ...
            for i in range(0, total_rows, batch_size):
                ...
                for _, row in batch_df.iterrows():
                    try:
                        ...
                        st.warning(
                            f"Error importing row {i + imported_count}: {str(e)}"
                        )
            ...
            st.success(
                f"✅ Successfully imported {imported_count} records as {node_label} nodes!"
            )

            # Add to history
            add_to_history(
                f"Imported {imported_count} records from {csv_file.name} to Neo4j"
            )
