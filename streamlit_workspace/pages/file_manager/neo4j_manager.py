...
from .models import DatabaseFilters, Neo4jNodeInfo
...
class Neo4jManager:
    ...
    def _show_statistics(self):
        ...
        try:
            ...
            with col2:
                st.metric(
                    "Total Relationships", f"{overall_stats['total_relationships']:,}"
                )
    ...
    def _show_query_interface(self):
        ...
        query_type = st.selectbox(
            "Query Type", ["Custom"] + list(predefined_queries.keys())
        )

        if query_type == "Custom":
            query = st.text_area(
                "Cypher Query", height=100, placeholder="MATCH (n) RETURN n LIMIT 10"
            )
        else:
        ...
        if st.button("Execute Query") and query:
            try:
                ...
                if result:
                    ...
                    st.download_button(
                        "Download Results", csv, "neo4j_query_results.csv", "text/csv"
                    )
                else:
