...
from .models import QdrantCollectionInfo, DatabaseFilters
...
class QdrantManager:
    ...
    def render_qdrant_interface(self, filters: DatabaseFilters):
        ...
        if not QDRANT_AVAILABLE:
            st.error("❌ qdrant-client not available. Install with: `pip install qdrant-client`")
            return
        ...
        qdrant_tabs = st.tabs(["📊 Collections", "🔍 Search", "📈 Analytics", "⚙️ Management"])
    ...
    def _render_collections_tab(self):
        ...
        with col4:
            avg_size = sum(c.vector_size for c in collections) / len(collections) if collections else 0
    ...
    def _render_search_tab(self):
        ...
        with col2:
            search_limit = st.number_input("Result Limit", value=10, min_value=1, max_value=100)
        ...
        if st.button("🔍 Search Vectors") and search_query:
            self._perform_semantic_search(selected_collection, search_query, search_limit, search_threshold)
    ...
    def _render_analytics_tab(self):
        ...
        size_data = pd.DataFrame([
            {'Collection': c.name, 'Vectors': c.vectors_count, 'Dimension': c.vector_size}
        ...
        if len(dimension_counts) > 1:
            ...
            st.info(f"All collections use {dimension_counts.index[0]}-dimensional vectors.")
        ...
        for collection in collections:
            ...
            estimated_mb = (collection.vectors_count * collection.vector_size * 4) / (1024 * 1024)
        ...
        try:
            ...
            if response[0]:  # Has records
                ...
                for point in response[0]:
                    sample_data.append({
                        'ID': str(point.id),
                        'Payload Keys': ', '.join(point.payload.keys()) if point.payload else 'None'
                    })
    ...
    def _search_collection(self, collection: QdrantCollectionInfo):
        ...
        st.info("Semantic search functionality would be implemented here with actual vector embeddings.")
    ...
    def _show_collection_analytics(self, collection: QdrantCollectionInfo):
        ...
        # Basic metrics
        estimated_size_mb = (collection.vectors_count * collection.vector_size * 4) / (1024 * 1024)
    ...
    def _perform_semantic_search(self, collection_name: str, query: str, limit: int, threshold: float):
        ...
        mock_results = pd.DataFrame({
            ...
            'Metadata': ['Type: Article', 'Type: Book', 'Type: Paper', 'Type: Note']
        })
    ...
    def _render_create_collection_form(self):
        ...
        with col1:
            collection_name = st.text_input("Collection Name", placeholder="documents_philosophy")
            vector_size = st.number_input("Vector Dimension", value=384, min_value=1, max_value=4096)
    ...
    def _render_collection_operations(self):
        ...
        if collections:
            selected_collection = st.selectbox(
                "Select Collection for Operations",
                [c.name for c in collections]
            )
