...
Handles CSV database file operations via API
"""
...
class CSVManager:
    ...
    def _render_domain_section(self, domain: str, files: List[CSVFileInfo]):
        ...
        with st.expander(
            f"📁 {domain.title()} Domain ({len(files)} files)", expanded=True
        ):
    ...
    def _view_csv_file(self, file_info: CSVFileInfo):
        ...
        try:
            ...
            st.write(
                f"**Size**: {file_info.row_count:,} rows × {file_info.column_count} columns"
            )
    ...
    def _edit_csv_file(self, file_info: CSVFileInfo):
        ...
        try:
            ...
            st.warning(
                "⚠️ Changes will modify the original file. A backup will be created."
            )
                ...
                if st.button("💾 Save Changes", type="primary"):
                    try:
                        ...
                        backup_path = (
                            ...
                            / f"{file_info.path.stem}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                        )
        ...
        if st.button("🚀 Start Import", type="primary"):
            try:
                ...
                for i in range(0, total_rows, batch_size):
                    ...
                    status_text.text(
                        f"Importing rows {i+1}-{batch_end} of {total_rows}..."
                    )
                    progress_bar.progress((batch_end) / total_rows)

                st.success(
                    f"✅ Successfully imported {total_rows} records as {import_as} nodes"
                )
    ...
    def _show_csv_statistics(self):
        ...
        for file_info in all_files:
            ...
            domain_stats[file_info.domain]["size_mb"] += file_info.size_bytes / (
                1024 * 1024
            )
