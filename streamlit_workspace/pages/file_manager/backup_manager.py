...
class BackupManager:
    ...
    def render_backup_interface(self, filters: DatabaseFilters):
        ...
        backup_tabs = st.tabs(["📤 Create Backup", "📥 Restore Backup", "📋 Backup History", "📊 Import Data"])
    ...
    def _render_create_backup_tab(self):
        ...
        if st.button("🚀 Create Backup", type="primary") and backup_name:
            self._create_backup(backup_name, backup_description, backup_contents, compression)
        ...
        if uploaded_backup:
            ...
            with col1:
                restore_mode = st.radio(
                    "Restore Mode",
                    ["Replace existing data", "Merge with existing data", "Preview only"]
                )
            
            with col2:
                create_backup_before = st.checkbox("Create backup before restore", value=True)
    ...
    def _render_backup_history_tab(self):
        ...
        for backup in backups:
            backup_data.append({
                ...
                'Size': backup['size'],
                'Description': backup['description'][:50] + '...' if len(backup['description']) > 50 else backup['description'],
    ...
    def _create_backup(self, name: str, description: str, contents: List[str], compression: str):
        """Create a new backup."""
    ...
    def _render_staging_import(self):
        ...
        if staging_content:
            ...
            if st.button("📥 Import Staging Content"):
                self._import_staging_content(staging_content, import_to, auto_categorize)
        else:
    ...
    def _import_json_data(self, json_data: Dict):
        """Import JSON data."""
        st.info("JSON import functionality would process the data structure and import to appropriate databases.")
        st.success("✅ JSON data import completed!")
    
    def _import_staging_content(self, content: List[ScrapedContentInfo], import_to: str, auto_categorize: bool):
