...
def main():
    ...
    st.markdown("**Complete project file and configuration management**")
    ...
    with st.sidebar:
        ...
        operation = st.selectbox(
            "Select Operation",
            ["🗂️ Browse Files", "📝 Edit CSV Files", "⚙️ Configuration Editor", "🐙 Git Management", "💾 Backup Manager"]
        )
        ...
        if st.button("📊 CSV Files", use_container_width=True):
            st.session_state.browse_path = project_root / "CSV"
            st.rerun()
        
        if st.button("🔧 Config Files", use_container_width=True):
            st.session_state.browse_path = project_root / "config"
            st.rerun()
        
        if st.button("🤖 Agent Files", use_container_width=True):
            st.session_state.browse_path = project_root / "agents"
            st.rerun()
        
        if st.button("📜 Scripts", use_container_width=True):
            st.session_state.browse_path = project_root / "scripts"
            st.rerun()
    
    # Main content based on operation
    if operation == "🗂️ Browse Files":
        show_file_browser(project_root)
    elif operation == "📝 Edit CSV Files":
        show_csv_editor(project_root)
    elif operation == "⚙️ Configuration Editor":
        show_config_editor(project_root)
    elif operation == "🐙 Git Management":
        show_git_management(project_root)
    elif operation == "💾 Backup Manager":
        show_backup_manager(project_root)

def show_file_browser(project_root):
    """Show file browser interface"""
    st.markdown("## 🗂️ Project File Browser")
    
    # Current path
    current_path = st.session_state.get('browse_path', project_root)
