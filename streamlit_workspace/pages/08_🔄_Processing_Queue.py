...
class ProcessingQueueManager:
    ...
    async def approve_item(self, submission_id: str, reviewer: str, reason: str):
        """Approve an item"""
        return await self.staging_manager.approve_content(
            submission_id, reviewer, reason
        )

    async def reject_item(self, submission_id: str, reviewer: str, reason: str):
        """Reject an item"""
        return await self.staging_manager.reject_content(
            submission_id, reviewer, reason
        )
        ...
        if queue_stats:
            ...
            if queue_stats["total_items"] > 0:
                ...
                for i, (status, count) in enumerate(
                    queue_stats["queue_counts"].items()
                ):
                    if count > 0:
                ...
                fig.update_layout(
                    title="Processing Queue Distribution", showlegend=True, height=400
                )
...
def display_items_by_status(status: ProcessingStatus):
    ...
    try:
        ...
        with col2:
            limit = st.selectbox(
                "Show items:", [10, 25, 50, 100], index=1, key=f"limit_{status.value}"
            )
        with col3:
...
def display_item_card(item: StagedContent, status: ProcessingStatus):
    ...
    with st.expander(f"📄 {item.metadata.title[:60]}... - {item.submission_id[:8]}"):
        ...
        with col2:
            ...
            if status == ProcessingStatus.ANALYZED:
                ...
                reviewer = st.text_input(
                    "Reviewer:", key=f"reviewer_{item.submission_id}"
                )
            ...
            export_format = st.selectbox(
                "Format:", ["json", "text"], key=f"export_format_{item.submission_id}"
            )
        ...
        # Analysis results
        if item.analysis_results and st.checkbox(
            "🔍 Analysis Results", key=f"analysis_{item.submission_id}"
        ):
...
def display_analytics():
    ...
    try:
                    ...
                    timeline_data.append(
                        {
                            "date": datetime.fromisoformat(
                                item.timestamps["submitted"].replace("Z", "")
                            ),
                            "status": status_name,
        ...
        if timeline_data:
            ...
            daily_counts = (
                df.groupby([df["date"].dt.date, "status"])
                .size()
...
def display_batch_operations():
        ...
        if st.button("🗑️ Cleanup Old Items"):
            try:
                ...
                cleaned_count = loop.run_until_complete(
                    st.session_state.queue_manager.staging_manager.cleanup_old_items(
                        cleanup_days
                    )
                )
                loop.close()
    ...
    with col2:
        ...
        export_format_batch = st.selectbox("Export format:", ["json", "csv", "text"])
...
def main():
    ...
    st.set_page_config(page_title="Processing Queue", page_icon="🔄", layout="wide")
