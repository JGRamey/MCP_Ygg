...
from .models import (
    GraphMode, LayoutType, GraphFilters, RelationshipFilters, 
    DOMAIN_OPTIONS, NODE_TYPE_OPTIONS
)
...
class GraphEditorUI:
    ...
    def render_sidebar_controls(self) -> tuple[str, str, int, int]:
        ...
        with st.sidebar:
            ...
            edge_width = st.slider("Edge Width", 1, 5, 2, help="Width of relationship lines")
    ...
    def render_graph_filters(self):
        ...
        level_range = st.slider(
            "Level Range", 1, 10, (1, 5),
            help="Filter by concept depth in hierarchy"
        )
        ...
        min_relationship_strength = st.slider(
            "Min Relationship Strength", 0.0, 1.0, 0.0,
            help="Filter weak relationships"
        )
    ...
    def render_concept_search(self) -> Optional[str]:
        ...
        if concept_search:
            ...
            try:
                ...
                concepts, _ = self.data_source_manager.get_concepts_with_fallback(limit=100)
                
                matching_concepts = [
                    c for c in concepts 
                    if concept_search.lower() in c['name'].lower() or concept_search.lower() in c['id'].lower()
                ]
    ...
    def render_relationship_analysis(self, concepts: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
...
class GraphReportGenerator:
    ...
    def generate_graph_report(concepts: List[Dict[str, Any]], analysis: Dict[str, Any]):
        ...
        st.markdown(f"**Total Relationships:** {analysis.get('total_relationships', 0)}")
        st.markdown(f"**Average Connections:** {analysis.get('avg_connections', 0):.2f}")
