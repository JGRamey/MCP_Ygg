...
class GraphEditorOrchestrator:
    ...
    def run(self):
        ...
        graph_mode, layout_type, node_size, edge_width = (
            self.ui.render_sidebar_controls()
        )

        # Create settings object
        settings = GraphSettings(
            layout_type=layout_type, node_size=node_size, edge_width=edge_width
        )
    ...
    def _render_full_network_view(self, settings: GraphSettings):
        ...
        with col1:
            ...
            try:
                ...
                concepts, data_source = self.data_manager.get_concepts_with_fallback(
                    filters
                )
                ...
                relationship_filters = st.session_state.get(
                    "relationship_filters", {"show": True}
                )
                fig = self.visualizer.create_network_graph(
                    concepts, settings, relationship_filters
                )
        ...
        with col2:
            ...
            if "selected_concept_id" in st.session_state:
                ...
                concept = self.data_manager.neo4j_connector.get_concept_by_id(
                    concept_id
                )
                if concept:
    ...
    def _render_focused_view(self, settings: GraphSettings):
        ...
        if concept_id:
            fig = self.visualizer.create_focused_graph(
                concept_id, focus_depth, settings
            )
    ...
    def _render_domain_explorer_view(self, settings: GraphSettings):
        ...
        with col1:
            selected_domain = st.selectbox(
                "Select Domain", [d["domain"] for d in domains]
            )

        with col2:
            show_cross_domain = st.checkbox(
                "Show Cross-Domain Connections", value=False
            )
        ...
        if selected_domain:
            ...
            if domain_concepts:
                ...
                fig = self.visualizer.create_network_graph(
                    domain_concepts, settings, relationship_filters
                )
                ...
                if fig:
                    ...
                    with st.expander("📊 Domain Statistics", expanded=True):
                        self.ui.render_domain_statistics(
                            selected_domain, domain_concepts
                        )
                else:
    ...
    def _render_relationship_analysis_view(self):
        ...
        try:
            ...
            with metrics_col3:
                st.metric(
                    "Avg Connections", f"{analysis.get('avg_connections', 0):.1f}"
                )
    ...
    def _render_cross_cultural_view(self, settings: GraphSettings):
        ...
        st.markdown(
            "**Discover concepts and relationships that transcend domain boundaries**"
        )

        try:
            # Get cross-cultural data from the data manager
            cross_concepts, cross_relationships, data_source = (
                self.data_manager.get_cross_cultural_data()
            )
            ...
            with tab1:
                ...
                if cross_concepts:
                    ...
                    for concept in cross_concepts:
                        with st.expander(
                            f"🌟 {concept['concept_name']} ({concept['domain_count']} domains)"
                        ):
                            col1, col2 = st.columns(2)

                            with col1:
                                st.write(
                                    f"**Domains**: {', '.join(concept['domains'])}"
                                )
                                if concept.get("description1"):
                                    st.write(
                                        f"**Description**: {concept['description1']}"
                                    )

                            with col2:
                                # Create a simple domain distribution chart
                                domain_data = {
                                    domain: 1 for domain in concept["domains"]
                                }
                                import pandas as pd
                    ...
                    with col2:
                        avg_domains = (
                            sum(c["domain_count"] for c in cross_concepts)
                            / len(cross_concepts)
                            if cross_concepts
                            else 0
                        )
                    ...
                    with col3:
                        max_domains = max(
                            (c["domain_count"] for c in cross_concepts), default=0
                        )
                ...
                if cross_relationships:
                    ...
                    for rel in cross_relationships[:20]:  # Limit to first 20
                        with st.expander(
                            ...
                            with col1:
                                st.write(
                                    f"**Source**: {rel['source_concept']} ({rel['source_domain']})"
                                )
                            ...
                            with col2:
                                st.write(
                                    f"**Target**: {rel['target_concept']} ({rel['target_domain']})"
                                )
                    ...
                    with col1:
                        ...
                        rel_df = pd.DataFrame(
                            list(rel_types.items()), columns=["Type", "Count"]
                        )
                    ...
                    with col2:
                        ...
                        domain_df = pd.DataFrame(
                            list(domain_pairs.items()), columns=["Connection", "Count"]
                        )
                    ...
                    st.markdown(
                        "- Adding relationships between concepts in different domains"
                    )
            ...
            with tab3:
                ...
                if cross_concepts:
                    ...
                    universal_concepts = sorted(
                        cross_concepts, key=lambda x: x["domain_count"], reverse=True
                    )[:10]

                    for i, concept in enumerate(universal_concepts, 1):
                        st.write(
                            f"{i}. **{concept['concept_name']}** - {concept['domain_count']} domains: {', '.join(concept['domains'])}"
                        )
                ...
                if cross_relationships:
                    ...
                    interaction_matrix = {
                        source: {target: 0 for target in domains} for source in domains
                    }

                    for rel in cross_relationships:
                        interaction_matrix[rel["source_domain"]][
                            rel["target_domain"]
                        ] += 1
                ...
                if cross_concepts or cross_relationships:
                    ...
                    if cross_concepts:
                        most_universal = max(
                            cross_concepts, key=lambda x: x["domain_count"]
                        )
                        insights.append(
                            f"🏆 Most universal concept: **{most_universal['concept_name']}** appears in {most_universal['domain_count']} domains"
                        )

                    if cross_relationships:
                        insights.append(
                            f"🔗 Found {len(cross_relationships)} cross-domain relationships"
                        )
                        ...
                        for rel in cross_relationships:
                            key = tuple(
                                sorted([rel["source_domain"], rel["target_domain"]])
                            )
                            domain_connections[key] = domain_connections.get(key, 0) + 1

                        if domain_connections:
                            most_connected = max(
                                domain_connections.items(), key=lambda x: x[1]
                            )
                            insights.append(
                                f"🤝 Strongest domain connection: **{most_connected[0][0]}** ↔ **{most_connected[0][1]}** ({most_connected[1]} connections)"
                            )
                    ...
                    st.markdown(
                        "- 🔗 Adding more relationships between concepts in different domains"
                    )
                    st.markdown(
                        "- 📚 Expanding the knowledge base with interdisciplinary content"
                    )
                    st.markdown(
                        "- 🌐 Ensuring Neo4j database has cross-domain relationships"
                    )
