...
from .models import Neo4jStatus, DataSource, DEMO_CONCEPTS
    ...
    def check_connection(self) -> Neo4jStatus:
        ...
        try:
            ...
            if "Connection refused" in error_msg:
                return Neo4jStatus(connected=False, message='Neo4j is not running (connection refused)')
            elif "ServiceUnavailable" in error_msg:
                return Neo4jStatus(connected=False, message='Neo4j service is unavailable')
            else:
                return Neo4jStatus(connected=False, message=f'Connection error: {error_msg}')
    
    def get_concepts(self, domain: Optional[str] = None, limit: int = 200) -> List[Dict[str, Any]]:
...
class DataSourceManager:
    ...
    def get_concepts_with_fallback(self, filters: Dict[str, Any] = None, limit: int = 500) -> tuple[List[Dict[str, Any]], DataSource]:
        """Get concepts with automatic fallback to CSV or demo data"""
        # Try Neo4j first
        concepts = self.neo4j_connector.get_concepts(limit=limit)
        
        if concepts:
            return self._apply_filters(concepts, filters), DataSource('neo4j', '🗄️ Connected to Neo4j database')
        
        # Fallback to CSV
        concepts = self._load_concepts_from_csv()
        if concepts:
            return self._apply_filters(concepts, filters), DataSource('csv', '📂 Showing data from CSV files')
        
        # Final fallback to demo data
        return self._apply_filters(DEMO_CONCEPTS, filters), DataSource('demo', '🎭 Showing demonstration data')
    ...
    def _apply_filters(self, concepts: List[Dict[str, Any]], filters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        if filters.get('domains') and "All Domains" not in filters['domains']:
            domain_names = [d.split(' ', 1)[1].lower() for d in filters['domains'] if ' ' in d]
            filtered_concepts = [c for c in filtered_concepts if c['domain'] in domain_names]
        
        # Apply type filter
        if filters.get('types') and "All Types" not in filters['types']:
            filtered_concepts = [c for c in filtered_concepts if c['type'] in filters['types']]
        
        # Apply level filter
        if filters.get('level_range'):
            min_level, max_level = filters['level_range']
            filtered_concepts = [c for c in filtered_concepts if min_level <= c.get('level', 1) <= max_level]
        
        return filtered_concepts
    
    def get_cross_cultural_data(self) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]], DataSource]:
        ...
        if concepts or relationships:
            return concepts, relationships, DataSource('neo4j', '🗄️ Cross-cultural data from Neo4j')
        ...
        return mock_concepts, mock_relationships, DataSource('demo', '🎭 Cross-cultural demonstration data')