...
class ContentScraperUI:
        ...
        with st.expander("Advanced Options"):
            ...
            with col2:
                min_content_length = st.number_input(
                    "Min content length", value=100, min_value=10
                )
                timeout = st.number_input(
                    "Timeout (seconds)", value=30, min_value=5, max_value=120
                )
                retry_attempts = st.number_input(
                    "Retry attempts", value=3, min_value=1, max_value=10
                )
    ...
    def _render_batch_urls_tab(self):
        ...
        with col2:
            max_concurrent = st.number_input(
                "Max concurrent", value=3, min_value=1, max_value=10
            )
            delay_between = st.number_input(
                "Delay between requests (s)", value=1.0, min_value=0.1, max_value=10.0
            )

        if st.button("🚀 Process Batch", disabled=not urls_text):
            urls = [url.strip() for url in urls_text.split("\n") if url.strip()]
            if urls:
                self._process_batch_urls(
                    urls, batch_profile, batch_domain, max_concurrent, delay_between
                )
        ...
        if st.session_state.scraper_history:
            ...
            with col2:
                domain_filter = st.selectbox(
                    "Filter by Domain",
                    (
                        ["All"] + list(history_df["domain"].unique())
    ...
    def _render_settings_tab(self):
        ...
        if hasattr(self, "unified_scraper"):
            ...
            with col4:
                st.metric(
                    "Avg Time (s)", f"{stats.get('average_processing_time', 0):.2f}"
                )
            ...
            if method_stats:
                method_df = pd.DataFrame(
                    list(method_stats.items()), columns=["Method", "Count"]
                )
        ...
        with col1:
            ...
            new_rate = st.number_input(
                "Requests per second",
                value=self.config.requests_per_second,
                min_value=0.1,
                max_value=10.0,
            )
            new_timeout = st.number_input(
                "Request timeout", value=self.config.timeout, min_value=5, max_value=180
            )

        with col2:
            st.write("**Content Extraction**")
            use_trafilatura = st.checkbox(
                "Use Trafilatura", value=self.config.use_trafilatura
            )
            respect_robots = st.checkbox(
                "Respect robots.txt", value=self.config.respect_robots_txt
            )
            ...
            try:
                ...
                try:
                    result = loop.run_until_complete(
                        self.unified_scraper.scrape_url(url)
                    )
                finally:
                    loop.close()
                ...
                if hasattr(self.intelligent_scraper, "analyze_content"):
                    analysis = self.intelligent_scraper.analyze_content(
                        result.content, url
                    )
                else:
        ...
        try:
            ...
            try:
                results = loop.run_until_complete(
                    self.unified_scraper.scrape_multiple(
            ...
            for i, (url, result) in enumerate(zip(urls, results)):
                ...
                if result.success:
                    ...
                    self._add_to_history(
                        url, result, {}, domain, "failed", result.error
                    )
    ...
    def _display_scraping_results(self, url: str, result, analysis: Dict, domain: str):
        ...
        with col3:
            content_length = (
                len(result.content.get("text", "")) if result.content else 0
            )
        ...
        if analysis:
            ...
            with col1:
                ...
                st.write(
                    f"**Authority Score**: {analysis.get('authority_score', 0):.2f}"
                )
    ...
    def _add_to_history(
        ...
        history_entry = {
            ...
            "content_length": (
                len(result.content.get("text", "")) if result and result.content else 0
            ),
            "content_type": (
                analysis.get("content_type", "unknown") if analysis else "unknown"
            ),
