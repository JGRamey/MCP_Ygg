...
class TestHybridSystemIntegration:
    ...
    def test_qdrant_connection(self):
        ...
        assert response.status_code == 404 or response.status_code == 200  # 404 is also OK for health endpoint
    ...
    def test_concept_data_integrity(self):
        ...
        with self.driver.session() as session:
            ...
            assert total_concepts == 371, f"Expected 371 concepts, found {total_concepts}"
            ...
            for domain, expected_count in expected_counts.items():
                ...
                assert domain_counts[domain] == expected_count, \
    ...
    def test_hybrid_query_workflow(self):
        ...
        similar_concept_ids = [result["payload"]["concept_id"] for result in similar_concepts]
...
class TestSystemPerformance:
    ...
    def test_neo4j_query_performance(self):
        ...
        with self.driver.session() as session:
            ...
            assert query_time < 1.0, f"Simple count query took {query_time:.2f}s, expected < 1.0s"
            ...
            assert query_time < 2.0, f"Relationship query took {query_time:.2f}s, expected < 2.0s"
    ...
    def test_qdrant_search_performance(self):
        ...
        assert response.status_code == 200
        assert search_time < 1.0, f"Vector search took {search_time:.2f}s, expected < 1.0s"
