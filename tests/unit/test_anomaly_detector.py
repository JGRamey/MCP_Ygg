...
from agents.anomaly_detector.models import Anomaly, AnomalyType, AnomalySeverity
from agents.anomaly_detector.detectors import TemporalAnomalyDetector, StatisticalAnomalyDetector
...
class TestAnomalySummaryGenerator:
    ...
    def test_generate_summary(self):
        ...
        anomalies = [
            Anomaly(
                id="1", anomaly_type=AnomalyType.TEMPORAL, severity=AnomalySeverity.HIGH,
                description="Test", data_source="test", node_id="1", anomaly_score=0.9,
                details={}, detected_at=datetime.now(), features={}, suggestions=[]
            ),
            Anomaly(
                id="2", anomaly_type=AnomalyType.CONTENT, severity=AnomalySeverity.MEDIUM,
                description="Test", data_source="test", node_id="2", anomaly_score=0.7,
                details={}, detected_at=datetime.now(), features={}, suggestions=[]
            )
        ]
    ...
    def test_get_anomalies_by_node(self):
        ...
        anomalies = [
            Anomaly(
                id="1", anomaly_type=AnomalyType.TEMPORAL, severity=AnomalySeverity.HIGH,
                description="Test", data_source="test", node_id="node1", anomaly_score=0.9,
                details={}, detected_at=datetime.now(), features={}, suggestions=[]
            ),
            Anomaly(
                id="2", anomaly_type=AnomalyType.CONTENT, severity=AnomalySeverity.MEDIUM,
                description="Test", data_source="test", node_id="node2", anomaly_score=0.7,
                details={}, detected_at=datetime.now(), features={}, suggestions=[]
            )
        ]
        
        node_anomalies = AnomalySummaryGenerator.get_anomalies_by_node(anomalies, "node1")
    ...
    def test_resolve_anomaly(self):
        ...
        anomalies = [
            Anomaly(
                id="test_anomaly", anomaly_type=AnomalyType.TEMPORAL, severity=AnomalySeverity.HIGH,
                description="Test", data_source="test", node_id="1", anomaly_score=0.9,
                details={}, detected_at=datetime.now(), features={}, suggestions=[]
            )
        ]
        
        success = AnomalySummaryGenerator.resolve_anomaly(anomalies, "test_anomaly", "Fixed manually")
        ...
        success = AnomalySummaryGenerator.resolve_anomaly(anomalies, "non_existent", "Test")
