...
from agents.neo4j_manager.neo4j_agent import OperationResult, NodeData, RelationshipData
...
class TestCachedNeo4jAgent:
    ...
    def cached_neo4j_agent(self, mock_cache_manager):
        ...
        with patch('agents.neo4j_manager.cached_neo4j_agent.cache_integration', mock_cache_manager):
            agent = CachedNeo4jAgent()
            agent.driver = AsyncMock()
            return agent
    
    @pytest.mark.asyncio
    async def test_cached_neo4j_agent_initialization(self, cached_neo4j_agent, mock_cache_manager):
    ...
    async def test_get_node_cached(self, cached_neo4j_agent):
        ...
        with patch.object(cached_neo4j_agent.__class__.__bases__[0], 'get_node', return_value=expected_result):
    ...
    async def test_get_concepts_by_domain_cached(self, cached_neo4j_agent):
        ...
        with patch.object(cached_neo4j_agent, 'execute_cypher', return_value=expected_result):
    ...
    async def test_create_node_with_cache_invalidation(self, cached_neo4j_agent):
        ...
        with patch.object(cached_neo4j_agent.__class__.__bases__[0], 'create_node', return_value=expected_result):
            with patch.object(cached_neo4j_agent.cache_manager, 'invalidate_domain_cache') as mock_invalidate:
                with patch.object(cached_neo4j_agent.cache_manager, 'clear_analytics_cache') as mock_clear:
                    result = await cached_neo4j_agent.create_node_with_cache_invalidation(node_data)
    ...
    async def test_warm_cache_for_domain(self, cached_neo4j_agent):
        ...
        with patch.object(cached_neo4j_agent, 'get_concepts_by_domain', return_value=OperationResult(success=True)):
            with patch.object(cached_neo4j_agent, 'get_domain_statistics', return_value=OperationResult(success=True)):
                await cached_neo4j_agent.warm_cache_for_domain(domain)
                
                # Verify the methods were called
                cached_neo4j_agent.get_concepts_by_domain.assert_called_once_with(domain, 50)
    ...
    async def test_cache_invalidation_on_write_operations(self, cached_neo4j_agent):
        ...
        # Mock parent class method and cache invalidation
        with patch.object(cached_neo4j_agent.__class__.__bases__[0], 'execute_cypher', return_value=expected_result):
            with patch.object(cached_neo4j_agent, '_invalidate_cache_by_query_pattern') as mock_invalidate:
...
class TestCachedQdrantAgent:
    ...
    def cached_qdrant_agent(self, mock_cache_manager):
        ...
        with patch('agents.qdrant_manager.cached_qdrant_agent.cache_integration', mock_cache_manager):
            agent = CachedQdrantAgent()
            agent.client = AsyncMock()
            return agent
    
    @pytest.mark.asyncio
    async def test_cached_qdrant_agent_initialization(self, cached_qdrant_agent, mock_cache_manager):
    ...
    async def test_search_vectors_cached(self, cached_qdrant_agent):
        ...
        # Mock the parent class method
        with patch.object(cached_qdrant_agent.__class__.__bases__[0], 'search_vectors', return_value=expected_result):
    ...
    async def test_get_points_by_domain_cached(self, cached_qdrant_agent):
        ...
        with patch.object(cached_qdrant_agent, 'search_by_payload_cached', return_value=expected_result):
    ...
    async def test_get_domain_vector_statistics_cached(self, cached_qdrant_agent):
        ...
        with patch.object(cached_qdrant_agent, 'get_points_by_domain_cached', return_value=mock_points_result):
            result = await cached_qdrant_agent.get_domain_vector_statistics_cached(domain)
    ...
    async def test_find_similar_concepts_cached(self, cached_qdrant_agent):
        ...
        mock_concept_result = VectorOperationResult(
            success=True,
            data={"vector": [0.1, 0.2, 0.3, 0.4]}
        )
        
        # Mock search_vectors_cached to return similar concepts
        mock_similar_result = VectorOperationResult(
            success=True,
            data={"results": [{"id": "similar_concept_1", "score": 0.9}]}
        )
        
        with patch.object(cached_qdrant_agent, 'get_point_cached', return_value=mock_concept_result):
            with patch.object(cached_qdrant_agent, 'search_vectors_cached', return_value=mock_similar_result):
    ...
    async def test_upsert_vector_with_cache_invalidation(self, cached_qdrant_agent):
        ...
        # Mock parent class method and cache invalidation
        with patch.object(cached_qdrant_agent.__class__.__bases__[0], 'upsert_vector', return_value=expected_result):
            with patch.object(cached_qdrant_agent.cache_manager, 'invalidate_domain_cache') as mock_invalidate:
                with patch.object(cached_qdrant_agent.cache_manager.cache, 'delete_pattern') as mock_delete:
                    result = await cached_qdrant_agent.upsert_vector_with_cache_invalidation(
    ...
    @pytest.mark.asyncio
    async def test_batch_upsert_vectors_with_cache_invalidation(self, cached_qdrant_agent):
        """Test batch vector upsert with cache invalidation."""
        collection_name = "test_collection"
        vector_points = [
            VectorPoint(id="point1", vector=[0.1, 0.2], payload={"domain": "philosophy"}),
        ...
        # Mock parent class method and cache invalidation
        with patch.object(cached_qdrant_agent.__class__.__bases__[0], 'batch_upsert_vectors', return_value=expected_result):
            with patch.object(cached_qdrant_agent.cache_manager, 'invalidate_domain_cache') as mock_invalidate:
                with patch.object(cached_qdrant_agent.cache_manager.cache, 'delete_pattern') as mock_delete:
    ...
    async def test_warm_cache_for_collection(self, cached_qdrant_agent):
        ...
        # Mock the cached methods
        with patch.object(cached_qdrant_agent, 'get_collection_info_cached', return_value=VectorOperationResult(success=True)):
            with patch.object(cached_qdrant_agent, 'get_collection_statistics_cached', return_value=VectorOperationResult(success=True)):
                with patch.object(cached_qdrant_agent, 'get_domain_vector_statistics_cached', return_value=VectorOperationResult(success=True)):
                    await cached_qdrant_agent.warm_cache_for_collection(collection_name)
                    
                    # Verify the methods were called
                    cached_qdrant_agent.get_collection_info_cached.assert_called_once_with(collection_name)
                    cached_qdrant_agent.get_collection_statistics_cached.assert_called_once_with(collection_name)
                    # Should warm cache for common domains
                    assert cached_qdrant_agent.get_domain_vector_statistics_cached.call_count == 6
    ...
    async def test_hybrid_search_cached(self, cached_qdrant_agent):
        ...
        with patch.object(cached_qdrant_agent.__class__.__bases__[0], 'hybrid_search', return_value=expected_result):
            with patch.object(cached_qdrant_agent.cache_manager.cache, 'get', return_value=None):
                with patch.object(cached_qdrant_agent.cache_manager.cache, 'set') as mock_set:
...
class TestCacheIntegration:
    ...
    @pytest.mark.asyncio
    async def test_cache_integration_initialization(self, mock_cache_integration_manager):
        """Test cache integration manager initialization."""
        with patch('cache.integration_manager.CacheIntegrationManager', return_value=mock_cache_integration_manager):
    ...
    async def test_domain_cache_invalidation(self, mock_cache_integration_manager):
        ...
        with patch('cache.integration_manager.CacheIntegrationManager', return_value=mock_cache_integration_manager):
            ...
            mock_cache_integration_manager.invalidate_domain_cache.assert_called_once_with(domain)
    ...
    async def test_concept_cache_invalidation(self, mock_cache_integration_manager):
        ...
        with patch('cache.integration_manager.CacheIntegrationManager', return_value=mock_cache_integration_manager):
            ...
            mock_cache_integration_manager.invalidate_concept_cache.assert_called_once_with(concept_id)
...
class TestCachePerformance:
    ...
    async def test_cache_memory_usage_monitoring(self):
        ...
        memory_usage_percent = (memory_stats['used_memory'] / memory_stats['max_memory']) * 100
    ...
    async def test_cache_performance_recommendations(self):
        ...
        recommendations = manager._generate_performance_recommendations(low_hit_rate_stats)
