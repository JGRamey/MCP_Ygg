...
def performance_test_data():
    ...
    for i in range(1000):
        node = {
            'id': f'node_{i}',
            'name': f'Entity_{i}',
            'domain': np.random.choice(['philosophy', 'science', 'mathematics', 'art', 'language', 'technology']),
    ...
    for i in range(5000):
        ...
        if source != target:  # Avoid self-loops
            edge = {
                ...
                'relationship_type': np.random.choice(['influences', 'studies', 'creates', 'belongs_to']),
...
def generate_test_time_series(days: int = 365, domains: List[str] = None) -> pd.DataFrame:
