...
class TestCSVImportFunctionality:
    ...
    def test_domain_specific_labels(self):
        ...
        with self.driver.session() as session:
            ...
            domains = ["Art", "Science", "Mathematics", "Philosophy", "Language", "Technology", "Religion", "Astrology"]
            ...
            for domain in domains:
                ...
                if count > 0:  # Only test domains that have data
                    ...
                    assert invalid_count == 0, f"Found {invalid_count} {domain} nodes without Concept label"
    ...
    def test_hierarchical_relationships(self):
        ...
        with self.driver.session() as session:
            ...
            assert hierarchical_count > 0, "No hierarchical BELONGS_TO relationships found"
            ...
            assert invalid_root_count == 0, f"Found {invalid_root_count} root concepts not at level 1"
    ...
    def test_data_integrity_constraints(self):
        ...
        with self.driver.session() as session:
            ...
            assert invalid_rels == 0, f"Found {invalid_rels} relationships not connecting Concept nodes"
    ...
    def test_domain_completeness(self):
        ...
        with self.driver.session() as session:
            ...
            expected_domains = ["Art", "Astrology", "Language", "Mathematics", "Philosophy", "Religion", "Science", "Technology"]
            
            for domain in expected_domains:
                assert domain in imported_domains, f"Domain {domain} not found in imported data"
    ...
    def test_metadata_fields(self):
        ...
        with self.driver.session() as session:
            ...
            for date in dates:
                assert isinstance(date, int), f"Date should be integer, got {type(date)}"
...
class TestCSVDataQuality:
    ...
    def test_no_empty_required_fields(self):
        ...
        with self.driver.session() as session:
            ...
            assert empty_domains == 0, f"Found {empty_domains} concepts with empty domains"
    ...
    def test_consistent_naming_conventions(self):
        ...
        with self.driver.session() as session:
            ...
            assert len(double_underscores) == 0, f"Found concepts with double underscores: {[r['name'] for r in double_underscores]}"
            ...
            assert len(long_names) == 0, f"Found concepts with excessively long names: {long_names}"
    ...
    def test_relationship_consistency(self):
        ...
        with self.driver.session() as session:
            ...
            assert len(invalid_hierarchy) == 0, f"Found invalid hierarchy relationships: {invalid_hierarchy}"
...
class TestCSVImportPerformance:
    ...
    def test_query_performance_on_imported_data(self):
        ...
        with self.driver.session() as session:
            ...
            assert lookup_time < 0.1, f"Concept lookup took {lookup_time:.3f}s, expected < 0.1s"
            ...
            assert domain_query_time < 0.5, f"Domain query took {domain_query_time:.3f}s, expected < 0.5s"
            ...
            assert traversal_time < 1.0, f"Relationship traversal took {traversal_time:.3f}s, expected < 1.0s"
    ...
    def test_index_effectiveness(self):
        ...
        with self.driver.session() as session:
            ...
            concept_id_indexes = [idx for idx in indexes if 'concept_id' in str(idx).lower()]
            ...
            plan_str = str(plan)
            assert 'Index' in plan_str or 'NodeByLabelScan' not in plan_str, "Query not using index efficiently"
