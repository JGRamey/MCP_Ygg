...
from utils.database_operations import get_all_concepts, get_domains, get_quick_stats
...
def main():
    ...
    st.set_page_config(
        page_title="Analytics Dashboard - MCP Yggdrasil",
        page_icon="📈",
        layout="wide"
    )
    ...
    with st.sidebar:
        ...
        analytics_view = st.selectbox(
            "View Type",
            ["🌟 Executive Summary", "📊 Detailed Metrics", "🔍 Deep Dive Analysis", "🤖 AI Insights", "📈 Custom Reports"]
        )
...
def show_kpi_dashboard():
    ...
    try:
        ...
        with col4:
            quality_score = 94.2  # Mock quality score
            st.markdown(f"""
            <div class="metric-card">
...
def show_activity_trends():
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=('Concepts Created', 'Relationships Added', 'Query Activity', 'Combined Activity'),
    ...
    # Concepts created
    fig.add_trace(
        go.Scatter(x=df_activity['Date'], y=df_activity['Concepts Created'], 
                  mode='lines+markers', name='Concepts', line=dict(color='#2E8B57')),
        row=1, col=1
    )
    
    # Relationships added
    fig.add_trace(
        go.Scatter(x=df_activity['Date'], y=df_activity['Relationships Added'], 
                  mode='lines+markers', name='Relationships', line=dict(color='#4ECDC4')),
        row=1, col=2
    )
    
    # Query activity
    fig.add_trace(
        go.Scatter(x=df_activity['Date'], y=df_activity['Queries Executed'], 
                  mode='lines+markers', name='Queries', line=dict(color='#FFD93D')),
        row=2, col=1
    )
    
    # Combined activity (stacked area)
    fig.add_trace(
        go.Scatter(x=df_activity['Date'], y=df_activity['Concepts Created'] + df_activity['Relationships Added'], 
                  mode='lines', fill='tonexty', name='Total Activity', line=dict(color='#667eea')),
        row=2, col=2
    )
    
    fig.update_layout(height=600, showlegend=False, title_text="30-Day Activity Overview")
    st.plotly_chart(fig, use_container_width=True)
...
def show_detailed_metrics():
    ...
    metric_category = st.selectbox(
        "Metric Category",
        ["🎯 Performance Metrics", "📊 Usage Statistics", "🔍 Quality Metrics", "🌐 Network Analysis"]
    )
...
def show_performance_metrics():
    ...
    fig.add_trace(
        go.Scatter(x=df_performance['Time'], y=df_performance['Avg Response Time (ms)'],
                  mode='lines', name='Response Time', line=dict(color='#FF6B6B')),
        secondary_y=False,
    )
    
    fig.add_trace(
        go.Bar(x=df_performance['Time'], y=df_performance['Query Count'],
               name='Query Count', opacity=0.7, marker_color='#4ECDC4'),
        secondary_y=True,
    )
    ...
    perf_metrics = [
        {"Metric": "Average Query Time", "Value": "147ms", "Target": "<200ms", "Status": "✅"},
        {"Metric": "95th Percentile", "Value": "298ms", "Target": "<500ms", "Status": "✅"},
        {"Metric": "Error Rate", "Value": "0.12%", "Target": "<1%", "Status": "✅"},
        {"Metric": "Throughput", "Value": "2,340 queries/hour", "Target": ">1,000", "Status": "✅"},
...
def show_usage_statistics():
    ...
    with col1:
        ...
        fig = px.bar(df_top, x='Queries', y='Concept', orientation='h',
    ...
    hours = list(range(24))
    usage_pattern = [10, 8, 5, 3, 2, 3, 8, 15, 25, 35, 45, 50, 55, 60, 58, 52, 48, 42, 38, 32, 28, 22, 18, 14]
    
    fig = px.bar(x=hours, y=usage_pattern, title="Usage by Hour of Day",
...
def show_quality_metrics():
    ...
    with col2:
        ...
        fig.add_trace(go.Scatter(x=dates, y=completeness_trend,
                               mode='lines', name='Completeness', line=dict(color='#2E8B57')))
        fig.add_trace(go.Scatter(x=dates, y=accuracy_trend,
                               mode='lines', name='Accuracy', line=dict(color='#4ECDC4')))
        
        fig.update_layout(title="Quality Metrics Trend (30 Days)", yaxis_title="Score (%)")
        st.plotly_chart(fig, use_container_width=True)
    ...
    quality_issues = [
        {"Issue Type": "Missing Descriptions", "Count": 23, "Priority": "Medium", "Trend": "↓"},
        {"Issue Type": "Duplicate Names", "Count": 5, "Priority": "High", "Trend": "→"},
        {"Issue Type": "Invalid Relationships", "Count": 12, "Priority": "Medium", "Trend": "↓"},
        {"Issue Type": "Orphaned Concepts", "Count": 8, "Priority": "Low", "Trend": "→"},
        {"Issue Type": "Inconsistent Formatting", "Count": 15, "Priority": "Low", "Trend": "↓"}
    ]
...
def show_network_analysis_detailed():
    ...
    fig = px.histogram(degrees, nbins=20, title="Node Degree Distribution",
    ...
    central_concepts = [
        {"Concept": "SCI0001: Quantum_Mechanics", "Degree": 45, "Betweenness": 0.23, "Closeness": 0.78},
        {"Concept": "PHIL0001: Philosophy", "Degree": 38, "Betweenness": 0.19, "Closeness": 0.72},
        {"Concept": "MATH0005: Mathematics", "Degree": 34, "Betweenness": 0.15, "Closeness": 0.68},
        {"Concept": "ART0001: Art", "Degree": 29, "Betweenness": 0.12, "Closeness": 0.65},
        {"Concept": "SCI0015: Physics", "Degree": 27, "Betweenness": 0.11, "Closeness": 0.63}
    ]
...
def show_deep_dive_analysis():
    ...
    analysis_type = st.selectbox(
        "Analysis Type",
        ["🎯 Domain Deep Dive", "🔗 Relationship Analysis", "📈 Growth Analysis", "🔍 Anomaly Detection"]
    )
...
def show_domain_deep_dive():
    ...
    if domains:
        ...
        if selected_domain:
            ...
            with col1:
                st.metric("Total Concepts", domain_data['concept_count'])
            
            with col2:
                # Mock calculation
                avg_concepts = sum(d['concept_count'] for d in domains) / len(domains)
                percentage = (domain_data['concept_count'] / avg_concepts - 1) * 100
            ...
            type_distribution = {
                ...
                'leaf': domain_data['concept_count'] - np.random.randint(8, 20)
            }
            
            fig = px.pie(values=list(type_distribution.values()), names=list(type_distribution.keys()),
...
def show_relationship_analysis_detailed():
    ...
    with col1:
        ...
        fig = px.bar(x=list(rel_types.keys()), y=list(rel_types.values()),
    ...
    with col2:
        ...
        fig = px.histogram(strengths, nbins=20, title="Relationship Strength Distribution",
...
def show_growth_analysis():
    ...
    historical_concepts = np.cumsum(np.random.poisson(lam=25, size=len(historical_dates)))
    ...
    # Historical data
    fig.add_trace(go.Scatter(x=historical_dates, y=historical_concepts,
                           mode='lines+markers', name='Historical', line=dict(color='#2E8B57')))
    
    # Projected data
    fig.add_trace(go.Scatter(x=projected_dates, y=projected_concepts[1:],
                           mode='lines+markers', name='Projected', line=dict(color='#FF6B6B', dash='dash')))
    
    fig.update_layout(title="Knowledge Graph Growth: Historical and Projected",
                     xaxis_title="Date", yaxis_title="Total Concepts")
    st.plotly_chart(fig, use_container_width=True)
...
def show_anomaly_detection():
    ...
    anomaly_types = st.multiselect(
        ...
        ["🔍 Usage Anomalies", "📊 Data Anomalies", "🔗 Network Anomalies", "⚡ Performance Anomalies"],
    ...
    if "🔍 Usage Anomalies" in anomaly_types:
        ...
        usage_anomalies = [
            {"Time": "2025-07-01 14:23", "Type": "Query Spike", "Description": "300% increase in Science domain queries", "Severity": "Medium"},
            {"Time": "2025-07-01 10:15", "Type": "Unusual Pattern", "Description": "High number of failed authentication attempts", "Severity": "High"},
            {"Time": "2025-06-30 22:45", "Type": "Off-hours Activity", "Description": "Significant activity detected during maintenance window", "Severity": "Low"}
        ]
    ...
    if "📊 Data Anomalies" in anomaly_types:
        ...
        data_anomalies = [
            {"Concept": "SCI0067", "Issue": "Description length 5x longer than domain average", "Confidence": "High"},
            {"Concept": "PHIL0023", "Issue": "Unusual relationship pattern (connected to 40+ concepts)", "Confidence": "Medium"},
            {"Concept": "MATH0045", "Issue": "Level inconsistency with related concepts", "Confidence": "Medium"}
        ]
...
def show_ai_insights():
    ...
    insight_category = st.selectbox(
        "Insight Category", 
        ["🎯 Optimization Recommendations", "🔮 Predictive Insights", "🎨 Pattern Discovery", "🚀 Growth Opportunities"]
    )
...
def show_custom_reports():
    ...
    st.markdown('<div class="section-header">📈 Custom Reports</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        report_type = st.selectbox("Report Type", ["Summary Report", "Detailed Analysis", "Trend Report", "Comparison Report"])
        
    with col2:
        date_range = st.date_input("Date Range", value=[datetime.now() - timedelta(days=30), datetime.now()])
        output_format = st.selectbox("Output Format", ["Interactive Dashboard", "PDF Report", "Excel Export", "JSON Data"])
    ...
    selected_sections = st.multiselect("Select Sections", available_sections, default=available_sections[:4])
    ...
    if st.button("📊 Generate Report", type="primary"):
        if report_name and selected_sections:
            with st.spinner("Generating custom report..."):
                ...
                for section in selected_sections[:3]:  # Show first 3 sections as preview
                    st.markdown(f"**{section}**")
                    if "Executive Summary" in section:
                        st.metric("Key Metric", "94.2%", delta="5.2%")
                    elif "Growth Metrics" in section:
                        st.line_chart(pd.DataFrame(np.random.randn(30, 1), columns=['Growth']))
                    elif "Quality Analysis" in section:
                        st.bar_chart(pd.DataFrame({'Quality Score': [92, 94, 88, 96]}, index=['A', 'B', 'C', 'D']))
