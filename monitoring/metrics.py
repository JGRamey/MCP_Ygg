...
class PrometheusMetrics:
    ...
    def __init__(self):
        ...
        if self.enabled:
            ...
            for attr_name in dir(self):
                if not attr_name.startswith('_') and attr_name not in ['enabled', 'update_system_metrics', 'get_metrics_response']:
    ...
    def record_database_query(self, db_type: str, query_type: str, duration: float, status: str):
        """Record database query metrics"""
        if self.enabled:
            if db_type == 'neo4j':
                self.neo4j_queries_total.labels(query_type=query_type, status=status).inc()
                self.neo4j_query_duration.labels(query_type=query_type).observe(duration)
            elif db_type == 'qdrant':
                collection = query_type.split('_')[0] if '_' in query_type else 'unknown'
                operation = query_type.split('_', 1)[1] if '_' in query_type else query_type
                self.qdrant_operations_total.labels(operation=operation, collection=collection, status=status).inc()
    ...
    def record_document_processing(self, source_type: str, status: str):
        ...
        if self.enabled:
            self.documents_processed_total.labels(source_type=source_type, status=status).inc()
    ...
    def record_ai_agent_operation(self, agent_type: str, operation: str, duration: float, status: str):
        """Record AI agent operation metrics"""
        if self.enabled:
            self.ai_agent_operations_total.labels(agent_type=agent_type, operation=operation, status=status).inc()
            self.ai_agent_processing_duration.labels(agent_type=agent_type, operation=operation).observe(duration)
