...
try:
    ...
    print(
        "⚠️ prometheus_client not available. Install with: pip install prometheus-client"
    )
...
class EnhancedMonitoring:
    ...
    def __init__(self, port: int = 8001):
        ...
        if self.metrics_enabled:
            ...
            self.scraping_duration = Histogram(
                "scraping_duration_seconds", "Web scraping duration", ["url", "status"]
            )
            ...
            self.active_connections = Gauge(
                "active_connections", "Number of active connections", ["type"]
            )
    ...
    async def start_metrics_server(self):
        ...
        if not self.metrics_enabled:
            logger.warning(
                "⚠️ Cannot start metrics server - Prometheus client not available"
            )
            return False
...
def setup_grafana_docker() -> bool:
    ...
    try:
        ...
        result = subprocess.run(
            ["docker", "ps", "-a", "--filter", "name=mcp-grafana"],
            capture_output=True,
            text=True,
        )
