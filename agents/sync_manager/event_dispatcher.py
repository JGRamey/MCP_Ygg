...
class EventDispatcher:
    ...
    async def _handle_failed_event(self, event: SyncEvent, event_data: str):
        ...
        if event.retry_count <= event.max_retries:
            ...
            self.failed_events += 1
            logger.error(f"Event {event.event_id} moved to dead letter queue after {event.retry_count} attempts")
    ...
    async def _monitor_queues(self):
        ...
        while self.is_running:
            try:
                ...
                if total_pending > 0:
                    logger.info(f"Queue status - Pending: {total_pending}, "
                              f"Processed: {self.processed_events}, "
                              f"Failed: {self.failed_events}")
                
                # Check for queue backup
                if total_pending > 1000:
                    logger.warning(f"High queue backlog detected: {total_pending} events")
    ...
    async def create_node_event(self, 
                               source_db: str,
                               node_id: str,
                               node_type: str,
    ...
    async def create_vector_event(self,
                                 source_db: str,
                                 vector_id: str,
                                 collection: str,
    ...
    async def reprocess_dead_letter_queue(self) -> int:
        ...
        try:
            ...
            while True:
                ...
                event = SyncEvent(**{k: v for k, v in event_dict.items() 
