...
from concept_explorer import ConceptExplorer, ConceptNode, RelationshipEdge, ConceptHypothesis
...
class ConceptDiscoveryService:
    ...
    async def discover_concepts_from_content(self, 
                                           content: str,
                                           source_document: str,
                                           domain: Optional[str] = None,
                                           include_hypotheses: bool = True,
                                           include_thought_paths: bool = True) -> DiscoveryResult:
        """
        ...
        try:
            ...
            relationships = await self.concept_explorer.discover_relationships(concepts, content)
            ...
            if include_hypotheses:
                hypotheses = await self.concept_explorer.generate_hypotheses(concepts, relationships)
            ...
            temporal_evolution = await self.concept_explorer.analyze_temporal_evolution(concepts)
            ...
            logger.info(f"Concept discovery completed for {source_document}: "
    ...
    async def discover_cross_document_patterns(self, 
        ...
        """
        try:
            logger.info(f"Starting cross-document pattern analysis for {len(documents)} documents")
            ...
            for concept in all_concepts:
                ...
                concept_frequency[concept_name]["documents"].append(concept.source_documents[0])
            ...
            for i, concept1 in enumerate(all_concepts):
                for j, concept2 in enumerate(all_concepts[i+1:], i+1):
                    if (concept1.source_documents[0] != concept2.source_documents[0] and
                        concept1.domain != concept2.domain):
                        
                        # Check for potential cross-document bridge
                        bridge_strength = await self._calculate_cross_document_bridge_strength(
                            concept1, concept2
                        )
    ...
    async def generate_knowledge_graph_data(self, 
                                          discovery_results: List[DiscoveryResult]) -> Dict[str, Any]:
        """
        Generate data formatted for knowledge graph integration
        """
        try:
            logger.info(f"Generating knowledge graph data from {len(discovery_results)} discovery results")
            ...
            for result in discovery_results:
                ...
                for hypothesis in result.hypotheses:
                    hypothesis_node = {
                        ...
                        "type": "hypothesis",
                        "domain": "cross_domain" if hypothesis.domain_bridge else "unknown",
            ...
            graph_stats = {
                ...
                "hypothesis_nodes": len([n for n in nodes if n["type"] == "hypothesis"]),
    ...
    async def _add_concepts_to_global_graph(self, 
                                          concepts: List[ConceptNode],
                                          relationships: List[RelationshipEdge]):
        ...
        try:
            ...
            for relationship in relationships:
                if (relationship.source_id in self.global_concept_graph and 
                    relationship.target_id in self.global_concept_graph):
    ...
    async def _generate_thought_paths(self, concepts: List[ConceptNode]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(len(domains)):
                for j in range(i + 1, len(domains)):
                    ...
                    for concept1 in domain1_concepts[:3]:  # Limit to avoid too many paths
    ...
    async def _calculate_cross_document_bridge_strength(self, 
    ...
    def _calculate_confidence_score(self, 
        ...
        try:
            ...
            if hypotheses:
                hypothesis_quality = sum(h.evidence_strength for h in hypotheses) / len(hypotheses)
            
            # Weighted average
            weights = [0.4, 0.3, 0.2, 0.1]
            scores = [concept_confidence, relationship_confidence, network_quality, hypothesis_quality]
    ...
    def _create_empty_result(self, discovery_id: str, source_document: str, start_time: datetime) -> DiscoveryResult:
