...
class ThoughtPathTracer:
    ...
    async def trace_path(
        ...
        try:
            ...
            if start_concept not in concept_graph or end_concept not in concept_graph:
                logger.warning(
                    f"Concepts {start_concept} or {end_concept} not found in graph"
                )
                return paths
            ...
            try:
                ...
                scored_paths.sort(
                    key=lambda p: (p.total_confidence * p.novelty_score), reverse=True
                )
                ...
                logger.info(
                    f"No direct path found between {start_concept} and {end_concept}"
                )
    ...
    async def _create_thought_path(
        self, path_id: str, path_nodes: List[str], concept_graph: nx.Graph
    ...
    def _generate_reasoning_explanation(
        ...
        base_explanation = explanations.get(
            relationship_type, f"'{from_concept}' relates to '{to_concept}'"
        )
        ...
        if (
            ...
            and to_domain != "unknown"
        ):
            base_explanation += f" (bridging {from_domain} and {to_domain} domains)"

        return base_explanation

    def _calculate_path_novelty(
        self, path_nodes: List[str], domains_traversed: List[str], path_length: int
    ) -> float:
    ...
    async def _find_hub_mediated_paths(
        self, start_concept: str, end_concept: str, concept_graph: nx.Graph
    ) -> List[ThoughtPath]:
        ...
        try:
            ...
            for hub_node, hub_score in hubs[:5]:  # Try top 5 hubs
                try:
                    ...
                    start_to_hub = list(
                        nx.shortest_path(concept_graph, start_concept, hub_node)
                    )

                    # Path from hub to end
                    hub_to_end = list(
                        nx.shortest_path(concept_graph, hub_node, end_concept)
                    )
    ...
    async def discover_reasoning_patterns(
        ...
        try:
            ...
            sample_pairs = list(
                combinations(nodes[:50], 2)
            )  # Sample to avoid explosion

            for start, end in sample_pairs:
                try:
                    paths = list(
                        nx.all_simple_paths(concept_graph, start, end, cutoff=4)
                    )
                    ...
                    for path in paths[:3]:  # Sample paths
                        if len(path) >= 3:  # Need at least 2 relationships
                            ...
                            for i in range(len(path) - 1):
                                edge_data = concept_graph.get_edge_data(
                                    path[i], path[i + 1], {}
                                )
                            ...
                            relationship_sequences[sequence_key]["examples"].append(
                                path
                            )
