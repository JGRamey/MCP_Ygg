...
try:
    from agents.text_processor.text_processor import TextProcessor, Entity, ProcessedDocument
...
class DomainTaxonomyMapper:
    ...
    def load_taxonomy(self):
        ...
        for domain, csv_path in domains.items():
            ...
            if full_path.exists():
                try:
                    self.domain_concepts[domain] = self._load_concepts_from_csv(full_path)
                    logger.info(f"Loaded {len(self.domain_concepts[domain])} concepts for {domain}")
                except Exception as e:
    ...
    def map_to_domain_taxonomy(self, text: str, concepts: List[str]) -> DomainMapping:
        ...
        taxonomy_matches = [
            concept_id for concept_id, score in concept_matches.items() 
    ...
    def _fallback_domain_mapping(self, text: str, concepts: List[str]) -> DomainMapping:
        ...
        domain_keywords = {
            'art': ['art', 'painting', 'sculpture', 'artist', 'aesthetic', 'creative', 'visual', 'design'],
            'language': ['language', 'linguistics', 'grammar', 'syntax', 'phonetics', 'morphology', 'literature'],
            'mathematics': ['mathematics', 'algebra', 'geometry', 'calculus', 'theorem', 'proof', 'equation'],
            'philosophy': ['philosophy', 'ethics', 'metaphysics', 'epistemology', 'logic', 'reasoning', 'wisdom'],
            'science': ['science', 'physics', 'chemistry', 'biology', 'experiment', 'hypothesis', 'theory'],
            'technology': ['technology', 'engineering', 'computer', 'software', 'algorithm', 'digital', 'innovation'],
            'religion': ['religion', 'spiritual', 'sacred', 'divine', 'worship', 'faith', 'belief'],
            'astrology': ['astrology', 'zodiac', 'horoscope', 'celestial', 'constellation', 'planets']
        }
...
class AdvancedEntityExtractor:
    ...
    def load_domain_entities(self):
        ...
        entity_files = {
            'people': ['art_people.csv', 'language_people.csv', 'mathematics_people.csv', 
                      'philosophy_people.csv', 'science_people.csv', 'technology_people.csv'],
            'works': ['art_works.csv', 'language_works.csv', 'mathematics_works.csv',
                     'philosophy_works.csv', 'science_works.csv', 'technology_works.csv'],
    ...
    def extract_entities_and_concepts(self, text: str, processed_doc: ProcessedDocument) -> EntityExtraction:
...
class AdvancedClaimExtractor:
    ...
    def _assess_claim_confidence(self, claim: str) -> float:
        ...
        high_confidence_words = ['proven', 'demonstrated', 'established', 'confirmed', 'verified']
        # Low confidence indicators
        low_confidence_words = ['might', 'could', 'possibly', 'perhaps', 'allegedly', 'supposedly']
    ...
    def _identify_evidence_type(self, claim: str) -> str:
        ...
        elif any(word in claim_lower for word in ['philosophical', 'argument', 'reasoning']):
...
class ContentAnalysisAgent:
    ...
    async def _perform_semantic_analysis(self, processed_doc: ProcessedDocument) -> SemanticAnalysis:
    ...
    async def _assess_content_quality(self, processed_doc: ProcessedDocument, 
                                    claim_extraction: ClaimExtraction, 
                                    scraped_doc: Dict[str, Any]) -> QualityIndicators:
    ...
    def _assess_academic_rigor(self, processed_doc: ProcessedDocument, 
                              claim_extraction: ClaimExtraction) -> float:
        """Assess academic rigor of content"""
        score = 0.5  # Base score
        
        # High-quality evidence types boost score
        empirical_claims = [c for c in claim_extraction.primary_claims 
    ...
    async def _assess_factual_consistency(self, claim_extraction: ClaimExtraction) -> float:
        ...
        try:
            ...
            avg_confidence = np.mean([c.get('confidence', 0.5) 
...
async def main():
    ...
    try:
        ...
        print(f"Entities Found: {len(analysis.entity_extraction.people)} people, {len(analysis.entity_extraction.concepts)} concepts")
        print(f"Claims Extracted: {len(analysis.claim_extraction.primary_claims)} primary claims")
