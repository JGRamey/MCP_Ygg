...
from .base import BaseAnalyzer, AnalysisConfig, AnalysisResult, CommunityAlgorithm
...
class CommunityAnalyzer(BaseAnalyzer):
    ...
    def _greedy_modularity_communities(self) -> List[Set[str]]:
        ...
        try:
            return list(nx.community.greedy_modularity_communities(
                self.graph, resolution=self.config.community_resolution
            ))
        except Exception:
            return []
    ...
    def _get_key_members(self, community: Set[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Get top members
            top_members = sorted(composite_scores.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    def _calculate_cohesion_score(self, density: float, size: int, 
                                 domain_diversity: int, avg_clustering: float) -> float:
    ...
    def _calculate_coverage(self, communities: List[Set[str]]) -> float:
        ...
        return len(all_nodes) / self.graph.number_of_nodes() if self.graph.number_of_nodes() > 0 else 0.0
    ...
    def _create_community_summary(self, communities: List[Set[str]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'communities_with_min_size': sum(1 for s in sizes if s >= self.config.community_min_size)
        }
    ...
    async def analyze_community_evolution(self, session, time_windows: List[str]) -> Dict[str, Any]:
        ...
        for window in time_windows:
            ...
            if temporal_graph.number_of_nodes() > 0:
                communities = self._greedy_modularity_communities_for_graph(temporal_graph)
                evolution_data[window] = {
                    'node_count': temporal_graph.number_of_nodes(),
                    'edge_count': temporal_graph.number_of_edges(),
                    'community_count': len(communities),
                    'modularity': self._calculate_modularity_for_graph(temporal_graph, communities),
                    'largest_community_size': max(len(c) for c in communities) if communities else 0
                }
        
        return evolution_data
    
    def _greedy_modularity_communities_for_graph(self, graph: nx.Graph) -> List[Set[str]]:
    ...
    def _calculate_modularity_for_graph(self, graph: nx.Graph, communities: List[Set[str]]) -> float:
