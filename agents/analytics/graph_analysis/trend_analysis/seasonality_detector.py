...
from ..models import TrendPoint, SeasonalityPattern, SeasonalDecomposition
...
class SeasonalityDetector:
    ...
    def __init__(self, config: Optional[TrendConfig] = None):
        ...
        self.default_periods = [7, 14, 30, 90, 365]  # Weekly, bi-weekly, monthly, quarterly, yearly
    ...
    async def detect_seasonality_patterns(
        ...
        try:
            if len(data) < self.min_data_points:
                self.logger.warning(f"Insufficient data for seasonality detection: {len(data)} points")
                return {
            ...
            periods = custom_periods or getattr(self.config, 'seasonality_periods', self.default_periods)
    ...
    async def _autocorrelation_detection(
        ...
        try:
            ...
            max_correlation = max([abs(score) for score in seasonality_scores.values()], default=0)
    ...
    async def _frequency_domain_detection(self, data: List[TrendPoint]) -> Dict[str, Any]:
        ...
        try:
                ...
                for freq, power in zip(peak_frequencies, peak_powers):
                    if freq > 0:
                        ...
                        if 2 <= period_days <= len(values) // 2:
                            strength = power / np.max(positive_power)  # Normalize by max power
            ...
            confidence = significant_patterns[0]['strength'] if significant_patterns else 0.0
    ...
    async def _decomposition_detection(
        ...
        try:
            ...
            for period in periods:
                if len(ts_series) >= 2 * period:
                    try:
                        ...
                        if seasonal_strength > 0.1:  # Threshold for meaningful seasonality
                            decomposition_results.append({
                                'period': period,
                                'strength': float(seasonal_strength),
                                'method': 'decomposition',
                                'trend_strength': float(trend.var() / ts_series.var()),
                                'residual_strength': float(residual_var / ts_series.var())
                            })
                    
                    except Exception as period_error:
                        self.logger.debug(f"Decomposition failed for period {period}: {period_error}")
                        continue
            ...
            confidence = decomposition_results[0]['strength'] if decomposition_results else 0.0
                        ...
                        if within_group_var > 0:
                            ...
                            if seasonal_strength > 0.2:
                                pattern_results.append({
                                    ...
                                    'group_means': {k: float(np.mean(v)) for k, v in grouped_data.items()}
                                })
    ...
    def _combine_detection_results(
        ...
        try:
            ...
            for result in [autocorr_results, fft_results, decomp_results, calendar_results]:
            ...
            # Generate insights
            insights = self._generate_seasonality_insights(all_patterns, combined_confidence)
    ...
    def _generate_seasonality_insights(
        ...
        try:
            ...
            if confidence > 0.7:
                insights.append("Strong seasonal patterns detected with high confidence")
            elif confidence > 0.4:
                insights.append("Moderate seasonal patterns detected")
            else:
                insights.append("Weak seasonal patterns detected - interpret cautiously")
            ...
            if len(methods_with_patterns) > 2:
                insights.append(f"Multiple detection methods confirm seasonality ({len(methods_with_patterns)} methods)")
...
def create_seasonality_detector(config: Optional[TrendConfig] = None) -> SeasonalityDetector:
    """
