...
from ..models import StatisticalSummary, TrendDirection, TrendPoint
...
class StatisticsEngine:
    ...
    def _analyze_distribution(self, values: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            distribution_stats["outlier_percentage"] = float(
    ...
    def _assess_data_quality(self, data: List[TrendPoint]) -> Dict[str, float]:
        ...
        try:
            ...
            quality_metrics["missing_value_percentage"] = float(
                nan_count / len(values) * 100
            )

            # Data consistency (check for duplicate timestamps)
            timestamps = [point.timestamp for point in data]
            unique_timestamps = len(set(timestamps))
            quality_metrics["timestamp_uniqueness"] = float(
            ...
            if len(timestamps) > 2:
                ...
                if intervals:
                    ...
                    quality_metrics["temporal_regularity"] = float(
                        1 / (1 + interval_cv)
                    )
    ...
    async def calculate_confidence_metrics(
        self, data: List[TrendPoint], statistics: Dict[str, float]
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            normality_confidence = min(
                normality_p * 2, 1.0
            )  # Scale p-value to confidence
            ...
            if not comparison_data:
                ...
                if min_length > 1:
                    ...
                    spearman_corr, _ = stats.spearmanr(
                        primary_values, comparison_values
                    )
    ...
    def generate_statistical_summary(
        self, statistics: Dict[str, float], confidence_metrics: Dict[str, float]
    ) -> List[str]:
        """
        ...
        try:
            ...
            if data_points < 20:
                insights.append(
                    "Limited data available - analysis may be less reliable"
                )
            ...
            if reliability_score > 0.8:
                ...
                insights.append(
                    "Data quality issues detected - interpret results cautiously"
                )
            ...
            if cv > 0.5:
                insights.append(
                    "High variability detected - values fluctuate significantly"
                )
            elif cv < 0.1:
                insights.append(
                    "Low variability detected - values are relatively stable"
                )
            ...
            if abs(growth_rate) > 0.5:
                ...
                insights.append(
                    f"Significant {direction} of {abs(growth_rate):.1%} detected"
                )
...
__all__ = ["StatisticsEngine", "create_statistics_engine"]
