...
from ..models import TrendAnalysis, TrendPoint, TrendType, TrendConfig
...
class TrendAnalyzer:
    ...
    async def analyze_trend(
        ...
        try:
            ...
            direction, strength = await self.trend_detector.detect_trend_direction(processed_data)
            
            # Calculate comprehensive statistics
            statistics = await self.statistics_engine.calculate_statistics(processed_data)
            
            # Detect seasonality patterns
            seasonality = await self.seasonality_detector.detect_seasonality(processed_data)
            ...
            confidence = self._calculate_confidence(processed_data, statistics, seasonality)
            ...
            if getattr(self.config, 'save_plots', True):
                visualization_path = await self.visualizer.generate_trend_visualization(trend_analysis)
            ...
            self.logger.info(f"Trend analysis completed for {trend_type.value} in {execution_time:.2f}s")
    ...
    def _extract_comprehensive_insights(
        ...
        if data_quality < 0.8:
            insights.append(f"Data quality moderate ({data_quality:.2f}) - results may be less reliable")
        elif data_quality > 0.95:
            insights.append(f"High data quality ({data_quality:.2f}) - results are highly reliable")
        
        # Trend significance
        if len(data) > 30:  # Sufficient data for significance testing
            if strength > 0.7:
                insights.append("Strong trend detected - significant pattern identified")
            elif strength > 0.3:
                insights.append("Moderate trend detected - noticeable pattern present")
            else:
                insights.append("Weak or no trend detected - data appears relatively stable")
    ...
    def _assess_data_quality(self, data: List[TrendPoint]) -> float:
        ...
        if len(values) > 5:
            ...
            if iqr > 0:
                ...
                outliers = sum(1 for v in values if abs(v - np.median(values)) > outlier_threshold)
