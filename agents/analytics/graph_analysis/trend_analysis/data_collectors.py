...
class TimeSeriesDataCollector:
    ...
    async def initialize(self, neo4j_driver: AsyncDriver) -> None:
        ...
        collectors = [
            ...
            self.pattern_collector,
            self.domain_collector,
            self.citation_collector,
            self.author_collector,
        ]
    ...
    async def get_time_series_data(
        ...
        try:
            if trend_type == TrendType.DOCUMENT_GROWTH:
                return await self.document_collector.collect_data(
                    domain, start_date, end_date, granularity
                )
            elif trend_type == TrendType.CONCEPT_EMERGENCE:
                return await self.concept_collector.collect_data(
                    domain, start_date, end_date, granularity
                )
            elif trend_type == TrendType.PATTERN_FREQUENCY:
                return await self.pattern_collector.collect_data(
                    domain, start_date, end_date, granularity
                )
            elif trend_type == TrendType.DOMAIN_ACTIVITY:
                return await self.domain_collector.collect_data(
                    domain, start_date, end_date, granularity
                )
            elif trend_type == TrendType.CITATION_NETWORKS:
                return await self.citation_collector.collect_data(
                    domain, start_date, end_date, granularity
                )
            elif trend_type == TrendType.AUTHOR_PRODUCTIVITY:
                return await self.author_collector.collect_data(
                    domain, start_date, end_date, granularity
                )
            else:
    ...
    def _smooth_data(
        self, data: List[TrendPoint], window_size: int = 3
    ) -> List[TrendPoint]:
...
class PatternFrequencyCollector:
    ...
    async def collect_data(
        ...
        async with self.neo4j_driver.session() as session:
            ...
            domain_clause = (
                "AND (n.domain = $domain OR m.domain = $domain)" if domain else ""
            )
...
class CitationNetworksCollector:
    ...
    async def collect_data(
        ...
        async with self.neo4j_driver.session() as session:
            domain_clause = (
                "AND (d1.domain = $domain OR d2.domain = $domain)" if domain else ""
            )
            params = {"domain": domain} if domain else {}
    ...
    async def collect_data(
        ...
        async with self.neo4j_driver.session() as session:
            ...
            async for record in result:
                ...
                productivity = (
                    total_documents / active_authors if active_authors > 0 else 0
                )
