...
class TrendPredictor:
    ...
    def __init__(self, config=None):
        ...
        self.prediction_horizon_days = (
            getattr(config, "prediction_horizon_days", 30) if config else 30
        )
        self.confidence_intervals = (
            getattr(config, "confidence_intervals", True) if config else True
        )
    ...
    async def generate_predictions(
        ...
        if len(data) < self.min_data_points:
            self.logger.warning(
                f"Insufficient data points for prediction: {len(data)} < {self.min_data_points}"
            )
            return []

        horizon = (
            horizon_days if horizon_days is not None else self.prediction_horizon_days
        )
        ...
        try:
            ...
            self.logger.info(
                f"Generated {len(ensemble_predictions)} predictions using {len(predictions_by_method)} methods"
            )
    ...
    async def _predict_with_method(
        ...
        if method == "linear":
            ...
            return self._polynomial_prediction(
                values, timestamps, original_data, horizon
            )
        elif method == "exponential_smoothing":
            return self._exponential_smoothing_prediction(
                values, original_data, horizon
            )
        elif method == "trend_decomposition":
            return self._trend_decomposition_prediction(
                values, timestamps, original_data, horizon
            )
    ...
    def _linear_prediction(
        ...
        try:
            ...
            for i in range(1, horizon + 1):
                ...
                days_from_start = (
                    future_timestamp - original_data[0].timestamp
                ).total_seconds() / 86400
    ...
    def _trend_decomposition_prediction(
        ...
        try:
            ...
            for i in range(1, horizon + 1):
                ...
                seasonal_index = (
                    (len(values) + i - 1) % len(seasonal) if len(seasonal) > 0 else 0
                )
            ...
            ensemble_predictions.append(
                TrendPoint(
                    ...
                    metadata={
                        ...
                        "method_weights": dict(
                            zip(predictions_by_method.keys(), weights)
                        ),
...
async def generate_predictions(
    data: List[TrendPoint], config=None, **kwargs
) -> List[TrendPoint]:
