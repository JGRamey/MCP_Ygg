...
from ..models import TrendPoint, TrendAnalysis, TrendDirection, TrendType
...
class TrendVisualizationEngine:
    ...
    def __init__(self, config: Optional[TrendConfig] = None, output_dir: Optional[Path] = None):
    ...
    async def _create_comprehensive_plot(self, trend_analysis: TrendAnalysis) -> plt.Figure:
    ...
    async def _create_statistical_plot(self, trend_analysis: TrendAnalysis) -> plt.Figure:
    ...
    async def _plot_main_trend(self, ax: plt.Axes, trend_analysis: TrendAnalysis):
        ...
        ax.plot(timestamps, values, 'o-', 
                color=self.colors['primary'], linewidth=2, markersize=4, label='Actual')
        ...
        if trend_analysis.predictions:
            ...
            ax.plot(pred_timestamps, pred_values, '--', 
                   color=self.colors['accent'], alpha=0.7, linewidth=2, label='Predicted')
        ...
        ax.axhline(y=np.mean(values), color=direction_color, linestyle=':', alpha=0.5, 
    ...
    async def _plot_statistical_summary(self, ax: plt.Axes, trend_analysis: TrendAnalysis):
        ...
        bars = ax.bar(key_stats.keys(), key_stats.values(), 
    ...
    async def _plot_value_distribution(self, ax: plt.Axes, trend_analysis: TrendAnalysis):
        """Plot value distribution as histogram."""
        values = [point.value for point in trend_analysis.data_points]
        
        ax.hist(values, bins=min(20, len(values)//2), 
    ...
    async def _plot_trend_characteristics(self, ax: plt.Axes, trend_analysis: TrendAnalysis):
        """Plot trend characteristics as radar/bar chart."""
        characteristics = {
            'Strength': trend_analysis.strength,
            'Confidence': trend_analysis.confidence,
            'Growth Rate': abs(trend_analysis.statistics.get('growth_rate', 0)),
            'Volatility': min(trend_analysis.statistics.get('volatility', 0) / 
                            trend_analysis.statistics.get('mean', 1), 1.0),
        ...
        bars = ax.bar(normalized_chars.keys(), normalized_chars.values(),
    ...
    async def _generate_interactive_visualization(
        ...
        try:
            ...
            if visualization_type == 'comprehensive':
                fig = make_subplots(
                    rows=2, cols=2,
                    subplot_titles=('Trend Analysis', 'Statistical Summary', 
                                  'Value Distribution', 'Trend Characteristics'),
            ...
            fig.add_trace(
                go.Scatter(x=timestamps, y=values, mode='lines+markers',
                          name='Actual', line=dict(color=self.colors['primary'])),
                row=1, col=1
            )
            ...
            if trend_analysis.predictions:
                ...
                fig.add_trace(
                    go.Scatter(x=pred_timestamps, y=pred_values, mode='lines',
                              name='Predicted', line=dict(color=self.colors['accent'], dash='dash')),
                    row=1, col=1
                )
            ...
            if visualization_type == 'comprehensive':
                ...
                fig.add_trace(
                    go.Bar(x=key_stats, y=stat_values, name='Statistics',
                ...
                fig.add_trace(
                    go.Bar(x=list(characteristics.keys()), y=list(characteristics.values()),
                          name='Characteristics', marker_color=self.colors['success']),
                    row=2, col=2
                )
    ...
    async def generate_comparison_visualization(
        ...
        try:
            ...
            for i, (label, analysis) in enumerate(trend_analyses.items()):
                ...
                ax1.plot(timestamps, values, 'o-', color=colors[i], label=label, alpha=0.7)
            ...
            ax4 = axes[1, 1]
            growth_rates = [analysis.statistics.get('growth_rate', 0) for analysis in trend_analyses.values()]
