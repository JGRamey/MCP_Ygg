...
from ..models import TrendPoint, TrendDirection
...
class TrendDetector:
    ...
    def __init__(self, config=None):
        ...
        self.volatility_threshold = getattr(config, 'volatility_threshold', 0.1) if config else 0.1
        self.trend_strength_threshold = getattr(config, 'trend_strength_threshold', 0.3) if config else 0.3
        self.min_r2_threshold = getattr(config, 'min_r2_threshold', 0.1) if config else 0.1
    ...
    async def detect_trend_direction(self, data: List[TrendPoint]) -> Tuple[TrendDirection, float]:
        """
        ...
        try:
            ...
            if len(direction_votes) > 2 and combined_strength < self.trend_strength_threshold:
                final_direction = TrendDirection.STABLE
                combined_strength = 0.0
            
            self.logger.info(f"Trend detected: {final_direction.value} (strength: {combined_strength:.3f})")
    ...
    def _detect_linear_trend(self, data: List[TrendPoint], values: np.ndarray) -> Tuple[TrendDirection, float]:
        ...
        try:
            ...
            normalized_slope = slope * time_span / np.mean(values) if np.mean(values) != 0 else 0
    ...
    def _detect_moving_average_trend(self, values: np.ndarray, window_size: Optional[int] = None) -> Tuple[TrendDirection, float]:
        ...
        try:
            ...
            ma_short = self._moving_average(values, window_size // 2) if window_size > 2 else values
            ...
            short_trend = (short_recent[-1] - short_recent[0]) / len(short_recent) if len(short_recent) > 1 else 0
            long_trend = (long_recent[-1] - long_recent[0]) / len(long_recent) if len(long_recent) > 1 else 0
    ...
    def _detect_extrema_trend(self, values: np.ndarray) -> Tuple[TrendDirection, float]:
        ...
        try:
            ...
            if len(troughs) >= 2:
                ...
                trough_trend = (trough_values[-1] - trough_values[0]) / len(trough_values)
    ...
    def detect_trend_changes(self, data: List[TrendPoint], window_size: int = 10) -> List[Dict[str, any]]:
        """
    ...
    def calculate_trend_strength_score(self, data: List[TrendPoint]) -> float:
        ...
        try:
            ...
            if len(values) > 1:
                ...
                if noise_level > 0:
                    signal_to_noise = trend_magnitude / (noise_level * np.sqrt(len(values)))
                    scores.append(min(signal_to_noise / 3, 1.0))  # Normalize
            
            # 4. Monotonicity score
            if len(values) > 2:
                sorted_values = np.sort(values)
                if np.array_equal(values, sorted_values) or np.array_equal(values, sorted_values[::-1]):
...
async def detect_trend_direction(data: List[TrendPoint], config=None) -> Tuple[TrendDirection, float]:
