...
from .models import NodeMetrics, CommunityInfo
    ...
    def calculate_efficiency_metrics(self, graph: nx.Graph) -> Dict[str, float]:
        ...
        try:
            ...
            if nx.is_connected(graph):
                ...
                if all_lengths:
                    metrics['average_efficiency'] = np.mean([1.0 / length for length in all_lengths])
                else:
...
class GraphValidator:
    ...
    def validate_graph(self, graph: nx.Graph) -> Tuple[bool, List[str]]:
        ...
        if len(graph.nodes) == 0:
            ...
            issues.append(f"Graph too large: {len(graph.nodes)} > {self.config.max_nodes}")
        
        if len(graph.edges) > self.config.max_edges:
            issues.append(f"Too many edges: {len(graph.edges)} > {self.config.max_edges}")
...
class GraphMetricsAggregator:
    ...
    def calculate_comprehensive_metrics(
        ...
        # Clustering metrics
        clustering_metrics = self.clustering_analyzer.calculate_clustering_metrics(graph)
        ...
        # Efficiency metrics
        efficiency_metrics = self.connectivity_analyzer.calculate_efficiency_metrics(graph)
