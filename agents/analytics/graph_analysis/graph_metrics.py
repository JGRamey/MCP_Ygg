...
from .base import GraphMetrics as BaseGraphMetrics
...
class GraphMetricsAnalyzer(BaseAnalyzer):
        ...
        for measure in CentralityMeasure:
            try:
                centrality = BaseGraphMetrics.calculate_centrality(
                    self.graph, measure, self.config.centrality_top_k
                )
    ...
    def _analyze_connectivity(self) -> Dict[str, Any]:
        ...
        if nx.is_connected(self.graph):
            connectivity_metrics["average_shortest_path"] = (
    ...
    def _analyze_clustering(self) -> Dict[str, Any]:
        ...
        top_clustered = sorted(
            local_clustering.items(), key=lambda x: x[1], reverse=True
        )[:10]
    ...
    def _calculate_efficiency_metrics(self) -> Dict[str, Any]:
        ...
        if nx.is_connected(self.graph):
            ...
            efficiency_metrics["cost_efficiency"] = (
                efficiency_metrics["global_efficiency"] / cost if cost > 0 else 0
            )
    ...
    def _analyze_robustness(self) -> Dict[str, Any]:
        ...
        try:
            robustness_metrics["algebraic_connectivity"] = nx.algebraic_connectivity(
                self.graph
            )
        except:
    ...
    async def get_node_importance(self, node_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "cross_domain_connections": (
                len(neighbor_domains) - 1 if len(neighbor_domains) > 1 else 0
            ),
    ...
    def _calculate_importance_score(
        self, centrality_metrics: Dict[str, float], degree: int
    ) -> float:
        """Calculate composite importance score."""
        weights = {"pagerank": 0.3, "betweenness": 0.3, "closeness": 0.2, "degree": 0.2}

        # Normalize degree (simple normalization)
        normalized_degree = (
            degree / self.graph.number_of_nodes()
