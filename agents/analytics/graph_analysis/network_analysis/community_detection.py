...
from ..models import AnalysisType, CommunityInfo, NetworkAnalysisResult, NodeMetrics
...
class CommunityDetector:
    ...
    async def analyze(self, graph: nx.Graph) -> NetworkAnalysisResult:
        ...
        node_metrics = self._create_node_metrics_with_communities(
            graph, communities_info
        )
        ...
        insights = self._generate_community_insights(
            graph, communities_info, graph_metrics
        )

        # Generate recommendations
        recommendations = self._generate_community_recommendations(
            communities_info, graph_metrics
        )
    ...
    def _detect_communities_multi_algorithm(self, graph: nx.Graph) -> Dict[str, any]:
        ...
        try:
            communities = greedy_modularity_communities(
                graph, resolution=self.config.resolution_parameter
            )
    ...
    def _select_best_communities(
        self, graph: nx.Graph, results: Dict[str, any]
    ) -> tuple:
    ...
    def _create_community_info(
        self, graph: nx.Graph, community_id: int, nodes: List[str]
    ) -> CommunityInfo:
        ...
        metadata = {
            ...
            "diameter": (
                ...
                if nx.is_connected(subgraph) and len(nodes) > 1
                else 0
            ),
        }
    ...
    def _generate_community_description(
        self, graph: nx.Graph, community_nodes: List[str]
    ) -> str:
        ...
        size_desc = (
            ...
            else "medium" if len(community_nodes) > 10 else "small"
        )
    ...
    def _create_node_metrics_with_communities(
        self, graph: nx.Graph, communities_info: List[CommunityInfo]
    ) -> List[NodeMetrics]:
    ...
    def _calculate_community_graph_metrics(
        self, graph: nx.Graph, communities: List[CommunityInfo], algorithm: str
    ) -> Dict[str, any]:
        ...
        if communities:
            ...
            if total_edges > 0:
                ...
                if total_possible_internal > 0:
                    metrics["performance"] = (
                        sum(internal_edges) / total_possible_internal
                    )
    ...
    def _generate_community_insights(
        ...
        insights.append(
            f"Detected {len(communities)} communities using {graph_metrics.get('algorithm_used', 'unknown')} algorithm"
        )
        ...
        if "modularity" in graph_metrics:
            ...
            if modularity > 0.3:
                insights.append(
                    "High modularity indicates well-defined community structure"
                )
            elif modularity < 0.1:
        ...
        if communities:
            ...
            if "coverage" in graph_metrics:
                ...
                insights.append(
                    f"Community coverage: {coverage:.1%} of all edges are within communities"
                )
    ...
    def _analyze_community_domains(
        self, graph: nx.Graph, communities: List[CommunityInfo]
    ) -> List[str]:
    ...
    def _generate_community_recommendations(
        self, communities: List[CommunityInfo], graph_metrics: Dict[str, any]
    ) -> List[str]:
        ...
        if modularity > 0.3:
            recommendations.append(
                "High modularity suggests opportunities for specialized approaches per community"
            )
        elif modularity < 0.1:
            recommendations.append(
                "Low modularity suggests focusing on global rather than community-specific strategies"
            )
        ...
        if communities:
            ...
            if size_variance > np.mean(sizes) ** 2:
                recommendations.append(
                    "High community size variance suggests different management strategies needed"
                )
            ...
            if "performance" in graph_metrics:
                ...
                if performance > 0.7:
                    recommendations.append(
                        "High community performance enables localized interventions"
                    )
                else:
                    recommendations.append(
                        "Low community performance requires cross-community coordination"
                    )
