...
from ..graph_utils import ClusteringAnalyzer as BaseClusteringAnalyzer, GraphMetricsAggregator
...
class ClusteringAnalyzer:
    ...
    async def analyze(self, graph: nx.Graph) -> NetworkAnalysisResult:
        ...
        clustering_metrics = self.base_clustering_analyzer.calculate_clustering_metrics(graph)
        
        # Analyze clustering distribution
        clustering_distribution = self._analyze_clustering_distribution(graph, clustering_metrics)
        
        # Analyze triangular structures
        triangle_analysis = self._analyze_triangular_structures(graph, clustering_metrics)
        
        # Identify clustering patterns
        clustering_patterns = self._identify_clustering_patterns(graph, clustering_metrics)
    ...
    def _analyze_clustering_distribution(
        ...
        distribution_stats = {
            ...
            'iqr': np.percentile(clustering_values, 75) - np.percentile(clustering_values, 25)
        }
        ...
        high_clustering_nodes = [node for node, coeff in clustering_dict.items() 
                               if coeff > high_clustering_threshold and coeff > 0.5]
        medium_clustering_nodes = [node for node, coeff in clustering_dict.items() 
                                 if medium_clustering_threshold <= coeff <= high_clustering_threshold]
        low_clustering_nodes = [node for node, coeff in clustering_dict.items() 
                              if coeff < medium_clustering_threshold]
        
        # Zero clustering analysis
        zero_clustering_nodes = [node for node, coeff in clustering_dict.items() if coeff == 0]
        ...
        return {
            ...
            'clustering_heterogeneity': distribution_stats['std'] / distribution_stats['mean'] if distribution_stats['mean'] > 0 else 0
        }
    ...
    def _analyze_triangular_structures(
        ...
        analysis = {
            ...
            'avg_triangles_per_node': np.mean(triangle_counts) if triangle_counts else 0,
        ...
        if triangle_counts:
            ...
            high_triangle_nodes = [node for node, count in triangles_dict.items() 
        ...
        if num_nodes >= 3:
            ...
            analysis['triangle_density'] = total_triangles / max_possible_triangles if max_possible_triangles > 0 else 0
        else:
    ...
    def _identify_clustering_patterns(
        ...
        for node in graph.nodes:
            ...
            if degree > np.percentile([graph.degree(n) for n in graph.nodes], 80) and clustering_coeff > 0.5:
    ...
    def _create_clustering_node_metrics(
        ...
        for node in graph.nodes:
            ...
            centrality_scores = {
                ...
                'is_high_clustering': node in clustering_distribution.get('high_clustering_nodes', []),
                'is_clustering_hub': node in clustering_patterns.get('clustering_hubs', []),
            ...
            clustering_importance = (clustering_coeff * 0.4 + 
    ...
    def _calculate_clustering_graph_metrics(
        ...
        if 'statistics' in clustering_distribution:
            ...
            metrics.update({
                ...
                'clustering_heterogeneity': clustering_distribution['clustering_heterogeneity'],
        ...
        # Categorical metrics
        metrics.update({
            'num_high_clustering': len(clustering_distribution.get('high_clustering_nodes', [])),
            'num_zero_clustering': len(clustering_distribution.get('zero_clustering_nodes', [])),
            'high_clustering_ratio': len(clustering_distribution.get('high_clustering_nodes', [])) / len(graph.nodes) if graph.nodes else 0
        })
        ...
        try:
            ...
            if num_edges > 0:
                ...
                clustering_enhancement = metrics['avg_clustering'] / random_clustering if random_clustering > 0 else 0
    ...
    def _generate_clustering_insights(
        ...
        if 'statistics' in clustering_distribution:
            ...
            if heterogeneity > 1.0:
                insights.append("High clustering heterogeneity indicates diverse local structures")
            elif heterogeneity < 0.3:
                insights.append("Low clustering heterogeneity suggests uniform local connectivity")
        ...
        try:
            ...
            if density > 0:
                ...
                enhancement = avg_clustering / random_clustering if random_clustering > 0 else 0
                insights.append(f"Clustering enhancement over random: {enhancement:.2f}x")
                
                if enhancement > 3:
                    insights.append("Strong clustering enhancement indicates structured local organization")
        except Exception:
            pass
        ...
        if high_clustering_nodes:
            insights.append(f"Identified {len(high_clustering_nodes)} highly clustered nodes")
        ...
        if triangle_density > 0.01:
            insights.append(f"Triangle density: {triangle_density:.4f} - rich triangular structure")
        ...
        if zero_clustering_nodes:
            zero_ratio = len(zero_clustering_nodes) / len(graph.nodes) if graph.nodes else 0
            insights.append(f"{len(zero_clustering_nodes)} nodes ({zero_ratio:.1%}) have zero clustering")
            
            if zero_ratio > 0.3:
                insights.append("High proportion of zero-clustering nodes suggests tree-like or star structures")
    ...
    def _generate_clustering_recommendations(
        ...
        if avg_clustering > 0.5:
            recommendations.append("High average clustering enables localized strategies")
        elif avg_clustering < 0.1:
            recommendations.append("Low clustering suggests focusing on global rather than local approaches")
        
        # Heterogeneity recommendations
        heterogeneity = clustering_distribution.get('clustering_heterogeneity', 0)
        
        if heterogeneity > 1.0:
            recommendations.append("High clustering heterogeneity requires differentiated local strategies")
        elif heterogeneity < 0.3:
            recommendations.append("Uniform clustering enables standardized local interventions")
        
        # High clustering nodes recommendations
        high_clustering_nodes = clustering_distribution.get('high_clustering_nodes', [])
        if high_clustering_nodes:
            recommendations.append("Consider highly clustered nodes for local influence strategies")
            recommendations.append("Leverage clustering hubs for community-based interventions")
        ...
        if total_triangles:
            ...
            if avg_triangles > 5:
                recommendations.append("Rich triangular structure supports trust-based mechanisms")
        
        # Zero clustering recommendations
        zero_clustering_nodes = clustering_distribution.get('zero_clustering_nodes', [])
        if len(zero_clustering_nodes) > len(clustering_distribution.get('high_clustering_nodes', [])):
            recommendations.append("Many nodes lack local clustering - consider strategic link formation")
    ...
    def identify_clustering_leaders(
        ...
        # Top clustered nodes
        clustering_dict = clustering_metrics['node_clustering']
        top_clustered = sorted(clustering_dict.items(), key=lambda x: x[1], reverse=True)[:top_k]
        ...
        triangles_dict = clustering_metrics['triangles']
        top_triangle_nodes = sorted(triangles_dict.items(), key=lambda x: x[1], reverse=True)[:top_k]
        leaders['triangle_leaders'] = [node for node, count in top_triangle_nodes if count > 0]
