...
class InfluenceAnalyzer:
        ...
        for node in nodes_to_analyze:
            ...
            for hops in range(1, 4):
                try:
                    ...
                    reach_metrics[f'reach_{hops}hop'][node] = len(reachable) - 1  # Exclude self
                except Exception:
    ...
    def _calculate_k_core_metrics(self, graph: nx.Graph) -> Dict[str, any]:
        ...
        try:
            ...
            for k in range(max_k + 1):
                k_shell_nodes = [node for node, core_num in k_core.items() if core_num == k]
                if k_shell_nodes:
    ...
    def _create_influence_node_metrics(
        ...
        for node in graph.nodes:
            ...
            centrality_scores = {
                ...
                'degree_centrality': influence_metrics['degree_centrality'].get(node, 0),
            }
    ...
    def _calculate_influence_graph_metrics(
        ...
        if k_core_values:
            metrics['influence_concentration'] = np.std(k_core_values) / np.mean(k_core_values) if np.mean(k_core_values) > 0 else 0
    ...
    def _generate_influence_insights(
        ...
        if max_k_core > 5:
            insights.append("High k-core values indicate presence of dense, influential subgroups")
        elif max_k_core < 2:
            insights.append("Low k-core values suggest sparse, tree-like structure with limited influence cores")
        ...
        if 'reach_2hop' in reach_metrics:
            ...
            if reach_values:
                ...
                if max_reach > len(graph.nodes) * 0.3:
                    insights.append("Some nodes can reach >30% of network within 2 hops - high influence potential")
        
        # Identify most influential nodes
        k_core_numbers = influence_metrics['k_core']
        top_k_core_nodes = sorted(k_core_numbers.items(), key=lambda x: x[1], reverse=True)[:5]
        ...
        if len(k_core_dist) > 1:
            ...
            insights.append(f"Most nodes ({most_populated_k[1]}) are in k-core level {most_populated_k[0]}")
        
        # Influence hierarchy insights
        if max_k_core > 3:
            insights.append("Multi-level influence hierarchy detected - consider tier-based strategies")
    ...
    def _generate_influence_recommendations(
        ...
        max_k_core = k_core_metrics['max_k_core']
        if max_k_core > 3:
            recommendations.append("Multi-level k-core structure suggests layered influence strategies")
        elif max_k_core < 2:
            recommendations.append("Low k-core structure suggests focusing on high-degree nodes for influence")
        ...
        if 'reach_2hop' in reach_metrics:
            ...
            if reach_values:
                ...
                if max_reach > avg_reach * 3:
                    recommendations.append("High reach variance suggests identifying and leveraging super-influencers")
                
                if avg_reach < 5:
                    recommendations.append("Low average reach suggests need for multiple seed nodes for broad influence")
        
        # Concentration based recommendations
        k_core_values = list(influence_metrics['k_core'].values())
        if k_core_values:
            concentration = np.std(k_core_values) / np.mean(k_core_values) if np.mean(k_core_values) > 0 else 0
            
            if concentration > 1.0:
                recommendations.append("High influence concentration enables targeted interventions on key nodes")
            else:
                recommendations.append("Distributed influence suggests broad-based engagement strategies")
    ...
    def identify_influence_leaders(
        ...
        # Return top-k nodes
        top_nodes = sorted(influence_scores.items(), key=lambda x: x[1], reverse=True)[:top_k]
