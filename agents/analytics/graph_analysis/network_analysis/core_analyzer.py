...
from ..graph_utils import GraphLoader, GraphMetricsAggregator, GraphValidator
...
class NetworkAnalyzer:
    ...
    async def analyze_network(
        ...
        try:
            ...
            self.logger.info(
                f"Loaded and validated graph with {len(graph.nodes)} nodes and {len(graph.edges)} edges"
            )
            ...
            self.logger.info(
                f"Network analysis completed in {execution_time:.2f} seconds"
            )
            return result
