...
class NetworkVisualizer:
    ...
    async def generate_network_visualization(
        ...
        try:
            self.logger.info(
                f"Generating visualization for {analysis_result.analysis_type.value}"
            )
            ...
            node_colors = self._get_node_colors(
                vis_graph, analysis_result, color_scheme
            )
    ...
    def generate_subgraph_visualization(
        ...
        try:
            ...
            subgraph_result = self._create_subgraph_result(
                analysis_result, nodes_subset
            )
    ...
    def _prepare_graph_for_visualization(self, graph: nx.Graph) -> nx.Graph:
        ...
        if len(graph.nodes) > max_nodes:
            self.logger.info(
                f"Sampling {max_nodes} nodes from {len(graph.nodes)} for visualization"
            )
    ...
    def _calculate_layout(
        ...
        try:
            ...
            self.logger.warning(
                f"Layout algorithm {algorithm} failed: {e}, using spring layout"
            )
            return nx.spring_layout(graph)

    def _get_node_colors(
        self, graph: nx.Graph, analysis_result: NetworkAnalysisResult, color_scheme: str
    ) -> List[float]:
        ...
        if color_scheme == "analysis_based" and analysis_result.node_metrics:
            ...
            if analysis_result.analysis_type == AnalysisType.CENTRALITY:
                ...
                centrality_key = self._get_primary_centrality_key(
                    analysis_result.node_metrics[0]
                )
    ...
    def _get_node_sizes(self, graph: nx.Graph) -> List[float]:
        ...
        node_size_multiplier = getattr(
            self.config, "node_size_multiplier", self.default_node_size_multiplier
        )

        node_sizes = [
            node_size_multiplier * (1 + graph.degree(node) / 10) for node in graph.nodes
        ]
    ...
    def _draw_network_components(
        ...
        nx.draw_networkx_edges(
            graph, pos, alpha=0.3, width=0.5, edge_color="gray", ax=ax
        )
    ...
    def _add_node_labels(
        self, graph: nx.Graph, pos: Dict[str, Tuple[float, float]], ax
    ) -> None:
    ...
    def _create_subgraph_result(
        self, analysis_result: NetworkAnalysisResult, nodes_subset: List[str]
        ...
        subgraph_node_metrics = [
            metric
            for metric in analysis_result.node_metrics
    ...
    def create_comparison_visualization(
        self, graphs: List[nx.Graph], titles: List[str], save_plot: bool = True
    ) -> str:
        ...
        try:
            ...
            for i, (graph, title) in enumerate(zip(graphs, titles)):
                ...
                nx.draw_networkx_edges(vis_graph, pos, alpha=0.3, width=0.5, ax=ax)
...
async def generate_network_visualization(
    graph: nx.Graph, analysis_result: NetworkAnalysisResult, config=None, **kwargs
) -> str:
    """Generate network visualization."""
    visualizer = create_network_visualizer(config)
    return await visualizer.generate_network_visualization(
        graph, analysis_result, **kwargs
    )
