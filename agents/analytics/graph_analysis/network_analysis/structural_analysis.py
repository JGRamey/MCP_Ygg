...
from ..graph_utils import ConnectivityAnalyzer, GraphMetricsAggregator, GraphStatistics
...
class StructuralAnalyzer:
    ...
    def _calculate_small_world_metrics(self, graph: nx.Graph) -> Dict[str, any]:
        ...
        try:
            ...
            if num_edges > 0:
                ...
                # Calculate ratios
                clustering_ratio = (
                    actual_clustering / random_clustering
    ...
    def _calculate_robustness_metrics(self, graph: nx.Graph) -> Dict[str, any]:
        ...
        if len(graph.nodes) > 10:  # Only for larger graphs
            try:
                ...
                high_degree_nodes = [
                    node for node, degree in degree_sorted[:sample_size]
                ]
    ...
    def _generate_structural_insights(
        ...
        if is_small_world:
            ...
            insights.append(
                f"Network shows small-world tendencies (coefficient: {small_world_coeff:.2f})"
            )
        ...
        if is_scale_free:
            insights.append(
                "Degree distribution suggests scale-free network properties"
            )
        ...
        if degree_heterogeneity > 1.0:
            ...
            insights.append(
                "Low degree heterogeneity suggests relatively uniform connectivity"
            )
        ...
        if algebraic_connectivity > 0.1:
            insights.append(
                "High algebraic connectivity indicates robust network structure"
            )
        elif algebraic_connectivity < 0.01:
            insights.append(
                "Low algebraic connectivity suggests structural vulnerability"
            )
    ...
    def _generate_structural_recommendations(
        ...
        if not is_connected:
            ...
            recommendations.append(
                f"Bridge {num_components} components to improve connectivity"
            )
        ...
        if is_small_world:
            recommendations.append(
                "Leverage small-world properties for efficient information spread"
            )
        else:
            clustering_ratio = small_world_metrics.get("clustering_ratio", 0)
            if clustering_ratio < 1:
                recommendations.append(
                    "Consider increasing local clustering to improve information retention"
                )
        ...
        if density < 0.05:
            recommendations.append(
                "Low density suggests opportunities for strategic link formation"
            )
        elif density > 0.5:
        ...
        if algebraic_connectivity < 0.01:
            recommendations.append(
                "Improve network robustness by strengthening weak connections"
            )
