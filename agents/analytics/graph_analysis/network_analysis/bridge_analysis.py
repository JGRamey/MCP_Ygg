...
class BridgeAnalyzer:
    ...
    async def analyze(self, graph: nx.Graph) -> NetworkAnalysisResult:
        ...
        graph_metrics = self._calculate_bridge_graph_metrics(
            graph, betweenness_metrics, edge_betweenness_metrics, 
            structural_holes_metrics, bridge_identification
        )
    ...
    def _calculate_betweenness_metrics(self, graph: nx.Graph) -> Dict[str, any]:
        ...
        try:
            ...
            if self.config.normalize_centrality:
                ...
                normalized_betweenness = {node: score / normalization_factor 
    ...
    def _identify_bridge_nodes(
        ...
        # Identify bridge nodes
        bridge_nodes = [node for node, score in betweenness.items() 
    ...
    def _create_bridge_node_metrics(
        ...
        for node in graph.nodes:
            ...
            centrality_scores = {
                ...
                'normalized_betweenness': betweenness_metrics['normalized_betweenness'].get(node, 0),
                'effective_size': structural_holes_metrics['effective_size'].get(node, 0),
                ...
                'is_articulation_point': node in bridge_identification['articulation_points'],
                'is_bridge_edge_node': node in bridge_identification['bridge_edge_nodes']
            }
            
            # Calculate bridge importance score
            betweenness_score = betweenness_metrics['betweenness'].get(node, 0)
            effective_size_score = structural_holes_metrics['effective_size'].get(node, 0)
            is_articulation = 1.0 if node in bridge_identification['articulation_points'] else 0.0
            
            bridge_importance = (betweenness_score * 0.4 + 
    ...
    def _calculate_bridge_graph_metrics(
        ...
        if betweenness_values:
            metrics.update({
                ...
                'bridge_concentration': len(bridge_identification['bridge_nodes']) / len(graph.nodes) if graph.nodes else 0
            })
        
        # Structural holes metrics
        effective_size_values = list(structural_holes_metrics['effective_size'].values())
        ...
        metrics['structural_vulnerability'] = len(bridge_identification['articulation_points']) / len(graph.nodes) if graph.nodes else 0
        
        # Edge criticality
        if edge_betweenness_metrics['edge_betweenness']:
            edge_bet_values = list(edge_betweenness_metrics['edge_betweenness'].values())
    ...
    def _generate_bridge_insights(
        ...
        insights.append(f"Identified {num_bridges} bridge nodes ({bridge_percentage:.1f}% of network)")
        ...
        if num_articulation_points > 0:
            vulnerability = num_articulation_points / total_nodes if total_nodes > 0 else 0
            insights.append(f"Network has {num_articulation_points} articulation points ({vulnerability:.1%} vulnerability)")
            
            if vulnerability > 0.1:
                insights.append("High structural vulnerability - network depends heavily on key nodes")
            elif vulnerability < 0.05:
                insights.append("Low structural vulnerability - network has redundant paths")
    ...
    def _generate_bridge_recommendations(
        ...
        if num_articulation_points > 0:
            recommendations.append("Create redundant paths around articulation points to improve robustness")
        
        if num_bridge_edges > 0:
            recommendations.append("Consider strengthening or creating alternative paths for critical bridge edges")
        
        # Structural holes recommendations
        effective_size = structural_holes_metrics['effective_size']
        if effective_size:
            max_effective_size = max(effective_size.values()) if effective_size.values() else 0
            
            if max_effective_size > 5:
                recommendations.append("Leverage nodes with structural holes for brokerage opportunities")
                recommendations.append("Consider nodes with high effective size for cross-group coordination")
        ...
        if betweenness:
            ...
            if max_betweenness > 0.1:
                recommendations.append("High betweenness nodes are bottlenecks - consider load balancing strategies")
            
            # Concentration analysis
            betweenness_values = list(betweenness.values())
            if len(betweenness_values) > 1:
                concentration = np.std(betweenness_values) / np.mean(betweenness_values) if np.mean(betweenness_values) > 0 else 0
                
                if concentration > 1.0:
                    recommendations.append("High betweenness concentration suggests centralized control points")
                else:
                    recommendations.append("Distributed betweenness suggests decentralized bridge structure")
    ...
    def identify_critical_nodes(
        ...
        critical_nodes = sorted(criticality_scores.items(), key=lambda x: x[1], reverse=True)[:10]
