    ...
    def _generate_centrality_insights(
        ...
        for measure_name, scores in centrality_measures.items():
            if scores:
                ...
                if len(values) > 1:
                    concentration = np.std(values) / np.mean(values) if np.mean(values) > 0 else 0
                    if concentration > 1.0:
                        insights.append(f"High {measure_name} concentration suggests hierarchical structure")
                    elif concentration < 0.5:
                        insights.append(f"Low {measure_name} concentration suggests egalitarian structure")
        ...
        if 'pagerank' in centrality_measures and 'betweenness' in centrality_measures:
            ...
            for node in graph.nodes:
                ...
                if pr_score > np.percentile(list(pr_scores.values()), 80) and \
                   bt_score > np.percentile(list(bt_scores.values()), 80):
                    influential_bridges.append(node)
            
            if influential_bridges:
                insights.append(f"Identified {len(influential_bridges)} influential bridge nodes")
        ...
        if 'pagerank' in centrality_measures:
            ...
            if pagerank_values:
                ...
                if pagerank_concentration > 1.0:
                    recommendations.append("High PageRank concentration suggests focusing on top-tier nodes for maximum impact")
                else:
                    recommendations.append("Distributed PageRank suggests multi-node strategies for broad influence")
        ...
        if 'betweenness' in centrality_measures:
            ...
            if betweenness_values:
                ...
                if max_betweenness > 0.1:  # High betweenness
                    recommendations.append("High betweenness centrality nodes are critical for network connectivity")
                else:
                    recommendations.append("Low betweenness values suggest robust, redundant connectivity")
        ...
        if 'degree' in centrality_measures:
            ...
            if degree_values:
                ...
                if avg_degree < 0.1:  # Low connectivity
                    recommendations.append("Low average degree suggests opportunities for strategic link formation")
                elif avg_degree > 0.5:  # High connectivity
                    recommendations.append("High connectivity enables rapid information diffusion")
