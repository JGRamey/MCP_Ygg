...
from ..graph_utils import GraphLoader, ConnectivityAnalyzer, GraphStatistics
...
class PathAnalyzer:
    ...
    async def analyze_paths(self, graph: nx.Graph) -> NetworkAnalysisResult:
        ...
        try:
            ...
            if not nx.is_connected(graph):
                ...
                self.logger.info(f"Working with largest connected component ({len(largest_cc)} nodes)")
            else:
                working_graph = graph
    ...
    def _calculate_path_metrics(self, working_graph: nx.Graph) -> Dict[str, Any]:
        ...
        try:
            if num_nodes <= 1000:  # Full calculation for smaller graphs
                ...
                path_metrics['diameter'] = max(eccentricities.values()) if eccentricities else 0
                path_metrics['radius'] = min(eccentricities.values()) if eccentricities else 0
                path_metrics['center_nodes'] = [
                    node for node, ecc in eccentricities.items() 
            ...
            if all_lengths:
                ...
                path_metrics['efficiency'] = np.mean([1.0 / length for length in all_lengths])
                
                # Path length distribution
                length_distribution = Counter(all_lengths)
                path_metrics['length_distribution'] = length_distribution
                path_metrics['most_common_length'] = length_distribution.most_common(1)[0][0]
            else:
                path_metrics.update({
                    'avg_path_length': 0,
                    'max_path_length': 0,
        ...
        for node in graph.nodes:
            ...
            try:
                ...
                if node in path_metrics.get('path_lengths', {}):
                    node_path_lengths = list(path_metrics['path_lengths'][node].values())
                    # Exclude self (distance 0)
                    node_path_lengths = [length for length in node_path_lengths if length > 0]
                    avg_distance_from_node = np.mean(node_path_lengths) if node_path_lengths else 0
                    max_distance_from_node = max(node_path_lengths) if node_path_lengths else 0
                else:
    ...
    def _compile_graph_metrics(
        ...
        graph_metrics = {
            ...
            'path_length_range': path_metrics.get('max_path_length', 0) - 1 
                if path_metrics.get('max_path_length', 0) > 0 else 0
        }
        ...
        if length_distribution:
            graph_metrics.update({
                'most_common_path_length': path_metrics.get('most_common_length', 0),
    ...
    def _generate_insights(
        ...
        if center_nodes:
            ...
            if len(center_nodes) <= 3:
                ...
                insights.append(f"Center nodes are strategically positioned for optimal reach")
        
        # Path length distribution analysis
        if path_metrics.get('most_common_length'):
            insights.append(f"Most common path length: {path_metrics['most_common_length']}")
        ...
        if diameter > 0 and radius > 0:
            ...
            if diameter_ratio > 1.8:
                insights.append("Large diameter-to-radius ratio suggests elongated network structure")
            elif diameter_ratio < 1.2:
                insights.append("Small diameter-to-radius ratio suggests compact, centralized structure")
    ...
    def _generate_recommendations(self, graph_metrics: Dict[str, float]) -> List[str]:
        ...
        if avg_path_length > 4:
            ...
            recommendations.append("Very short paths suggest high density - monitor for overcrowding")
        
        efficiency = graph_metrics.get('efficiency', 0)
        if efficiency < 0.2:
            recommendations.append("Low efficiency suggests need for better connectivity")
        
        num_center_nodes = graph_metrics.get('num_center_nodes', 0)
        if num_center_nodes == 1:
            recommendations.append("Single center node creates vulnerability - consider backup paths")
        elif num_center_nodes > 5:
            recommendations.append("Multiple center nodes provide redundancy and robustness")
