...
class LicenseDetector:
    ...
    def _load_license_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            'cc_by_nc': {
                ...
                'indicators': ['CC BY-NC', 'Creative Commons Attribution-NonCommercial'],
                'is_free': True,
            ...
            'cc_by_nd': {
                ...
                'indicators': ['CC BY-ND', 'Creative Commons Attribution-NoDerivatives'],
                'is_free': True,
                'commercial_use': True,
    ...
    def detect_license(self, text: str, url: Optional[str] = None) -> Optional[LicenseInfo]:
        ...
        for license_type, license_data in self.license_patterns.items():
            ...
            if confidence > 0.3:  # Minimum threshold
                license_info = LicenseInfo(
                    ...
                    modification_allowed=license_data.get('modification_allowed', False),
                    distribution_allowed=license_data.get('is_free', False),
                    attribution_required=license_data.get('attribution_required', False),
                    share_alike_required=license_data.get('share_alike_required', False),
...
class PublicDomainDatabase:
    ...
    def __init__(self, db_path: str = "agents/copyright_checker/lists/public_domain.json"):
...
class DomainSpecificChecker:
    ...
    def __init__(self):
        ...
        self.domain_rules = {
            'religion': {
                'ancient_texts': [
                    'bible', 'quran', 'torah', 'vedas', 'upanishads',
                    'tripitaka', 'talmud', 'book of mormon'
                ],
            ...
            'philosophy': {
                'ancient_authors': [
                    'plato', 'aristotle', 'confucius', 'lao tzu', 'marcus aurelius',
                    'epictetus', 'seneca', 'cicero'
                ],
            ...
            'literature': {
                'classic_authors': [
                    ...
                    'cervantes', 'molière', 'goethe'
                ],
                'public_domain_threshold': 1923,
                'risk_factors': ['modern adaptation', 'new edition', 'annotated version']
            },
            'history': {
                'ancient_sources': [
                    'herodotus', 'thucydides', 'tacitus', 'suetonius',
                    'plutarch', 'josephus', 'bede'
                ],
                'public_domain_threshold': 1900,
            ...
            'science': {
                'historical_works': [
                    'newton', 'galileo', 'copernicus', 'kepler', 'darwin'
                ],
                'public_domain_threshold': 1923,
                'risk_factors': ['modern textbook', 'recent research', 'contemporary analysis']
            },
            'math': {
                'classical_works': [
                    'euclid', 'archimedes', 'pythagoras', 'fibonacci',
                    'fermat', 'pascal', 'leibniz', 'euler'
                ],
                'public_domain_threshold': 1900,
                'risk_factors': ['modern proof', 'contemporary explanation', 'new methodology']
            }
        }
    
    def check_domain_specific_rules(self, domain: str, title: str, author: str, content: str) -> Tuple[bool, str, List[str]]:
        ...
        ancient_items = rules.get('ancient_texts', []) + rules.get('ancient_authors', []) + rules.get('classic_authors', [])
...
class CopyrightChecker:
            ...
            try:
                ...
                for result_data in data.get('results', []):
                    ...
                    if result_data.get('license_info'):
                        result_data['license_info'] = LicenseInfo(**result_data['license_info'])
                    
                    result = CopyrightStatus(**result_data)
                    self.copyright_results[result.doc_id] = result
                
                logger.info(f"Loaded {len(self.copyright_results)} existing copyright results")
    ...
    async def check_copyright(self, document_data: Dict[str, Any]) -> CopyrightStatus:
        ...
        content = document_data.get('content', '') or document_data.get('cleaned_content', '')
        ...
        try:
            ...
            if self.config.get('check_content', True) and content:
                ...
                copyright_notices = self.license_detector.detect_copyright_notices(content)
                ...
                if copyright_notices:
                    ...
                    confidence_factors.append(('copyright_notices', -0.5))  # Negative factor
                
                # Detect public domain indicators
                pd_indicators = self.license_detector.detect_public_domain_indicators(content)
            ...
            if domain:
                likely_pd, reason, risk_factors = self.domain_checker.check_domain_specific_rules(
            ...
            result.risk_level = self._calculate_risk_level(result.confidence, result.copyright_free)
            ...
            logger.info(f"Copyright check completed for {doc_id}: {result.risk_level} risk, {result.confidence:.2f} confidence")
    ...
    def get_free_documents(self, min_confidence: float = 0.7) -> List[CopyrightStatus]:
        ...
        return [
            result for result in self.copyright_results.values()
    ...
    def generate_compliance_report(self) -> Dict[str, Any]:
        ...
        risk_distribution = {
            'low': len([r for r in self.copyright_results.values() if r.risk_level == 'low']),
            'medium': len([r for r in self.copyright_results.values() if r.risk_level == 'medium']),
            'high': len([r for r in self.copyright_results.values() if r.risk_level == 'high'])
        }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if risky_docs:
            recommendations.append(f"Review {len(risky_docs)} documents with unclear copyright status")
        
        high_risk = [r for r in self.copyright_results.values() if r.risk_level == 'high']
        if high_risk:
            recommendations.append(f"Remove or replace {len(high_risk)} high-risk documents")
        
        no_license = [r for r in self.copyright_results.values() if not r.license_info]
        if no_license:
            recommendations.append(f"Investigate licensing for {len(no_license)} documents without clear licenses")
