...
from ..knowledge_integration.integration_orchestrator import (
    KnowledgeIntegrationOrchestrator,
)
from ..quality_assessment.reliability_scorer import ConfidenceLevel, ReliabilityScorer

# Import Phase 4 components
from ..scraper.intelligent_scraper_agent import ContentType, IntelligentScraperAgent

sys.path.append("/Users/grant/Documents/GitHub/MCP_Ygg/data")
from staging_manager import ContentMetadata, Priority, SourceType, StagingManager
...
class Phase4ValidationPipeline:
    ...
    async def run_complete_pipeline(
        self, url: str, test_name: str = "default"
    ) -> PipelineTestResult:
        ...
        try:
            ...
            logger.info(
                f"   Authority Score: {scraped_doc.metadata.authority_score:.2f}"
            )
            ...
            logger.info(
                f"   Verifiable claims: {sum(1 for c in content_analysis.claims if c.verifiable)}"
            )
            ...
            logger.info(
                "Stage 3: Cross-Reference Validation Against Authoritative Sources"
            )

            # Validate claims
            verifiable_claims = [
                claim for claim in content_analysis.claims if claim.verifiable
            ]
            if verifiable_claims:
                cross_ref_result = (
                    await self.cross_reference_engine.cross_reference_search(
            ...
            citation_validations = await self.cross_reference_engine.validate_citations(
                citations
            )
            ...
            if verifiable_claims:
                graph_validation = (
                    await self.cross_reference_engine.check_against_knowledge_graph(
            ...
            logger.info(
                f"   Citations validated: {sum(1 for c in citation_validations if c.found)}/{len(citation_validations)}"
            )
            logger.info(
                f"   Graph consistency: {graph_validation.consistency_score:.2f}"
            )
            ...
            logger.info(
                f"   Confidence level: {reliability_score.confidence_level.value}"
            )
            ...
            content_metadata = ContentMetadata(
                ...
                author=(
                    scraped_doc.metadata.authors[0]
                    if scraped_doc.metadata.authors
                    else None
                ),
                date=(
                    scraped_doc.metadata.publication_date.isoformat()
            ...
            analysis_results = AnalysisResults(
                concepts_extracted=[
                    concept.name for concept in content_analysis.concepts
                ],
                claims_identified=[
                    claim.claim_text
            ...
            await self.staging_manager.complete_analysis(
                submission_id, analysis_results
            )

            # Auto-approve or require manual review based on confidence
            if reliability_score.confidence_level == ConfidenceLevel.HIGH:
                await self.staging_manager.approve_content(
                    submission_id, "Auto-approved by Phase 4 pipeline"
                )
            elif reliability_score.confidence_level == ConfidenceLevel.LOW:
                await self.staging_manager.reject_content(
                    submission_id,
                    "Low reliability score",
                    "Auto-rejected by Phase 4 pipeline",
                )
            ...
            if reliability_score.confidence_level != ConfidenceLevel.LOW:
                ...
                try:
                    integration_result, provenance = (
                        await self.integration_orchestrator.comprehensive_integration(
                            scraped_doc,
                            content_analysis,
                            reliability_score,
                            [
                    ...
                    logger.info(
                        f"   Relationships created: {integration_result.relationships_created}"
                    )
                    logger.info(
                        f"   Vectors inserted: {integration_result.vectors_inserted}"
                    )
                    ...
                    if integration_result.errors:
                        ...
                        logger.warning(
                            f"   Integration errors: {len(integration_result.errors)}"
                        )
            ...
            success = (
                len(stages_completed) >= 4 and "reliability_scoring" in stages_completed
            )

            logger.info(
                f"🎉 Pipeline test '{test_name}' completed in {processing_time:.2f}s"
            )
            logger.info(
                f"   Stages completed: {len(stages_completed)}/{len(self.stages)}"
            )
            ...
            return PipelineTestResult(
                ...
                metadata={
                    ...
                    "verifiable_claims": sum(
                        1 for c in content_analysis.claims if c.verifiable
                    ),
                    "submission_id": (
...
async def run_comprehensive_tests():
    ...
    if results:
        ...
        for stage in sorted(all_stages):
            completion_rate = (
                sum(1 for r in results if stage in r.stages_completed)
                / total_tests
                * 100
            )
...
async def run_performance_benchmarks():
    ...
    components = [
        (
            "intelligent_scraping",
            lambda: pipeline.scraper_agent.scrape_with_intelligence(test_url),
        ),
        (
            "content_analysis",
            lambda: pipeline.content_analyzer.analyze_content(None),
        ),  # Would need scraped doc
        (
            "cross_reference",
            lambda: pipeline.cross_reference_engine.cross_reference_search(
                "test claim", "philosophy"
            ),
        ),
        (
            "reliability_scoring",
            lambda: pipeline.reliability_scorer.calculate_reliability_score(
                None, None, None, [], None
            ),
        ),
    ]
    ...
    for component_name, component_func in components:
        try:
            ...
            logger.info(
                f"✅ {component_name}: {benchmarks[component_name]['avg_time']:.3f}s"
            )

        except Exception as e:
            benchmarks[component_name] = {"avg_time": 0, "status": f"error: {str(e)}"}
