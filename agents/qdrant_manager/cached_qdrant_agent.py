...
from .qdrant_agent import QdrantAgent, VectorOperationResult, VectorPoint, SearchQuery
...
class CachedQdrantAgent(QdrantAgent):
    ...
    async def get_collection_info_cached(self, collection_name: str) -> VectorOperationResult:
        """Get collection info with caching."""
        return await super().get_collection_info(collection_name)
    
    @cache_integration.cache_vector_search(ttl=600)  # 10 minutes
    async def get_point_cached(self, collection_name: str, point_id: Union[str, int]) -> VectorOperationResult:
    ...
    @cache_integration.cache_vector_search(ttl=3600)  # 1 hour
    async def get_collection_statistics_cached(self, collection_name: str) -> VectorOperationResult:
    ...
    @cache_integration.cache_vector_search(ttl=3600)  # 1 hour
    async def get_domain_vector_statistics_cached(self, domain: str) -> VectorOperationResult:
        ...
        statistics = {
            ...
            "mean_vector": vectors_np.mean(axis=0).tolist() if vectors_np.size > 0 else [],
            "std_vector": vectors_np.std(axis=0).tolist() if vectors_np.size > 0 else [],
            "vector_norms": {
                "mean": float(np.linalg.norm(vectors_np, axis=1).mean()) if vectors_np.size > 0 else 0,
                "std": float(np.linalg.norm(vectors_np, axis=1).std()) if vectors_np.size > 0 else 0,
                "min": float(np.linalg.norm(vectors_np, axis=1).min()) if vectors_np.size > 0 else 0,
                "max": float(np.linalg.norm(vectors_np, axis=1).max()) if vectors_np.size > 0 else 0
            }
        }
    ...
    async def upsert_vector_with_cache_invalidation(
        ...
        if result.success:
            ...
            await self.cache_manager.cache.delete_pattern(f"vector_search:*{collection_name}*")
    ...
    async def warm_cache_for_collection(self, collection_name: str):
        ...
        try:
            ...
            common_domains = ['philosophy', 'science', 'mathematics', 'art', 'language', 'technology']
            for domain in common_domains:
                await self.get_domain_vector_statistics_cached(domain)
            
            logger.info(f"Cache warmed for collection: {collection_name}")
        except Exception as e:
            logger.warning(f"Cache warming failed for collection {collection_name}: {e}")
