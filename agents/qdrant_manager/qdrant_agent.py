...
class QdrantAgent:
    ...
    def _default_config(self) -> Dict[str, Any]:
        ...
        return {
            "connection": {"host": "localhost", "port": 6333, "timeout": 30},
            "collections": {
                "auto_create": True,
                "default_vector_size": 384,
                "default_distance": "Cosine",
            },
            "performance": {"batch_size": 100, "max_retries": 3},
        }
    ...
    async def upsert_point(
        self, collection_name: str, point: VectorPoint
        ...
        try:
            ...
            qdrant_point = PointStruct(
                id=point.id, vector=point.vector, payload=enhanced_payload
            )

            # Upsert to collection
            operation_info = self.client.upsert(
                collection_name=collection_name, points=[qdrant_point], wait=True
            )
            ...
            return VectorOperationResult(
                ...
                data={
                    ...
                    "operation_info": (
                        operation_info.dict()
                        if hasattr(operation_info, "dict")
                        else str(operation_info)
                    ),
                },
    ...
    async def get_collection_info(self, collection_name: str) -> VectorOperationResult:
        ...
        try:
            ...
            return VectorOperationResult(
                success=True, data=collection_stats, execution_time=execution_time
            )
    ...
    def _get_collection_vector_size(self, collection_name: str) -> Optional[int]:
        ...
        domain_collections = self.config.get("collections", {}).get(
            "domain_collections", {}
        )
        ...
        for field, value in filter_conditions.items():
            if isinstance(value, dict):
                ...
                if "match" in value:
                    conditions.append(
                        FieldCondition(
                            key=field, match=models.MatchValue(value=value["match"])
                        )
                    )
                elif "range" in value:
    ...
    async def optimize_collection(self, collection_name: str) -> VectorOperationResult:
        ...
        try:
            ...
            if (
                ...
                < self.config["optimization"]["auto_optimize_threshold"]
            ):
                return VectorOperationResult(
                    success=True,
