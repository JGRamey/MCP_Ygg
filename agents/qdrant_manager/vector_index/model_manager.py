...
from .models import ModelPerformance, ModelConfig
from .utils import (
...
class ModelManager:
    ...
    def select_optimal_model(self, content: Dict[str, Any]) -> str:
        ...
        logger.debug(f"Selected model '{selected_model}' with score {best_score:.2f} for content: {content.get('title', 'unknown')}")
    ...
    def benchmark_model(self, model_key: str, test_texts: List[str]) -> ModelPerformance:
        ...
        try:
            ...
            logger.info(f"Benchmarked {model_key}: speed={speed:.1f} docs/sec, quality={quality_score:.2f}")
            return performance
    ...
    def update_model_performance(self, model_key: str, feedback_score: float) -> None:
        ...
        if model_key in self.model_performances:
            ...
            logger.debug(f"Updated {model_key} performance based on feedback: {feedback_score}")
