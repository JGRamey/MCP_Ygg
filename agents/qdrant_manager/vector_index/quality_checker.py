...
from .utils import TRANSFORMERS_AVAILABLE, get_quality_thresholds, validate_embedding
...
class EmbeddingQualityChecker:
    ...
    def assess_quality(
        self, embedding: np.ndarray, content: Dict[str, Any], model_used: str
    ) -> EmbeddingQuality:
        ...
        if not validate_embedding(embedding):
            return self._create_failed_assessment(
                doc_id, model_used, "Invalid embedding vector"
            )
        ...
        # Outlier detection
        outlier_score = self._detect_outliers(
            embedding, content.get("domain", "general")
        )
        ...
        if outlier_score > self.quality_thresholds["outlier_threshold"]:
            ...
            recommendations.append(
                "Embedding appears to be an outlier - content may be unusual"
            )
    ...
    def _create_failed_assessment(
        self, doc_id: str, model_used: str, reason: str
    ) -> EmbeddingQuality:
    ...
    def _assess_vector_norm(self, embedding: np.ndarray) -> tuple[float, List[str]]:
        ...
        if embedding_norm < self.quality_thresholds["minimum_norm"]:
            quality_score = 0.3
            recommendations.append(
                "Embedding norm too low - may indicate poor text quality or processing issues"
            )
        elif embedding_norm > self.quality_thresholds["maximum_norm"]:
            quality_score = 0.7
            recommendations.append(
                "Embedding norm too high - may indicate outlier content or model issues"
            )
        elif embedding_norm < 0.5:
            quality_score = 0.8
            recommendations.append(
                "Embedding norm lower than typical - consider text preprocessing"
            )
    ...
    def _calculate_semantic_density(
        self, embedding: np.ndarray, content: Dict[str, Any]
    ) -> float:
        ...
        variance_factor = min(
            1.0, embedding_std * 10
        )  # Good embeddings have some variance
    ...
    def _generate_recommendations(
        ...
        if quality_score < 0.7:
            recommendations.append(
                "Consider trying a different embedding model for better quality"
            )

        if consistency_score < 0.6:
            recommendations.append(
                "Content may be too noisy or ambiguous - consider text preprocessing"
            )

        if semantic_density < 0.4:
            recommendations.append(
                "Content may lack semantic richness - ensure sufficient meaningful text"
            )

        if outlier_score > 2.5:
            recommendations.append(
                "Content appears unusual for its domain - verify content appropriateness"
            )

        if quality_score < 0.4:
            recommendations.append(
                "Consider reprocessing with different parameters or manual review"
            )

        # Positive recommendations
        if quality_score > 0.85 and consistency_score > 0.8:
            recommendations.append(
                "High-quality embedding - suitable for production use"
            )
    ...
    def update_quality_thresholds(
        self, domain: str, feedback_data: List[Dict[str, Any]]
    ) -> None:
        ...
        if len(quality_scores) >= 10:  # Need sufficient data
            ...
            if correlation > 0.5:  # Good correlation
                ...
                logger.info(
                    f"Updated quality threshold for {domain}: {old_threshold:.3f} -> {new_threshold:.3f}"
                )
