...
from .models import EmbeddingQuality, VectorIndexResult
...
class EnhancedVectorIndexer:
    ...
    def __init__(
        self,
        config_path: str = "agents/qdrant_manager/vector_index/enhanced_config.yaml",
    ):
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance": {"batch_size": 50, "max_workers": 4, "cache_ttl": 7200},
        }
    ...
    async def index_content_enhanced(
        self, content: Dict[str, Any], force_model: Optional[str] = None
    ) -> VectorIndexResult:
        ...
        with PerformanceTimer("Enhanced indexing") as timer:
            try:
                ...
                logger.debug(
                    f"Selected model '{selected_model}' for content: {content.get('title', 'untitled')}"
                )
                ...
                if self._should_try_alternative_model(quality_assessment):
                    alternative_model = self.model_manager.get_alternative_model(
                        selected_model
                    )
                    if alternative_model != selected_model:
                        logger.info(
                            f"Trying alternative model '{alternative_model}' for quality improvement"
                        )
                        alt_embedding = await self._generate_embedding(
                            content, alternative_model
                        )
                        if alt_embedding is not None:
                            ...
                            if (
                                ...
                                logger.info(
                                    f"Used alternative model with improved quality: {alt_quality.quality_score:.3f}"
                                )

                # Index using base indexer if available
                if self.base_indexer:
                    indexing_success = await self._index_with_base_indexer(
                        content, embedding
                    )
                else:
                ...
                confidence_score = self._calculate_confidence_score(
                    quality_assessment, selected_model
                )
    ...
    def _select_model(self, content: Dict[str, Any], force_model: Optional[str]) -> str:
        ...
        if self.config.get("enhanced_features", {}).get(
            "dynamic_model_selection", True
        ):
    ...
    async def _generate_embedding(
        self, content: Dict[str, Any], model_key: str
    ) -> Optional[np.ndarray]:
        ...
        try:
            ...
            embedding = await loop.run_in_executor(
                self.executor, self._generate_embedding_sync, model, text
            )
    ...
    def _should_try_alternative_model(
        self, quality_assessment: EmbeddingQuality
    ) -> bool:
        """Determine if alternative model should be tried"""
        min_threshold = self.config.get("quality_thresholds", {}).get(
            "minimum_quality_score", 0.6
        )
        return quality_assessment.quality_score < min_threshold

    async def _index_with_base_indexer(
        self, content: Dict[str, Any], embedding: np.ndarray
    ) -> bool:
        """Index using base indexer if available"""
        try:
            # Prepare content for base indexer
            indexer_content = {**content, "embeddings": embedding.tolist()}

            # Run in thread pool to avoid blocking
            loop = asyncio.get_event_loop()
            success = await loop.run_in_executor(
                self.executor, self.base_indexer.index_document, indexer_content
            )
    ...
    def _calculate_confidence_score(
        self, quality_assessment: EmbeddingQuality, model_used: str
    ) -> float:
    ...
    async def search_enhanced(
        ...
        try:
            ...
            query_model = (
                model_preference
                if model_preference in self.model_manager.models
    ...
    async def batch_process_enhanced(
        self, content_list: List[Dict[str, Any]], progress_callback=None
        ...
        for i in range(0, total, batch_size):
            ...
            batch_tasks = [self.index_content_enhanced(content) for content in batch]
...
async def main():
    ...
    try:
        ...
        for model, perf in stats.items():
            print(
                f"- {model}: quality={perf['quality_score']:.2f}, speed={perf['speed']:.1f} docs/sec"
            )
