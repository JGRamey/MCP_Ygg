...
class CollectionManager:
    ...
    async def initialize_collections(self, client: QdrantClient) -> bool:
        ...
        try:
            ...
            for collection_name, collection_config in self.domain_collections.items():
                if collection_name not in existing_names:
                    success = await self._create_collection(
                        client, collection_name, collection_config
                    )
                    if success:
                        created_count += 1
            ...
            logger.info(
                f"Collection initialization completed. Created {created_count} new collections"
            )
            return True
    ...
    async def _create_collection(
        self, client: QdrantClient, collection_name: str, config: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            for field_name, field_type in indexes:
                try:
                    ...
                    logger.debug(
                        f"Index creation skipped for {collection_name}.{field_name}: {e}"
                    )
    ...
    async def ensure_collection_exists(
        ...
        try:
            ...
            if collection_name in self.domain_collections:
                ...
                default_config = {
                    "vector_size": self.config.get("collections", {}).get(
                        "default_vector_size", 384
                    ),
                    "distance": self.config.get("collections", {}).get(
                        "default_distance", "Cosine"
                    ),
                    "hnsw_config": {"m": 16, "ef_construct": 200},
                }
                return await self._create_collection(
                    client, collection_name, default_config
                )
    ...
    async def recreate_collection(
        ...
        try:
            ...
            if collection_name in self.domain_collections:
                ...
                logger.error(
                    f"No configuration found for collection: {collection_name}"
                )
                return False
    ...
    def get_recommended_config(
        self, collection_purpose: str, expected_size: int
    ) -> Dict[str, Any]:
    ...
    def get_domain_collections(self) -> List[str]:
        ...
        return [
            name
            for name in self.domain_collections.keys()
    ...
    def get_entity_collections(self) -> List[str]:
        ...
        return [
            name
            for name in self.domain_collections.keys()
            if name in ["entities", "concepts", "claims"]
        ]
