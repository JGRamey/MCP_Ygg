...
from ..fact_verifier.cross_reference_engine import (
    CitationValidation,
...
class ReliabilityScorer:
    ...
    def __init__(self):
        ...
        # Quality thresholds
        self.thresholds = {"high": 0.8, "medium": 0.6, "low": 0.0}
    ...
    def calculate_reliability_score(
        ...
        improvements = self._generate_improvements(
            scores, scraped_doc, content_analysis
        )
    ...
    def _score_source_authority(self, scraped_doc: ScrapedDocument) -> float:
        ...
        if scraped_doc.metadata.publication_date:
            ...
            years_old = (
                datetime.datetime.now() - scraped_doc.metadata.publication_date
            ).days / 365
    ...
    def _score_cross_reference(self, cross_ref_results: CrossReferenceResult) -> float:
        ...
        if supporting_count + contradicting_count > 0:
            ...
            balance_factor = (
                2 * support_ratio * (1 - support_ratio) + 0.5 * support_ratio
            )
        else:
    ...
    def _score_citations(self, citation_validations: List[CitationValidation]) -> float:
        ...
        high_confidence_citations = sum(
            1 for c in citation_validations if c.validation_confidence >= 0.8
        )
        ...
        if valid_citations > 0:
            avg_confidence = (
                sum(c.validation_confidence for c in citation_validations if c.found)
                / valid_citations
            )
    ...
    def _score_expert_consensus(self, graph_validation: GraphValidation) -> float:
        ...
        final_score = (
            base_score + consistency_bonus + support_bonus - contradiction_penalty
        )
        return max(0.0, min(1.0, final_score))

    def _score_academic_rigor(
        self, content_analysis: ContentAnalysis, scraped_doc: ScrapedDocument
    ) -> float:
        ...
        domain_specificity = (
            max(content_analysis.domain_mapping.values())
        ...
        verifiable_claims = sum(
            1 for claim in content_analysis.claims if claim.verifiable
        )
        ...
        entity_density = len(content_analysis.entities) / max(
            1, scraped_doc.metadata.word_count / 100
        )
    ...
    def _generate_improvements(
        ...
        if scores["source_authority"] < 0.6:
            suggestions.append(
                "Consider sourcing from more authoritative domains (.edu, .gov, established publishers)"
            )

        if scores["citation_quality"] < 0.6:
            suggestions.append(
                "Add properly formatted academic citations with verifiable sources"
            )

        if scores["academic_rigor"] < 0.6:
            suggestions.append(
                "Improve content structure with methodology, results, and conclusions sections"
            )
        ...
        if scores["expert_consensus"] < 0.6:
            suggestions.append(
                "Check consistency with established knowledge in the field"
            )
        ...
        if len(content_analysis.entities) < 3:
            suggestions.append(
                "Content could benefit from more specific entities and terminology"
            )
    ...
    def _generate_detailed_analysis(
        ...
        if content_analysis.domain_mapping:
            primary_domain = max(
                content_analysis.domain_mapping.items(), key=lambda x: x[1]
            )[0]
