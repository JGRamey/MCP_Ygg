...
class DocumentNode(GraphNode):
    ...
    era: Optional[
        str
    ]  # Ancient, Classical, Medieval, Renaissance, Modern, Contemporary
    source: str
    language: str
    word_count: int
...
class ConceptNode(GraphNode):
    ...
    def generate_id(self) -> str:
        ...
        concept_hash = hashlib.sha256(
            f"{self.name}_{self.domain}".encode()
        ).hexdigest()[:12]
...
class EventNode(GraphNode):
    ...
    def generate_id(self) -> str:
        ...
        event_hash = hashlib.sha256(
            f"{self.name}_{self.event_date}".encode()
        ).hexdigest()[:12]
...
class PatternNode(GraphNode):
    ...
    def generate_id(self) -> str:
        ...
        pattern_hash = hashlib.sha256(
            f"{self.name}_{self.pattern_type}".encode()
        ).hexdigest()[:12]
...
class GraphRelationship:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for Neo4j"""
        return {"created_at": self.created_at.isoformat(), **self.properties}
...
class GraphBuilder:
    ...
    def add_document(self, processed_doc: Dict) -> Optional[str]:
        ...
        try:
            ...
            result = self.neo4j.execute_query(
                doc_query, {"properties": doc_node.to_dict(), "domain": doc_node.domain}
            )
    ...
    def _add_person_entity(self, doc_id: str, person_name: str) -> None:
        ...
        self.neo4j.execute_query(person_query, {"name": person_name, "doc_id": doc_id})

    def _add_concept_entity(
        self, doc_id: str, concept_name: str, concept_type: str
    ) -> None:
        ...
        self.neo4j.execute_query(
            concept_query,
            {"name": concept_name, "concept_type": concept_type, "doc_id": doc_id},
        )
    ...
    def _add_event_entity(self, doc_id: str, event_name: str) -> None:
        ...
        self.neo4j.execute_query(event_query, {"name": event_name, "doc_id": doc_id})
    ...
    def add_cross_domain_pattern(self, pattern: PatternNode) -> Optional[str]:
        ...
        try:
            ...
            result = self.neo4j.execute_query(
                pattern_query, {"properties": pattern.to_dict()}
            )
            ...
            if pattern_id:
                ...
                for domain in pattern.domains:
                    ...
                    """
                    self.neo4j.execute_query(
                        domain_query, {"pattern_id": pattern_id, "domain": domain}
                    )
