...
class QueryOptimizer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.performance_cache_size = config.get("performance", {}).get(
            "cache_size", 1000
        )
    ...
    async def optimize_query(
        ...
        try:
            ...
            logger.debug(
                f"Query optimized with {len(optimization_result.optimizations_applied)} improvements"
            )
    ...
    def _remove_unnecessary_optional(self, query: str, pattern_info: Dict) -> Dict:
        ...
        for match in matches:
            ...
            regular_match = optional_match.replace("OPTIONAL MATCH", "MATCH").replace(
                " IS NOT NULL", ""
            )
    ...
    def _optimize_relationship_direction(self, query: str, pattern_info: Dict) -> Dict:
        ...
        for match in matches:
            ...
            if self._can_optimize_direction(start_node, end_node, query):
                ...
                rel_part = (
                    f"[{rel_var}{'*' + depth if depth else ''}]" if rel_var else ""
                )
    ...
    def _can_optimize_direction(
        self, start_node: str, end_node: str, query: str
    ) -> bool:
        ...
        for source_type, relationship, target_types in direction_hints:
            if source_type.lower() in query_lower and any(
                target.lower() in query_lower for target in target_types
            ):
                return True
    ...
    def _update_query_stats(
        self, query_hash: str, query: str, optimization_result: OptimizationResult
    ):
    ...
    def get_query_stats(self) -> Dict[str, Any]:
        ...
        frequent_queries = sorted(
            self.query_stats.values(), key=lambda x: x.execution_count, reverse=True
        )[:10]
        ...
        slow_queries = sorted(
            self.query_stats.values(), key=lambda x: x.avg_execution_time, reverse=True
        )[:10]
        ...
        optimized_queries = sum(
            1 for stats in self.query_stats.values() if stats.optimizations_applied
        )

        if self.query_stats:
            stats_summary["optimization_success_rate"] = optimized_queries / len(
                self.query_stats
            )
