...
class SemanticRelationshipDetector:
    ...
    def detect_semantic_relationships(
        self, nodes: List[NodeInfo], content: str
                        ...
                        for pattern in compatible_patterns:
                            ...
                            if content_support:
                                ...
                                proposal = RelationshipProposal(
                                    ...
                                    evidence=[
                                        f"Semantic similarity: {node_similarity:.3f}",
                                        "Content analysis",
                                    ],
    ...
    def _find_compatible_patterns(
        self, node1: NodeInfo, node2: NodeInfo
        ...
        for pattern in self.relationship_patterns:
            ...
            node2_matches_from = any(
                label in pattern.from_labels for label in node2.labels
            )
    ...
    def _check_content_support(
        self, content: str, pattern: RelationshipPattern
    ) -> float:
...
class WebCrawlRelationshipDetector:
    ...
    async def crawl_for_relationships(
                ...
                for query in search_queries:
                    try:
                        ...
                        for url in urls[:3]:  # Limit to first 3 results
                            try:
                                relationships = (
                                    await self._extract_relationships_from_url(
                                        url, node, nodes
                                    )
                                )
    ...
    async def _extract_relationships_from_url(
        self, url: str, target_node: NodeInfo, all_nodes: List[NodeInfo]
    ) -> List[RelationshipProposal]:
                    ...
                    for pattern in patterns:
                        ...
                        for match in matches:
                            ...
                            matching_node = self._find_matching_node(
                                related_entity, all_nodes
                            )

                            if (
                                matching_node
                                and matching_node.node_id != target_node.node_id
                            ):
                                relationship_type = self._infer_relationship_type(
                                    pattern, extractor_type
                                )
    ...
    def _find_matching_node(
        self, entity_text: str, nodes: List[NodeInfo]
    ) -> Optional[NodeInfo]:
...
class UserRelationshipInterface:
    ...
    def add_user_relationship(
        ...
        logger.info(
            f"User relationship proposed: {from_node_id} -> {relationship_type} -> {to_node_id}"
        )
...
class RelationshipManager:
    ...
    def __init__(
        self, config_path: str = "agents/node_relationship_manager/config.yaml"
    ):
        ...
        if proposals_file.exists():
            try:
                ...
                for proposal_data in data.get("proposals", []):
                    ...
                    proposal_data["proposed_at"] = datetime.fromisoformat(
                        proposal_data["proposed_at"]
                    )
                    if proposal_data.get("reviewed_at"):
                        proposal_data["reviewed_at"] = datetime.fromisoformat(
                            proposal_data["reviewed_at"]
                        )
                ...
                logger.info(
                    f"Loaded {len(self.relationship_proposals)} existing relationship proposals"
                )
    ...
    async def discover_relationships(
        ...
        try:
            ...
            if self.config.get("enable_semantic_analysis", True):
                ...
                for domain, domain_nodes in domain_groups.items():
                    if len(domain_nodes) > 1:
                        ...
                        semantic_proposals = (
                            self.semantic_detector.detect_semantic_relationships(
            ...
            if self.config.get("enable_web_crawling", True):
                ...
                web_proposals = await self.web_detector.crawl_for_relationships(
                    important_nodes
                )
            ...
            logger.info(
                f"Discovered {len(filtered_proposals)} new relationship proposals"
            )
    ...
    def _select_important_nodes(
        self, nodes: List[NodeInfo], limit: int
    ) -> List[NodeInfo]:
    ...
    def _filter_and_deduplicate_proposals(
        ...
        for proposal in proposals:
            ...
            if self._relationship_exists(
                proposal.from_node_id, proposal.to_node_id, proposal.relationship_type
            ):
                continue
    ...
    def review_proposal(
        self, proposal_id: str, approved: bool, reviewer: str, notes: str = ""
    ) -> bool:
            ...
            if proposal.proposal_id == proposal_id:
                ...
                if approved:
                    ...
                    if success:
                        ...
                        logger.error(
                            f"Failed to create approved relationship: {proposal_id}"
                        )
                else:
    ...
    def auto_approve_high_confidence_proposals(self) -> int:
        ...
        for proposal in self.relationship_proposals:
            if (
                proposal.approved is None
                and proposal.confidence >= threshold
                and not self.config["approval_requirements"].get(
                    proposal.source_type, True
                )
            ):
                ...
                proposal.reviewed_at = datetime.now()
                proposal.notes = (
                    f"Auto-approved (confidence: {proposal.confidence:.3f})"
                )
    ...
    def get_relationship_statistics(self) -> Dict[str, Any]:
        ...
        rel_type_stats = Counter(
            p.relationship_type for p in self.relationship_proposals
        )
        ...
        return {
            ...
            "average_confidence": (
                sum(p.confidence for p in self.relationship_proposals) / total
                if total > 0
                else 0
            ),
        }
...
async def main():
    ...
    try:
        ...
        for proposal in pending[:5]:  # Show first 5
            print(
                f"- {proposal.from_node_id} -> {proposal.relationship_type} -> {proposal.to_node_id}"
            )
            print(
                f"  Confidence: {proposal.confidence:.3f}, Source: {proposal.source_type}"
            )
        ...
        manager.review_proposal(
            user_proposal.proposal_id, True, "admin", "User-provided historical fact"
        )
