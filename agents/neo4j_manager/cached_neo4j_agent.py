...
from .neo4j_agent import Neo4jAgent, NodeData, OperationResult, RelationshipData
...
class CachedNeo4jAgent(Neo4jAgent):
    ...
    async def get_relationships_by_type(
        self, relationship_type: str, limit: int = 100
    ) -> OperationResult:
    ...
    async def create_node_with_cache_invalidation(
        ...
        if result.success:
            ...
            await self.cache_manager.invalidate_domain_cache(
                node_data.properties.get("domain", "")
            )
            await self.cache_manager.clear_analytics_cache()

        return result

    async def update_node_with_cache_invalidation(
    ...
    async def delete_node_with_cache_invalidation(
        self, node_id: str
    ) -> OperationResult:
    ...
    async def create_relationship_with_cache_invalidation(
    ...
    async def execute_cypher(
        ...
        if any(
            ...
            for keyword in ["CREATE", "UPDATE", "DELETE", "MERGE", "SET"]
        ):
