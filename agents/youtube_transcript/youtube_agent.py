...
class YouTubeAgent:
    ...
    async def get_video_info(self, video_id: str) -> Optional[YouTubeVideo]:
        ...
        try:
            ...
            if duration_seconds < min_duration:
                logger.warning(f"Video too short: {duration_seconds}s < {min_duration}s")
                return None
            ...
            chapters = await self._extract_chapters(video_id, snippet.get('description', ''))
    ...
    async def _extract_chapters(self, video_id: str, description: str) -> List[Dict[str, Any]]:
        """
    ...
    async def get_transcript(self, video_id: str, language: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            target_language = language or self.config.get('processing', {}).get('default_language', 'en')
            ...
            if not selected_transcript:
                ...
                if available_transcripts:
                    ...
                    if self.config.get('processing', {}).get('auto_translate', True):
                        try:
                            selected_transcript = selected_transcript.translate(target_language)
                        except:
                            pass
    ...
    async def process_video(self, url: str, language: Optional[str] = None) -> ProcessingResult:
        """
        ...
        try:
            ...
            metadata = await self.metadata_extractor.extract_metadata(video_info, transcript)
    ...
    async def process_playlist(self, url: str, language: Optional[str] = None) -> List[ProcessingResult]:
        """
