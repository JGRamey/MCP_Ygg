...
class EfficientYouTubeAgent:
    ...
    async def get_transcript(
        ...
        try:
                ...
                try:
                    ...
                    if available_transcripts:
                        ...
                        if hasattr(transcript, "translate") and language_preference:
                            try:
                                selected_transcript = transcript.translate(
            ...
            max_length = (
                self.config.get("scraping", {})
                .get("youtube", {})
                .get("max_transcript_length", 50000)
            )
            if len(full_text) > max_length:
                logger.warning(
                    f"Transcript truncated from {len(full_text)} to {max_length} characters"
                )
    ...
    async def extract_transcript(
        self, youtube_url: str, extract_metadata: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = {"video_id": video_id, "url": youtube_url, "success": False}
            ...
            if extract_metadata:
                ...
                if "error" not in metadata_result:
                    result["metadata"] = {
                        ...
                        "description": (
                            metadata_result["description"][:500] + "..."
                            if len(metadata_result["description"]) > 500
                            else metadata_result["description"]
                        ),
                    }
                else:
                    result["metadata"] = {
    ...
    async def batch_extract_transcripts(
        self, urls: List[str], extract_metadata: bool = True
    ) -> List[Dict[str, Any]]:
        """
...
async def extract_youtube_transcript(
    url: str, extract_metadata: bool = True
) -> Dict[str, Any]:
    """
