...
class MetadataExtractor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.domain_keywords = {
            'art': [
                'painting', 'sculpture', 'gallery', 'museum', 'artist', 'artwork',
                'canvas', 'exhibition', 'aesthetic', 'visual', 'design', 'creative'
            ],
            'science': [
                'research', 'experiment', 'hypothesis', 'theory', 'data', 'analysis',
                'biology', 'chemistry', 'physics', 'mathematics', 'scientific', 'study'
            ],
            'philosophy': [
                'ethics', 'metaphysics', 'logic', 'philosophy', 'philosopher', 'argument',
                'reasoning', 'moral', 'virtue', 'truth', 'knowledge', 'wisdom'
            ],
            'technology': [
                'software', 'hardware', 'programming', 'algorithm', 'computer', 'digital',
                'artificial intelligence', 'machine learning', 'coding', 'technology'
            ],
            'language': [
                'linguistics', 'grammar', 'vocabulary', 'literature', 'writing', 'language',
                'communication', 'rhetoric', 'poetry', 'prose', 'narrative', 'discourse'
            ],
            'mathematics': [
                'equation', 'formula', 'calculation', 'theorem', 'proof', 'geometry',
                'algebra', 'calculus', 'statistics', 'probability', 'mathematical'
            ]
        }
    ...
    async def extract_metadata(self, video_info, transcript_data: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if transcript_data:
                ...
                # Temporal analysis
                metadata['temporal_analysis'] = self._analyze_temporal_content(text_content)
        ...
        try:
            return {
                ...
                'categorization': {
                    ...
                    'channel_type': self._classify_channel_type(video_info.channel_name)
                }
            }
    ...
    def _classify_academic_domains(self, text: str, video_info) -> Dict[str, Any]:
        ...
        for domain, keywords in self.domain_keywords.items():
            ...
            for keyword in keywords:
                ...
                desc_matches = description_lower.count(keyword) * 2  # Description weight
        ...
        return {
            ...
            'classification_confidence': domain_scores.get(primary_domain, {}).get('confidence', 0.0)
        }
    ...
    def _calculate_quality_metrics(self, transcript_data: Dict[str, Any], video_info) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            has_references = len(self._find_references(transcript_data.get('cleaned_text', ''))) > 0
            complexity = self._calculate_complexity_score(transcript_data.get('cleaned_text', ''))
            
            # Overall quality score
            quality_factors = [
                (not auto_generated) * 0.2,  # Manual transcripts are higher quality
                min(confidence_score, 1.0) * 0.15,
                min(abs(sentiment_score) * 2, 1.0) * 0.1,  # Neutral to positive sentiment
            ...
            return {
                ...
                'educational_indicators': {
                    ...
                    'reference_density': len(self._find_references(transcript_data.get('cleaned_text', ''))) / max(word_count / 1000, 1)
                }
            }
    ...
    def _extract_topics(self, text: str) -> List[str]:
        ...
        common_words = {'this', 'that', 'with', 'have', 'will', 'from', 'they', 'been', 'have', 'their', 'said', 'each', 'which', 'make', 'like', 'into', 'time', 'very', 'when', 'come', 'here', 'just', 'also', 'only', 'know', 'take', 'people', 'could', 'year', 'work', 'well', 'world', 'think', 'still', 'after', 'being', 'now', 'made', 'before', 'through', 'where', 'much', 'good', 'never', 'again', 'right', 'little', 'these', 'going', 'something', 'important', 'different', 'using', 'really', 'things', 'about', 'other', 'between', 'because', 'should', 'looking', 'need', 'there', 'actually', 'pretty', 'getting', 'doing', 'might', 'want', 'what', 'even', 'more', 'most', 'some', 'many', 'than', 'over', 'such', 'both', 'those', 'does', 'would', 'while', 'first', 'second', 'third', 'example', 'part', 'next', 'last', 'video', 'today', 'back', 'going', 'show', 'talk', 'tell', 'look', 'see'}
    ...
    def _extract_concepts(self, text: str) -> List[str]:
        ...
        for pattern in concept_patterns:
            ...
            for match in matches:
                if len(match) > 3 and match.lower() not in ['this', 'that', 'with', 'from']:
    ...
    def _calculate_complexity_score(self, text: str) -> float:
        ...
        complexity = (
            ...
            (avg_sentence_length - 15) / 15 * 0.4 +  # Normalize around 15 words/sentence
    ...
    def _assess_educational_value(self, text: str, video_info) -> float:
        ...
        academic_keywords = ['research', 'study', 'analysis', 'theory', 'hypothesis', 
                           'evidence', 'conclusion', 'methodology', 'data', 'experiment']
        academic_count = sum(1 for keyword in academic_keywords if keyword in text.lower())
    ...
    def _categorize_content(self, text: str, video_info) -> List[str]:
        ...
        if any(word in title_lower for word in ['lecture', 'course', 'lesson', 'tutorial']):
            categories.append('educational')
        
        if any(word in title_lower for word in ['interview', 'discussion', 'conversation']):
    ...
    def _classify_channel_type(self, channel_name: str) -> str:
        ...
        if any(word in name_lower for word in ['university', 'college', 'edu', 'academy']):
            return 'academic'
        elif any(word in name_lower for word in ['tv', 'news', 'media', 'broadcasting']):
            return 'media'
    ...
    def _classify_temporal_marker(self, text: str) -> str:
        ...
        if re.match(r'\d{4}', text):
            ...
            return 'era'
        elif text.lower() in ['ancient', 'medieval', 'renaissance', 'modern', 'contemporary']:
            return 'period'
        else:
            return 'other'
    
    def _identify_historical_periods(self, temporal_markers: List[Dict[str, Any]]) -> List[str]:
        """
    ...
    def _analyze_chronological_focus(self, temporal_markers: List[Dict[str, Any]]) -> str:
        """
