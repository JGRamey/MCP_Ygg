...
class ChartGenerator:
        ...
        try:
            ...
            if export_format == "html":
                output_file = self.html_exporter.export_html(
                    viz_data, title, VisualizationType.YGGDRASIL_TREE, layout_params
                )
            elif export_format == "svg":
                output_file = self.html_exporter.export_svg(
                    viz_data, title, VisualizationType.YGGDRASIL_TREE
                )
    ...
    async def generate_network_graph(
        ...
        try:
            ...
            if export_format == "html":
                output_file = self.html_exporter.export_html(
                    viz_data, title, VisualizationType.NETWORK_GRAPH, layout_params
                )
            elif export_format == "svg":
                output_file = self.html_exporter.export_svg(
                    viz_data, title, VisualizationType.NETWORK_GRAPH
                )
            elif export_format == "png":
                output_file = self.html_exporter.export_png(
                    viz_data, title, VisualizationType.NETWORK_GRAPH
