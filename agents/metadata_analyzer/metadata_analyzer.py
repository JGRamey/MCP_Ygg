...
class LanguageDetector:
    ...
    def __init__(self):
        ...
        self.ancient_language_patterns = {
            'latin': {
                'patterns': [r'\b(et|in|ad|de|ex|cum|pro|per|ab|sine)\b', r'\w+us\b', r'\w+um\b', r'\w+is\b'],
                'common_words': ['et', 'in', 'ad', 'de', 'ex', 'cum', 'pro', 'per', 'ab', 'sine', 'est', 'sunt']
            },
            'greek': {
...
class StructureAnalyzer:
    ...
    def analyze_structure(self, text: str) -> Tuple[List[Chapter], List[Dict[str, Any]]]:
    ...
    def _match_chapter_patterns(self, line: str) -> Optional[Dict[str, Any]]:
        ...
        for pattern in self.chapter_patterns:
            ...
            if match:
                ...
                if len(groups) >= 2:
                    return {
                        'number': self._parse_chapter_number(groups[1]) if len(groups) > 1 else None,
    ...
    def _calculate_chapter_metrics(self, chapters: List[Chapter], text: str) -> None:
        ...
        for i, chapter in enumerate(chapters):
            ...
            chapter.summary = '. '.join(meaningful_sentences[:2]) + '.' if meaningful_sentences else ""
...
class KeywordExtractor:
    ...
    def __init__(self):
        ...
        self.domain_terms = {
            'religion': ['god', 'divine', 'sacred', 'holy', 'spiritual', 'faith', 'prayer', 'scripture'],
            'science': ['theory', 'hypothesis', 'experiment', 'data', 'analysis', 'research', 'study'],
            'philosophy': ['truth', 'knowledge', 'existence', 'consciousness', 'reality', 'ethics'],
            'math': ['theorem', 'proof', 'equation', 'formula', 'calculation', 'mathematics'],
            'literature': ['character', 'plot', 'theme', 'narrative', 'story', 'poetry', 'drama'],
            'history': ['civilization', 'empire', 'war', 'culture', 'society', 'period', 'era']
        }
    
    def extract_keywords(self, text: str, domain: Optional[str] = None, max_keywords: int = 20) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            combined_keywords = self._combine_keyword_lists(tfidf_keywords, freq_keywords)
    ...
    def _is_valid_phrase(self, phrase: str) -> bool:
        ...
        return (len(words) >= 2 and 
                len(words) <= 5 and
                not any(word in self.stop_words for word in words) and
                not phrase.isdigit())
    
    def _boost_domain_terms(self, keywords: List[Tuple[str, float]], domain: str) -> List[Tuple[str, float]]:
    ...
    def _combine_keyword_lists(self, list1: List[Tuple[str, float]], list2: List[Tuple[str, float]]) -> List[Tuple[str, float]]:
...
class ConceptAnalyzer:
    ...
    def __init__(self):
        ...
        self.domain_concepts = {
            'religion': [
                'divine', 'sacred', 'holy', 'spiritual', 'faith', 'belief', 'prayer', 'worship',
                'scripture', 'doctrine', 'theology', 'salvation', 'redemption', 'grace',
                'sin', 'virtue', 'soul', 'eternal', 'heaven', 'hell', 'angel', 'demon'
            ],
            'science': [
                'theory', 'hypothesis', 'experiment', 'observation', 'data', 'analysis',
                'research', 'method', 'evidence', 'discovery', 'innovation', 'technology',
                'evolution', 'genetics', 'physics', 'chemistry', 'biology', 'astronomy'
            ],
            'philosophy': [
                'truth', 'knowledge', 'wisdom', 'existence', 'being', 'consciousness',
                'reality', 'perception', 'ethics', 'morality', 'justice', 'beauty',
                'good', 'evil', 'mind', 'reason', 'logic', 'argument', 'premise'
            ],
            'math': [
                'number', 'equation', 'formula', 'theorem', 'proof', 'calculation',
                'algebra', 'geometry', 'calculus', 'statistics', 'probability',
                'infinity', 'zero', 'function', 'variable', 'constant', 'set'
            ],
            'literature': [
                'narrative', 'story', 'character', 'plot', 'theme', 'symbol', 'metaphor',
                'poetry', 'drama', 'comedy', 'tragedy', 'epic', 'novel', 'verse',
                'prose', 'style', 'voice', 'perspective', 'archetype', 'motif'
            ],
            'history': [
                'civilization', 'culture', 'society', 'empire', 'kingdom', 'dynasty',
                'war', 'battle', 'revolution', 'reform', 'progress', 'decline',
                'tradition', 'custom', 'legacy', 'influence', 'change', 'development'
            ]
        }
    
    def analyze_concepts(self, text: str, domain: Optional[str] = None) -> List[Concept]:
...
class MetadataAnalyzer:
    ...
    async def analyze_document(self, document_data: Dict[str, Any]) -> TextMetadata:
        ...
        try:
            ...
            content = document_data.get('cleaned_content', '') or document_data.get('content', '')
            ...
            # Entity analysis
            entities_list, entity_summary = self.entity_analyzer.analyze_entities(content)
            entities = [asdict(entity) for entity in entities_list]
            
            # Concept analysis
            concepts_list = self.concept_analyzer.analyze_concepts(content, domain)
            concepts = [asdict(concept) for concept in concepts_list[:self.config.get('max_concepts', 25)]]
            
            # Readability analysis
            readability_scores = {}
            if self.config.get('enable_readability', True):
                readability_scores = self.readability_analyzer.calculate_readability_scores(content)
            ...
            # Generate summary
            summary = self._generate_summary(content, self.config.get('summary_sentences', 3))
            ...
            logger.info(f"Generated metadata for document {doc_id} in {processing_time:.2f}s")
            return metadata
    ...
    def _calculate_quality_scores(self, content: str, word_count: int, sentence_count: int, lang_confidence: float) -> Dict[str, float]:
        ...
        if sentence_lengths:
            ...
            scores['structure_quality'] = min(1.0, 1.0 - abs(avg_length - 15) / 15)  # Optimal ~15 words
        else:
    ...
    def _extract_citations_and_references(self, content: str) -> Tuple[List[Dict[str, Any]], List[str]]:
        """Extract citations and references"""
        citations = []
        references = []
    ...
    def _generate_tags(self, keywords: List[Tuple[str, float]], concepts: List[Concept], domain: Optional[str]) -> List[str]:
    ...
    def _determine_subjects(self, concepts: List[Concept], domain: Optional[str]) -> List[str]:
    ...
    def _calculate_confidence_score(self, lang_confidence: float, quality_scores: Dict[str, float], word_count: int) -> float:
