...
class EnhancedReasoningAgent:
    ...
    def __init__(self, use_langchain: bool = True):
        ...
        logger.info(f"   LangGraph: {'Available' if LANGGRAPH_AVAILABLE else 'Not available'}")
    ...
    def create_content_quality_analyzer(self) -> Optional[LLMChain]:
        ...
        prompt = PromptTemplate(
            input_variables=[
                "title", "author", "source_url", "date_published", "content_text",
                "has_author", "has_date", "domain", "content_length", "verification_results"
            ],
            template=template
        )
    ...
    def _rule_based_scraping_decision(self, url: str, requirements: Dict) -> Dict:
        ...
        # Academic sites
        if any(academic in domain for academic in ['arxiv.org', 'pubmed', 'scholar.google']):
...
async def test_enhanced_reasoning():
    ...
    sample_content = {
        ...
        'main_text': 'This paper presents a comprehensive analysis of machine learning applications in academic research...' * 10,
        'date': '2024-01-15'
    }
