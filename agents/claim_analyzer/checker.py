...
from .exceptions import EvidenceSearchError, FactCheckingError, RateLimitError
...
class FactChecker:
    ...
    def __init__(self, db_connector: DatabaseConnector, config: Dict[str, Any]):
        ...
        self.source_credibility = config.get(
            "source_credibility",
            {
                "wikipedia.org": 0.8,
                "snopes.com": 0.9,
    ...
    async def fact_check_claim(self, claim: Claim) -> FactCheckResult:
        ...
        self.verification_steps = []
        self._add_step(
            "evidence_collection", "Starting multi-source evidence collection"
        )
        ...
        try:
            with PerformanceTimer("enhanced_fact_check"):
                ...
                self._add_step(
                    "cross_verification", f"Cross-verified {len(evidence_list)} sources"
                )
                ...
                logger.info(
                    f"Enhanced fact-check complete: {verdict} (confidence: {confidence:.2f})"
                )
                return result
            ...
            raise FactCheckingError(
                f"Fact-checking failed for claim: {claim.claim_id}"
            ) from e
        ...
        try:
            async with self.db_connector.neo4j_driver.session() as session:
                ...
                result = await session.run(
                    query, {"claim_id": claim.claim_id, "domain": claim.domain}
                )
    ...
    async def _search_external_apis(self, claim: Claim) -> List[Evidence]:
        ...
        if "moon landing" in claim_lower and any(
            term in claim_lower for term in ["fake", "hoax", "staged"]
        ):
            evidence = Evidence(
    ...
    def _analyze_evidence(
        self, claim: Claim, evidence_list: List[Evidence], similar_claims: List[Dict]
    ) -> Tuple[str, float, str]:
        ...
        for evidence in evidence_list:
            ...
            if evidence.stance == "neutral":
                evidence.stance = self._determine_evidence_stance(
            ...
            reasoning_parts.append(
                f"Source: {evidence.source_url[:50]}... - {evidence.stance} (credibility: {credibility:.2f})"
            )
        ...
        avg_credibility = (
            total_credibility / len(evidence_list) if evidence_list else 0.0
        )
        confidence = min(confidence * avg_credibility, 1.0)

        reasoning = (
            f"Based on {len(evidence_list)} evidence sources and {len(similar_claims)} similar claims. "
    ...
    def _determine_evidence_stance(self, claim_text: str, evidence_text: str) -> str:
        ...
        refute_count = sum(
            1 for indicator in refute_indicators if indicator in evidence_lower
        )
        ...
        try:
            async with self.db_connector.neo4j_driver.session() as session:
                ...
                result_id = hashlib.md5(
                    f"{result.claim.claim_id}{result.timestamp}".encode()
                ).hexdigest()
    ...
    async def _enhanced_evidence_search(self, claim: Claim) -> List[Evidence]:
        ...
        self._add_step(
            "multi_source_search", f"Collected {len(evidence_list)} pieces of evidence"
        )
        return self._deduplicate_evidence(evidence_list)
    ...
    def _enhanced_evidence_analysis(
        self, claim: Claim, evidence_list: List[Evidence], similar_claims: List[Dict]
    ) -> Tuple[str, float, str]:
        ...
        diversity_bonus = min(
            source_count * 0.05, 0.15
        )  # Up to 15% bonus for source diversity
        ...
        reasoning = (
            f"Based on {len(evidence_list)} sources from {source_count} unique origins. "
            f"Support score: {support_score:.2f}, Refute score: {refute_score:.2f}."
        )

        self._add_step(
            "analysis_complete", f"Verdict: {verdict}, Confidence: {confidence:.2f}"
        )
        return verdict, confidence, reasoning
