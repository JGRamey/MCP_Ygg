...
class DeepContentAnalyzer:
    ...
    def __init__(self):
        ...
        if TRANSFORMERS_AVAILABLE:
            try:
                ...
                logger.warning("Transformer models not initialized. Using fallback methods.")
    ...
    def extract_concepts_from_text(self, doc, domain_mapping: Dict[str, float]) -> List[ConceptExtraction]:
        ...
        primary_domain = max(domain_mapping.items(), key=lambda x: x[1])[0] if domain_mapping else 'general'
        ...
        for phrase in noun_phrases[:30]:  # Top 30 phrases
            ...
            related = [ent.text for ent in doc.ents 
                      if phrase in ent.text or ent.text in phrase]
    ...
    def _is_verifiable_claim(self, text: str, claim_type: str) -> bool:
        ...
        if claim_type == 'factual':
            ...
            if any(word in text.lower() for word in 
                   ['first', 'last', 'only', 'never', 'always', 'discovered', 'invented']):
                return True
    ...
    def analyze_sentiment_and_tone(self, text: str) -> Dict:
        ...
        sentiment_result = {'sentiment': 'neutral', 'confidence': 0.5, 'tone': 'neutral'}
        ...
        text_lower = text.lower()
        if any(term in text_lower for term in ['research', 'study', 'analysis', 'methodology']):
            sentiment_result['tone'] = 'academic'
        elif '!' in text or '?' in text[:100]:
            sentiment_result['tone'] = 'conversational'
        elif any(term in text_lower for term in ['therefore', 'hence', 'thus', 'consequently']):
