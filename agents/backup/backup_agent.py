...
class CloudStorageManager:
    ...
    async def upload_backup(self, local_path: Path, backup_id: str, metadata: BackupMetadata) -> List[str]:
        ...
        if self.aws_client and self.config.get('aws', {}).get('enabled', False):
            try:
                ...
                self.aws_client.upload_file(str(local_path), bucket, s3_key, ExtraArgs=extra_args)
    ...
    async def download_backup(self, backup_id: str, component: str, local_path: Path) -> bool:
        """Download backup from cloud storage"""
        # Try each storage provider until successful
        for provider in ['aws', 'azure', 'gcp']:
            if await self._download_from_provider(provider, backup_id, component, local_path):
                return True
        
        return False
    
    async def _download_from_provider(self, provider: str, backup_id: str, component: str, local_path: Path) -> bool:
...
class Neo4jBackupManager:
    ...
    async def create_backup(self, backup_path: Path) -> Tuple[bool, str, int]:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                logger.info(f"Neo4j backup completed: {compressed_file} ({file_size} bytes)")
    ...
    async def restore_backup(self, backup_file: Path, target_database: str = None) -> Tuple[bool, str]:
...
class QdrantBackupManager:
    ...
    async def create_backup(self, backup_path: Path) -> Tuple[bool, str, int]:
        ...
        try:
            ...
            for collection_name in collections:
                ...
                success, size = await self._backup_collection(collection_name, collection_file)
                if success:
            ...
            if backup_files:
                ...
                logger.info(f"Qdrant backup completed: {archive_file} ({final_size} bytes)")
        ...
        try:
            ...
            try:
                ...
                for collection_file in temp_dir.glob("qdrant_*.json"):
                    ...
                    success = await self._restore_collection(collection_name, collection_file)
                    if success:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.qdrant_url}/collections") as response:
                    if response.status == 200:
                        ...
                        return [coll['name'] for coll in data.get('result', {}).get('collections', [])]
                    else:
    ...
    async def _backup_collection(self, collection_name: str, output_file: Path) -> Tuple[bool, int]:
        """Backup a single Qdrant collection"""
        try:
            # Get collection info
            async with aiohttp.ClientSession() as session:
                # Get collection info
                async with session.get(f"{self.qdrant_url}/collections/{collection_name}") as response:
                    if response.status != 200:
                        return False, 0
                ...
                logger.info(f"Backed up collection {collection_name}: {file_size} bytes")
    ...
    async def _restore_collection(self, collection_name: str, backup_file: Path) -> bool:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                for i in range(0, len(points), batch_size):
                    ...
                    async with session.put(
                        ...
                        if response.status not in [200, 201]:
                            logger.error(f"Failed to restore points batch for {collection_name}")
                            return False
                
                logger.info(f"Restored collection {collection_name} with {len(points)} points")
                return True
...
class IntegrityChecker:
    ...
    async def verify_backup(self, backup_metadata: BackupMetadata, backup_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for component in backup_metadata.components:
                ...
                if not matching_files:
                    results['errors'].append(f"Missing backup file for component: {component}")
            ...
            all_checksums_ok = all(v['valid'] for v in results['checksum_verification'].values())
            all_content_ok = all(results['content_validation'].values())
            
            if all_files_ok and all_checksums_ok and all_content_ok and not results['errors']:
        ...
        try:
            ...
            if backup_file.suffix in ['.tar', '.gz']:
                with tarfile.open(backup_file, 'r:gz') as tar:
                    ...
                    return len(members) > 0 and any(m.endswith('.json') for m in members)
...
class BackupAgent:
    ...
    def load_backup_history(self) -> None:
        ...
        history_file = Path(self.config['storage']['local_backup_dir']) / "backup_history.json"
    ...
    async def create_backup(
        ...
        asyncio.create_task(self._execute_backup_job(job, components or ['neo4j', 'qdrant'], description, tags or {}))
    ...
    async def _execute_backup_job(
        ...
        try:
            ...
            if 'neo4j' in components:
                ...
                success, file_or_error, size = await self.neo4j_manager.create_backup(backup_dir)
                if success:
            ...
            if 'qdrant' in components:
                ...
                success, file_or_error, size = await self.qdrant_manager.create_backup(backup_dir)
                if success:
                    backup_files.append(file_or_error)
                    total_size += size
            ...
            if self.config['verification']['verify_after_backup']:
                ...
                verification_results = await self.integrity_checker.verify_backup(metadata, backup_dir)
                metadata.verification_results = verification_results
                
                if verification_results['overall_status'] != 'valid':
                    raise Exception(f"Backup verification failed: {verification_results['errors']}")
    ...
    async def restore_backup(
        ...
        try:
            ...
            local_backup_dir = Path(self.config['storage']['local_backup_dir']) / backup_id
            ...
            if self.config['verification']['verify_before_restore']:
                ...
                if verification_results['overall_status'] != 'valid':
                    return False, f"Backup verification failed: {verification_results['errors']}"
            ...
            if 'neo4j' in (components or backup_metadata.components):
                ...
                if neo4j_files:
                    success, message = await self.neo4j_manager.restore_backup(neo4j_files[0])
            ...
            if 'qdrant' in (components or backup_metadata.components):
                ...
                if qdrant_files:
                    success, message = await self.qdrant_manager.restore_backup(qdrant_files[0])
    ...
    def cleanup_old_backups(self) -> int:
        ...
        for backup in self.backup_history[:]:  # Create copy for iteration
            if backup.retention_until < now:
                # Remove local files
                local_backup_dir = Path(self.config['storage']['local_backup_dir']) / backup.backup_id
