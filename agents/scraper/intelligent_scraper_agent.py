...
class ContentClassifier:
    ...
    def classify(self, content: str, scraped_data: Dict) -> ContentType:
        ...
        academic_score = sum(
            1 for indicator in self.academic_indicators if indicator in content_lower
        )
        if academic_score >= 3:
            return ContentType.ACADEMIC_PAPER
        ...
        news_score = sum(
            1 for indicator in self.news_indicators if indicator in content_lower
        )
        if news_score >= 2:
            return ContentType.NEWS_ARTICLE
        ...
        # Blog post detection
        blog_score = sum(
            1 for indicator in self.blog_indicators if indicator in content_lower
        )
        if blog_score >= 2:
            return ContentType.BLOG_POST
        ...
        if any(
            term in content_lower for term in ["encyclopedia", "reference work", "wiki"]
        ):
        ...
        if any(
            ...
            for term in ["documentation", "api reference", "user guide"]
        ):
...
class IntelligentScraperAgent:
    ...
    def detect_content_type(
        self, content: str, url: str, scraped_data: Dict
    ) -> ContentType:
        """Classify content type using multiple signals."""
        ...
        url_lower = url.lower()
        if any(
            domain in url_lower for domain in [".edu", "arxiv.org", "pubmed", "jstor"]
        ):
        ...
        if any(
            domain in url_lower
            for domain in ["wikipedia.org", "britannica.com", "stanford.edu/entries"]
        ):
            return ContentType.ENCYCLOPEDIA
    ...
    def extract_metadata(
        self, scraped_data: Dict, content_type: ContentType
    ) -> DocumentMetadata:
    ...
    def assess_source_authority(
        self, url: str, metadata: DocumentMetadata, content_type: ContentType
    ) -> Tuple[float, AuthorityLevel]:
