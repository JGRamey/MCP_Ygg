...
class ScraperConfig:
    ...
    def _load_proxies_from_env(self) -> List[str]:
        ...
        if os.getenv('PROXY_LIST'):
            ...
            proxy_list.extend([p.strip() for p in os.getenv('PROXY_LIST').split(',') if p.strip()])
...
def create_custom_profile(base_profile: str = 'comprehensive', **overrides) -> ScraperConfig:
    ...
    return ScraperConfig(**{k: v for k, v in config_dict.items() if hasattr(ScraperConfig, k)})
...
if __name__ == "__main__":
    ...
    custom = create_custom_profile('academic', requests_per_second=1.5, quality_threshold=0.9)
    print(f"Custom academic profile: {custom.requests_per_second} req/s, quality: {custom.quality_threshold}")