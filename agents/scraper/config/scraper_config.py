...
SCRAPER_CONFIG = {
    ...
    "domains": {
        ...
        "history": {
            "sources": ["archive.org", "loc.gov", "europeana.eu", "worldhistory.org"],
            "keywords": [
                "historical",
                "ancient",
                "medieval",
    ...
    "source_types": {
        "academic": {
            "api_keys": {"crossref": None, "pubmed": None, "arxiv": None, "ieee": None},
            "rate_limits": {"crossref": 1.0, "pubmed": 0.5, "arxiv": 3.0, "ieee": 2.0},
            "authentication": True,
        },
        "public_domain": {
            "rate_limits": {"gutenberg": 1.0, "archive_org": 2.0, "wikisource": 1.0},
...
class UserSourcesManager:
        ...
        if self.sources_file.exists():
            try:
                with open(self.sources_file, "r") as f:
                    data = json.load(f)
                    self.sources = [
                        UserSource(**item) for item in data.get("sources", [])
                    ]
    ...
    def search_sources(self, query: str) -> List[UserSource]:
        ...
        for source in self.sources:
            if (
                query_lower in (source.title or "").lower()
                or query_lower in (source.description or "").lower()
                or any(query_lower in tag.lower() for tag in source.tags)
            ):
...
class SourceValidator:
    ...
    def __init__(self):
        self.academic_regex = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.ACADEMIC_PATTERNS
        ]
        self.public_domain_regex = [
