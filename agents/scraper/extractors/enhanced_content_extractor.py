...
class EnhancedContentExtractor:
    ...
    def __init__(self):
        ...
        logger.info(f"   LangDetect: {'Available' if LANGDETECT_AVAILABLE else 'Not available'}")
    ...
    def extract_main_content(self, html_content: str, url: str) -> Dict:
        ...
        extracted_content = {
            'main_text': content_data.get('text', '') or content_data.get('raw_text', ''),
            'title': content_data.get('title', ''),
            'author': content_data.get('author', ''),
            'date': content_data.get('date', ''),
            'tags': content_data.get('tags', []) or content_data.get('categories', []),
            'language': content_data.get('language', ''),
            'excerpt': content_data.get('excerpt', '') or content_data.get('description', ''),
            'source_url': url,
            'extraction_method': 'trafilatura'
        }
        
        # Detect language if not provided and detector available
        if not extracted_content['language'] and extracted_content['main_text'] and self.language_detector_available:
            language_info = self._detect_language(extracted_content['main_text'])
            extracted_content['language'] = language_info.get('language', '')
            extracted_content['language_confidence'] = language_info.get('confidence', 0.0)
    ...
    def extract_metadata(self, html_content: str, url: str) -> Dict:
        ...
        try:
            ...
            if traf_metadata:
                metadata.update(traf_metadata.__dict__ if hasattr(traf_metadata, '__dict__') else traf_metadata)
                    ...
                    if isinstance(item, dict):
                        ...
                        if item.get('@type') == 'Article':
                            metadata['article_data'] = {
                                ...
                                'author': self._extract_author_from_structured(item.get('author')),
                                'datePublished': item.get('datePublished'),
                                'dateModified': item.get('dateModified'),
                                'description': item.get('description'),
                                'keywords': item.get('keywords'),
                                'publisher': self._extract_publisher_from_structured(item.get('publisher'))
                            }
    ...
    def extract_links_and_references(self, html_content: str, url: str) -> Dict:
        ...
        links_data['total_links'] = len(links_data['internal_links']) + len(links_data['external_links'])
    ...
    def get_extraction_stats(self) -> Dict:
        ...
        return {
            ...
            'extraction_methods': ['trafilatura_precision', 'trafilatura_recall', 'trafilatura_basic'],
            'metadata_sources': ['trafilatura', 'json-ld', 'opengraph', 'microdata', 'microformat']
        }
...
async def test_enhanced_extractor():
    ...
    print(f"   Word count: {content.get('content_metrics', {}).get('word_count', 'N/A')}")
