...
class AdvancedLanguageDetector:
    ...
    def detect_language(self, text: str) -> Dict:
        ...
        final_result = self._combine_results(
            cld3_result, langdetect_result, cleaned_text
        )

        # Add academic language analysis
        final_result["academic_indicators"] = self._detect_academic_language(
            cleaned_text, final_result["language"]
        )
    ...
    def _combine_results(
        self, cld3_result: Optional[Dict], langdetect_result: Optional[Dict], text: str
    ) -> Dict:
        ...
        if len(text) > 100 and cld3_result["is_reliable"]:
            ...
            if langdetect_result["confidence"] > 0.9:
                ...
                primary_result = (
                    cld3_result if cld3_result["is_reliable"] else langdetect_result
                )
                secondary_result = (
                    langdetect_result if primary_result == cld3_result else cld3_result
                )
        ...
        if cld3_result["language"] == langdetect_result["language"]:
            ...
            result["combined_confidence"] = (
                cld3_result["confidence"] + langdetect_result["confidence"]
            ) / 2
        else:
            ...
            if secondary_result not in result.get("alternative_languages", []):
                ...
                result["alternative_languages"].insert(
                    0,
                    {
        ...
        result["detectors_used"] = [
            primary_result["detector"],
            secondary_result["detector"],
        ]
    ...
    def detect_mixed_languages(self, text: str, chunk_size: int = 500) -> Dict:
        ...
        for chunk in chunks:
            ...
            if result["language"] != "unknown":
                ...
                if (
                    result.get("is_reliable", False)
                    or result.get("confidence", 0) > 0.8
                ):
