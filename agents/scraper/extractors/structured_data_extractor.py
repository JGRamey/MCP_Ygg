...
class StructuredDataExtractor:
    ...
    def __init__(self):
        self.supported_syntaxes = ['json-ld', 'microdata', 'rdfa', 'opengraph', 'microformat']
            ...
            if isinstance(item, dict):
                ...
                if 'Article' in str(item_type) or 'NewsArticle' in str(item_type):
                    ...
                    processed.append({
                        ...
                        'name': item.get('name'),
                        'url': item.get('url'),
                        'affiliation': self._extract_affiliation(item.get('affiliation')),
                    ...
                    for breadcrumb in items:
                        if isinstance(breadcrumb, dict):
                            breadcrumbs.append({
                                ...
                                'url': breadcrumb.get('item', {}).get('@id') if isinstance(breadcrumb.get('item'), dict) else breadcrumb.get('item')
                            })
                    processed.append({
                        'type': 'BreadcrumbList',
