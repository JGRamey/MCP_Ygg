...
class AdvancedOCR:
    ...
    def extract_text_with_confidence(self, image_path: str, language: str = 'eng') -> Dict:
        ...
        try:
            ...
            result = {
                ...
                'low_confidence_words': len([c for c in data['conf'] if 0 < int(c) <= self.confidence_threshold])
            }
            ...
            return {'text': '', 'confidence': 0, 'word_count': 0, 'low_confidence_words': 0}
    
    def extract_text_from_regions(self, image_path: str, language: str = 'eng') -> List[Dict]:
    ...
    def extract_metadata_from_text(text: str) -> Dict:
        ...
        for line in lines:
            ...
            if re.search(r'by\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)', line, re.IGNORECASE):
                match = re.search(r'by\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)', line, re.IGNORECASE)
...
class URLValidator:
    ...
    ACADEMIC_DOMAINS = {
        'arxiv.org', 'pubmed.ncbi.nlm.nih.gov', 'ieee.org', 'acm.org',
        'jstor.org', 'springer.com', 'sciencedirect.com', 'nature.com',
        'scholar.google.com', 'researchgate.net'
    }
    
    PUBLIC_DOMAIN_DOMAINS = {
        'gutenberg.org', 'archive.org', 'wikisource.org', 'sacred-texts.com',
        'perseus.tufts.edu', 'ccel.org', 'bartleby.com'
    }
