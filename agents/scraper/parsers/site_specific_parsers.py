        ...
        try:
            ...
            if content_div:
                ...
                for unwanted in content_div.find_all(
                    ["table", "div"], class_=["infobox", "navbox", "reflist"]
                ):
                    unwanted.decompose()
...
class PubMedParser(BaseSiteParser):
    ...
    def can_parse(self, url: str) -> bool:
        parsed = urlparse(url)
        return (
            any(domain in parsed.netloc for domain in self.domains) and "pubmed" in url
        )
    ...
    def parse(self, html_content: str, url: str) -> ParsedContent:
        ...
        try:
            ...
            if accepted_elem:
                ...
                if answer_body:
                    accepted_answer = answer_body.get_text().strip()[
                        :500
                    ]  # First 500 chars
...
class GitHubParser(BaseSiteParser):
    ...
    def _parse_repo_page(self, soup, url: str) -> ParsedContent:
        ...
        languages = [
            lang.get_text().strip() for lang in lang_elems if lang.get_text().strip()
        ]
