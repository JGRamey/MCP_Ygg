...
class UnifiedWebScraper:
    ...
    async def scrape_url(self, url: str, **kwargs) -> ScrapingResult:
        ...
        try:
            ...
            if self.config.extract_links:
                links_data = self.content_extractor.extract_links_and_references(html_content, url)
            ...
            logger.info(f"✅ Successfully scraped {url} in {processing_time:.2f}s using {method_used}")
            return result
    ...
    async def scrape_multiple(self, urls: List[str], max_concurrent: int = 5) -> List[ScrapingResult]:
        ...
        logger.info(f"🚀 Starting batch scraping of {len(urls)} URLs (max concurrent: {max_concurrent})")
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            logger.debug(f"HTTP success for {url} (attempt {attempt + 1})")
                            return content
                        elif response.status in [403, 429]:
                            # Likely blocked, don't retry with HTTP
                            logger.warning(f"HTTP blocked ({response.status}) for {url}")
                            return None
                        else:
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'cache_hit_rate': f"{(self.stats['method_usage']['cached'] / total * 100):.1f}%" if total > 0 else "0%"
        }
