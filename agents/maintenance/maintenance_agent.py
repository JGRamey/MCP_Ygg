...
class DatabaseMaintainer:
    ...
    async def propose_action(
        ...
        if len(self.pending_actions) >= self.config.max_pending_actions:
            raise ValueError("Too many pending actions. Please review existing actions first.")
    ...
    async def _delete_node(self, details: Dict[str, Any]) -> bool:
        ...
        async with self.neo4j_driver.session() as session:
            ...
            """
            rollback_result = await session.run(rollback_query, node_id=details['node_id'])
    ...
    async def _reindex_vector(self, details: Dict[str, Any]) -> bool:
        ...
        await self.qdrant_client.recreate_collection(
            ...
            hnsw_config=models.HnswConfigDiff(
                ef_construct=details.get('ef_construct', 100),
                m=details.get('m', 16)
            )
        )
        return True
    ...
    def _cleanup_actions(self) -> None:
        ...
        completed_statuses = {ActionStatus.COMPLETED, ActionStatus.FAILED, ActionStatus.REJECTED}
        self.pending_actions = {
            action_id: action for action_id, action in self.pending_actions.items()
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        health_info = {
            ...
            "pending_actions": len([a for a in self.pending_actions.values() if a.status == ActionStatus.PENDING]),
...
async def main():
    ...
    parser.add_argument("--config", default="config/maintenance.yaml", help="Configuration file path")
    parser.add_argument("--action", choices=[
        "list", "approve", "reject", "execute", "health", "propose"
    ...
    parser.add_argument("--reason", help="Reason for rejection or justification for proposal")
    ...
    try:
        if args.action == "list":
            ...
            for action in actions:
                print(f"  {action['id']}: {action['description']} (by {action['created_by']})")
