...
class MultiSourceVerifier:
    ...
    async def verify_content(self, content: Dict[str, Any]) -> ContentVerification:
        ...
        recommendations = self._generate_recommendations(verification_results, overall_confidence)
        ...
        logger.info(f"✅ Verification complete for {content_id}: {verification_level.value} ({overall_confidence:.2f})")
        return content_verification
    
    async def _verify_academic_sources(self, content: Dict[str, Any]) -> Optional[VerificationResult]:
        ...
        if any(academic_indicators):
            ...
            evidence = [f"Academic indicator found: {indicator}" for indicator in academic_indicators if indicator]
            
            return VerificationResult(
                source_type=SourceType.ACADEMIC,
                confidence_score=min(confidence, 1.0),
                verification_level=VerificationLevel.HIGH_CONFIDENCE if confidence > 0.8 else VerificationLevel.MEDIUM_CONFIDENCE,
                evidence=evidence,
                contradictions=[],
                timestamp=datetime.now(timezone.utc),
    ...
    async def _verify_encyclopedia_sources(self, content: Dict[str, Any]) -> Optional[VerificationResult]:
        ...
        if any(encyclopedia_sources):
            ...
            evidence = [f"Encyclopedia source: {source}" for source in encyclopedia_sources if source]
    ...
    async def _verify_primary_sources(self, content: Dict[str, Any]) -> Optional[VerificationResult]:
        ...
        if any(primary_indicators):
            ...
            evidence = [f"Primary source indicator: {indicator}" for indicator in primary_indicators if indicator]
    ...
    async def _verify_metadata_consistency(self, content: Dict[str, Any]) -> Optional[VerificationResult]:
        ...
        if content.get('url') and content.get('domain'):
            url_domain = content['url'].split('//')[1].split('/')[0] if '//' in content['url'] else ''
        ...
        if completeness > 0.6:  # At least 60% fields present
            ...
            return VerificationResult(
                ...
                confidence_score=min(confidence, 1.0),
                verification_level=VerificationLevel.MEDIUM_CONFIDENCE if confidence > 0.7 else VerificationLevel.LOW_CONFIDENCE,
                evidence=[f"Metadata completeness: {completeness:.1%}"] + consistency_checks,
                contradictions=[],
                timestamp=datetime.now(timezone.utc),
                metadata={'completeness': completeness, 'consistency_checks': len(consistency_checks)}
            )
        
        return None
    
    async def _verify_pattern_matching(self, content: Dict[str, Any]) -> Optional[VerificationResult]:
        ...
        quality_patterns = [
            ...
            any(word in content_text for word in ['research', 'study', 'analysis', 'evidence']),  # Academic language
            any(word in content_text for word in ['according to', 'based on', 'source']),  # Attribution
            not any(word in content_text for word in ['click here', 'buy now', 'advertisement'])  # Not commercial
        ]
        ...
        if pattern_score > 0.5:
            confidence = pattern_score * 0.7  # Pattern matching has lower base confidence
            
            evidence = []
            if any(quality_patterns):
                evidence.append(f"Quality indicators: {sum(quality_patterns)}/{len(quality_patterns)}")
            if any(structure_patterns):
                evidence.append(f"Structure indicators: {sum(structure_patterns)}/{len(structure_patterns)}")
            
            return VerificationResult(
                source_type=SourceType.PATTERN_MATCH,
                confidence_score=confidence,
                verification_level=VerificationLevel.MEDIUM_CONFIDENCE if confidence > 0.6 else VerificationLevel.LOW_CONFIDENCE,
                evidence=evidence,
                contradictions=[],
                timestamp=datetime.now(timezone.utc),
    ...
    def _generate_recommendations(self, results: List[VerificationResult], confidence: float) -> List[str]:
        ...
        if metadata_results and metadata_results[0].metadata.get('completeness', 1.0) < 0.7:
    ...
    def get_verification_stats(self) -> Dict[str, Any]:
        ...
        avg_confidence = sum(v.overall_confidence for v in verifications) / len(verifications)
        ...
        return {
            ...
            "average_processing_time": sum(v.processing_time for v in verifications) / len(verifications)
        }
...
async def test_multi_source_verifier():
    ...
    for content in test_contents:
        ...
        print(f"   Sources: {', '.join([r.source_type.value for r in verification.results])}")
